...
def _build_risk_config(risk_cfg: Dict[str, Any]) -> RiskConfig:
    ...
    return RiskConfig(
        ...
        sizing_mode=str(risk_cfg.get("sizing_mode", "nominal")),
        risk_per_trade_pct=float(risk_cfg.get("risk_per_trade_pct", 0.7)),
        risk_max_position_pct=float(risk_cfg.get("risk_max_position_pct", 18.0)),
    ...
    try:
        ...
        for component, params in overrides.items():
            ...
            if section:
                ...
                # risk_config → kr.risk에도 머지 (KR 트레이딩 설정이 kr.risk를 우선 참조)
                if component == "risk_config":
