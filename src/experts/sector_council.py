...
신호 구성 (섹터당):
  ...
  - 정성 40%: Perplexity 1회 호출 — **뉴스 재료만** 평가 (수주·실적·정책·사고).
    ...
    LLM이 이 규칙을 어기는 사례가 있어 **±50 클램프** (2026-08-10, 반도체 -88 사례
    — 급락 자체를 -70으로 채점해 정량 -100과 이중 반영. 최종 영향 폭 ±20으로 제한)
...
class SectorCouncilExpert(ExpertAgent):
    ...
    async def _fetch_qual_signals(self) -> Dict[str, Dict[str, Any]]:
        ...
        for sector in SECTOR_ETF_MAP:
            ...
            out[sector] = {
                # ±50 클램프 (2026-08-10): LLM이 "가격 채점 금지" 규칙을 어기고
                # 급락 자체를 -70으로 채점 → 정량 축과 이중 계산되던 문제.
                # 재료 점수의 영향 폭을 최종 ±20(×0.4)으로 제한한다.
                "score": max(-50, min(50, int(sm.group(1)))),
