...
class RiskManager:
    ...
    async def on_signal(self, event: SignalEvent) -> Optional[List[Event]]:
        ...
        if event.side == OrderSide.SELL and self._is_exit_exempt(event.symbol):
            if getattr(self.engine, "_execution_runtime", None) is not None:
                # 게시된 면제 집합으로 거부하되 legacy 사유/스로틀 장부를 수정하지 않는다.
                logger.warning(f"[리스크] 연결 모드 자동매도 금지 종목 SELL 차단: {event.symbol}")
                return None
    ...
    async def on_heartbeat(self, event: Event) -> None:
        ...
        """
        # 설치된 owner의 pending/송신 책임과 legacy 재시도 writer를 섞지 않는다.
        # gateway 미설치 attach도 동일하게 차단한다(부분 설치를 legacy fallback으로 읽지 않음).
        if getattr(self.engine, "_execution_runtime", None) is not None:
            return None
