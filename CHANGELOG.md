...
## 2026-09-21 — fix(kis): 연속조회 요청 헤더 `tr_cont` 와 잘린 미체결 목록 (운영 동작 변경 2건)
...
- **무엇 ②(미체결 조회):** `get_exchange_open_orders` 는 페이지 루프가 없는 1회 호출(최대 50건)이다. `rt_cd` 성공 뒤 응답 헤더가 `F`/`M`(다음 페이지 있음)이면 잘린 목록 대신 `None`(판단 불가)을 돌려준다. 이 목록은 pending 만료(이중 매도 방지)의 근거라, 잘린 목록은 "SELL 미체결 없음"으로 오독된다. 호출자(`run_trader.py:870` pending 검증자)는 `None` 을 이미 "pending 유지"로 처리한다 — **호출자 수정 0줄**(코드로 확인만 했다). `D`/`E`/빈 값/키 부재는 현행 그대로.
