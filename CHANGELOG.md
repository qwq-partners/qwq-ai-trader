...
- **낭비** (`kr_scheduler.run_portfolio_sync`): 세션 게이트 없이 24시간 30초 → 야간·주말 8434R 1,440건/일(일요일에도 거절 315건). CLOSED 세션은 300초, 전환 직후 1회는 30초 유지(마감 직전 체결 확정). CLOSED 에선 `submit_order` 가 주문을 거부해 재정합할 신규 체결이 없다. **채택하지 않은 안**: 장중 pending 유무로 30→180초 연장 — `_pending_orders` 는 POST 성공 응답 뒤에만 등록되므로 응답 유실(체결은 됨) 시나리오에서 정확히 fail-open 하여 미등록 체결이 손절 없이 방치된다.
- 계측: `_api_get` 의 3번째 500 도 EGW00201/00215 분류·리미터 백오프를 거치고 KIS 오류 본문이 아니라 실패 dict 로 반환(이전엔 계측 누락 + 오류 본문이 정상 응답처럼 흘러 '포지션 0건' 으로 둔갑). 매수가능조회(8908R) 실패의 예수금 폴백을 DEBUG→WARNING(그 값이 `portfolio.cash` 에 들어간다).
