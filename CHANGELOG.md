...
## 2026-09-21 — fix(engine,scheduler): 취소 0건인 '살아 있는 SELL' 위 재발행 금지 — 분할 매도 이중 발행 (미배포, PR #81 위에 쌓음)

PR #81 교차 리뷰 P1-5(당시 '최우선 후속 과제'로 기록)의 수정이다. **배포·재시작·주문·설정 변경 0.** 현금 고갈로 봇 매매가 없어 두 경로 모두 현재는 실행되지 않으며, 매수 재개 시점부터 의미가 있다.

- **결함:** `KISKRBroker.cancel_all_for_symbol` 은 미연결·hashkey·rt_cd≠0·예외를 전부 삼키고 0 을 돌려준다. 100주 보유·1차 익절 10주 지정가 SELL 활성·취소 실패에서 ① 엔진 90초 폴백이 반환값을 보지 않고 시장가 10주를 추가 제출(합계 20주는 보유 수량 안이라 KIS 가 거절하지 않는다) ② 스케줄러 `_cleanup_stale_pending`(별도 장부, 3분)이 양쪽 pending 을 풀고 `rollback_stage` → ExitManager 가 같은 10주를 재발행. 방금 체결된 경우엔 롤백이 `ExitManager.on_fill` 의 stage 승격까지 지운다. 두 경로 모두 RED 시험으로 재현했다.
- **설계:** 주문 생존의 근거는 거래소 실 미체결(브로커 인메모리 장부는 1차 힌트)이고, 엔진은 '취소→시장가 재발행', 스케줄러는 '청산 의도(stage) 롤백'의 유일한 실행자다. 둘 다 같은 판정 함수 `RiskManager.stale_order_still_live`(PR #81 의 `_stale_buy_still_live` 를 방향 인자를 받도록 일반화, BUY 동작 불변)로 **생존이 부정되기 전에는 자기 행동을 하지 않는다.** 취소 ≥1건이면 두 경로 모두 종전 그대로다(지연 0).
- **엔진 0건:** 포지션 없음 → 종전 해제. 첫 회는 추적 여부와 무관하게 조회 없이 대기(장부는 완전 체결·취소 성공에서만 빠진다 — 미추적인데 pending 이 남았다면 FillEvent 가 오는 중), 20초 뒤 취소 재시도. 그 뒤 거래소에 있으면 재주문 없이 유지(상한 없음, 확인 때마다 ERROR 로그 + 5분 넘으면 헬스 모니터 교착 경보), 없거나 브로커 미추적이면 종전 시장가 폴백(원 주문 수량), 판단 불가가 **연속** 9회(270초 = 종전 최장 보유 90초×3)면 **재주문 없이** 해제. 횟수는 PR #81 의 BUY 유지와 같은 뜻(생존 확인·첫 회 대기는 1 로 되돌림)이라 오래 살아 있던 주문이 조회 실패 한 번으로 풀리지 않는다.
- **스케줄러 0건(stale·고아 루프 공통):** 브로커 미추적 → 종전 즉시 해제(주문 자체가 없던 고아가 흔한 경우 — 기다리면 청산 판정이 더 막힌다). 추적 중 첫 회 → 60초 스로틀만큼 대기. 그 뒤 거래소에 있으면 해제·롤백 없이 유지(상한 없음) + 텔레그램 CRITICAL 1회("청산 주문 취소 불가"), 없으면 종전 해제+롤백, 판단 불가가 **연속** 15회(60초 스로틀 → 종전 상한 15분)면 강제 해제. 유지 표식은 pending 등록 시각에 묶어 이전 pending 의 표식이 새 pending 의 첫 회 대기를 건너뛰게 하지 않는다.
- **청산 지연 대 이중 매도:** 전량 매도(손절)는 원 주문이 살아 있는 한 시장가를 보내도 KIS 가 주문가능수량 0 으로 거절한다 — 종전 동작도 청산을 앞당기지 못했고 거절 2회로 폴백 예산을 태운 뒤 '청산 시그널 소실'로 끝났다. 앞당기는 수단은 취소 재시도뿐이라 엔진 간격을 20초로 잡았다(일시 장애의 추가 지연 = 재시도 1회분). 분할 매도가 살아 있는 동안엔 pending 을 풀어도 전량 손절은 수량 초과로 거절되고 같은 분할 익절만 재발행되므로 **생존 확인 시 상한 없이 유지 + 사람 경보**, 판단 불가는 종전 시간 예산 안에서 해제. 엔진 스로틀은 pending 시각을 되감지 않고 별도 장부 `_pending_cancel_keep`(횟수, 마지막 시도 시각)으로 만들었다 — 되감으면 헬스 모니터의 5분 교착 경보가 '손절이 막힌 SELL 유지'를 보지 못한다. 폴백 횟수 장부와 나눠 유지가 시장가 폴백 예산(2회)을 깎지 않는다.
- **브로커 계약 변경안은 채택하지 않았다:** 호출처 6곳이 전부 try/except 라 '예외 = 유지' 분기가 한꺼번에 살아나고 운영 스크립트 동작까지 바뀐다 · 실패를 정확히 알아도 '취소 거절'은 체결/생존을 말해 주지 않아 거래소 조회는 어차피 필요하다 · 필요한 힌트는 현 계약에서 정확히 유도된다(취소 성공은 장부에서 빼므로 0건 뒤 남은 활성 주문 = 취소 실패 주문) · 엔진 실행 안전성 이행 브랜치가 그 계약을 새로 설계 중이다(중복·충돌 회피).
- **중복 구현 확인:** `feature/engine-safety-design-20260917` 은 이 결함을 S1 으로 인식하나 해법은 owner 경로 재설계이고 legacy 루프는 '현행 고정' 특성화 시험만 있으며 스케줄러 정리는 10C 로 미뤄져 있다(미병합·운영 승격 차단). 운영은 legacy 경로라 이 수정이 필요하다. 그 브랜치가 main 을 병합할 때 `test_engine_legacy_stale_eviction_characterization.py` 의 '0건 → 시장가 재주문' 기대와 `object.__new__(RiskManager)` 하네스(`_pending_cancel_keep = {}`)를 갱신해야 한다.
- **한계(잔존):** 엔진 재시도는 SIGNAL 의존(20초·9회는 하한) — ExitManager 발 매도만 틱 구동 스케줄러가 받친다 · 취소 일부 성공(≥1)은 종전 경로 · '거래소에 없음'은 체결 확인이 20/60초 넘게 밀리면 '체결 미반영'과 구분 불가(ExitManager 검증자와 같은 신뢰 수준) · 스케줄러 '미추적 즉시 해제'의 수 ms~수백 ms 창은 종전과 같음 · 고아 루프 관문은 현재 도달하지 않는 심층 방어. 상세 `docs/risk/risk-and-exit.md`.
- **검증:** RED 19건 실패(엔진 11·스케줄러 8)/회귀 가드 8건 통과 → 수정 후 신규 `tests/test_stale_sell_cancel_failure.py` 32건 + PR #81 시험 30건 UTC/KST 각 62 passed, 격리 위반 0. 방어 코드 변이 27개 중 26개 kill(생존 1 = 위 고아 루프 관문; 처음 생존했던 엔진 상한 경계는 8회 유지/9회 해제 시험으로 보강). 작업 중 PR #81 head 가 `ba966d1` → `9c9d6dc` 로 이동해 그 위로 rebase 했고(`get_exchange_open_orders(symbol=)`·'연속 판단 불가' 의미를 SELL 쪽에도 맞춤), RED 는 `ba966d1` 기준 기록이다. 엔진·스케줄러·ExitManager 를 다루는 기존 시험 묶음 KST 543 passed / 2 known xfailed — 새 장부 속성 때문에 `object.__new__(RiskManager)` 하네스 4곳에 초기화 한 줄 추가(단언 변경 0). 전체 suite·독립 리뷰·교차 공급자 리뷰 결과는 PR 에 기록한다.
