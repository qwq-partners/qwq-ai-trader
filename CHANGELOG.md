...
- **적용 범위 = 분할 매도만, 판정은 등록 시점의 의도:** 과매도는 `원 주문 수량 < 보유 수량` 일 때만 가능하다. 전량 청산(손절·트레일링·EOD, 수량상 전량인 익절)은 원 주문이 살아 있으면 KIS 가 주문가능수량 0 으로 거절한다 → **두 경로 모두 종전 그대로 두어 손절 지연을 만들지 않는다.** 분할 여부는 pending 을 등록할 때 남긴다 — 엔진 `_pending_signal_cache[종목]["sell_partial_intent"]`(`exit_action != "sell_all"` 이고 주문 수량 < 그 시점 보유), 스케줄러 `_partial_exit_pending[종목] = 등록 시각`(`sell_partial` 발행 시). 현재 수량 비교·`pending_stage` 로 추정하면 늦은 잔고 스냅샷이나 '롤백 없는 해제' 뒤의 새 손절을 분할로 오분류한다.
...
- **엔진(분할):** 포지션 없음 → 종전 해제. 취소 실패 가능 첫 회는 추적 여부와 무관하게 조회 없이 대기(장부는 완전 체결·취소 성공에서만 빠진다 — 미추적인데 pending 이 남았다면 FillEvent 가 오는 중), 20초 뒤 취소 재시도. 그 뒤 거래소에 있으면 재주문 없이 유지(상한 없음, 재시도 간격 60초로 백오프, ERROR 로그 1회 + 5분 넘으면 헬스 모니터 교착 경보), 없거나 브로커 미추적이면 종전 시장가 폴백(원 주문 수량), 판단 불가가 **연속** 9회(270초 = 종전 최장 보유 90초×3)면 **재주문 없이** 해제.
...
- **검증:** RED(기준 `ba966d1`) 19건 실패(엔진 11·스케줄러 8)/회귀 가드 8건 통과 → 수정. 리뷰 2회 반영 후 `tests/test_stale_sell_cancel_failure.py` 63건 + PR #81 시험 30건 UTC/KST 각 **93 passed**, 격리 위반 0. 방어 코드 변이 49개 중 48개 kill(생존 1 = 고아 루프 관문; 리뷰 반영분의 가드 — 엔진 재검증 2곳·스케줄러 세대 재검증·등록 시점 의도·방향 필터·하트비트 선택·백오프·롤백 없는 해제 — 도 각각 kill 확인). 엔진·스케줄러·ExitManager·하트비트를 다루는 기존 시험 묶음 KST 586 passed / 2 known xfailed. 새 장부 속성 때문에 `object.__new__(RiskManager)` 하네스 4곳에 초기화 한 줄 추가(단언 변경 0). 전체 suite KST: 1회차 반영 전 1910 → 1회차 반영 후(`9206fe1`) 1925 passed / 2 known xfailed(격리 0); 2회차 반영 후 전체는 PR 에 기록한다. 작업 중 PR #81 head 가 `ba966d1` → `9c9d6dc` 로 이동해 그 위로 rebase 했다.
