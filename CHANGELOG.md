...
## 2026-09-21 — fix(engine,scheduler): 취소 실패한 '살아 있는 분할 SELL' 위 재발행 금지 — 분할 매도 이중 발행 (미배포, PR #81 위에 쌓음)

PR #81 교차 리뷰 P1-5(당시 '최우선 후속 과제'로 기록)의 수정이다. **배포·재시작·주문·설정 변경 0.** 현금 고갈로 봇 매매가 없어 두 경로 모두 현재는 실행되지 않으며, 매수 재개 시점부터 의미가 있다.
...
- **검증:** RED(기준 `ba966d1`) 19건 실패(엔진 11·스케줄러 8)/회귀 가드 8건 통과 → 수정. 리뷰 4회 반영 후 `tests/test_stale_sell_cancel_failure.py` 70건 + PR #81 시험 30건 UTC/KST 각 **100 passed**, 격리 위반 0. 방어 코드 변이 55개 중 54개 kill(생존 1 = 고아 루프 관문; 리뷰 반영분의 가드 — 엔진 재검증 2곳·스케줄러 세대 재검증·등록 시점 의도·방향 필터·하트비트 선택·백오프·롤백 없는 해제 — 도 각각 kill 확인). 엔진·스케줄러·ExitManager·하트비트를 다루는 기존 시험 묶음 KST 586 passed / 2 known xfailed. 새 장부 속성 때문에 `object.__new__(RiskManager)` 하네스 4곳에 초기화 한 줄 추가(단언 변경 0). 전체 suite KST: 1회차 반영 전 1910 → 1회차 반영 후(`9206fe1`) 1925 passed / 2 known xfailed(격리 0); 2회차 반영 후(`83a3db4`) 1941 passed; 3회차 반영 후(`859f7dc`) 1946 passed; 4회차 반영 후 전체는 PR 에 기록한다. 작업 중 PR #81 head 가 `ba966d1` → `9c9d6dc` 로 이동해 그 위로 rebase 했다.
