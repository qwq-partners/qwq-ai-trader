...
- **적용 범위 = 분할 매도만:** 과매도는 `원 주문 수량 < 보유 수량` 일 때만 가능하다. 전량 청산(손절·트레일링·EOD)은 원 주문이 살아 있으면 KIS 가 주문가능수량 0 으로 거절하고 롤백할 stage 도 없다 → **두 경로 모두 종전 그대로 두어 손절 지연을 만들지 않는다**(엔진 `0 < pending 수량 < 보유 수량`, 스케줄러 ExitManager `pending_stage` 가 걸린 것만).
...
- **엔진(분할):** 포지션 없음 → 종전 해제. 취소 실패 가능 첫 회는 추적 여부와 무관하게 조회 없이 대기(장부는 완전 체결·취소 성공에서만 빠진다 — 미추적인데 pending 이 남았다면 FillEvent 가 오는 중), 20초 뒤 취소 재시도. 그 뒤 거래소에 있으면 재주문 없이 유지(상한 없음, ERROR 로그 1회 + 5분 넘으면 헬스 모니터 교착 경보), 없거나 브로커 미추적이면 종전 시장가 폴백(원 주문 수량), 판단 불가가 **연속** 9회(270초 = 종전 최장 보유 90초×3)면 **재주문 없이** 해제.
...
- **재시도 구동:** stale 루프는 SIGNAL 이 와야 돌고 스케줄러 장부는 부분체결만 돼도 지워진다 → 유지분의 재시도만은 엔진 하트비트(10초, 직렬 큐)의 `RiskManager.on_heartbeat` 가 구동한다. SELL 폴백 한 종목 처리를 `_fallback_stale_sell` 로 옮겨(내용 불변) on_signal 루프와 하트비트가 같이 부른다. 유지분이 없으면 하트비트는 아무것도 하지 않는다.
- **await 뒤 재검증:** 엔진은 취소·생존 조회 `await` 를 건넌 뒤 같은 SELL pending 인지 확인하고 아니면 멈춘다 — 그 사이 스케줄러가 pending 을 풀면 수량 장부가 비어 폴백이 보유 전량으로 번진다(10주 익절 → 100주 시장가). 같은 가드가 기존 취소 `await` 의 경쟁(엔진 2회차 폴백과 스케줄러 3분 정리가 모두 ~180초)도 닫는다.
...
- **검증:** RED(기준 `ba966d1`) 19건 실패(엔진 11·스케줄러 8)/회귀 가드 8건 통과 → 수정. 리뷰 반영 후 `tests/test_stale_sell_cancel_failure.py` 47건 + PR #81 시험 30건 UTC/KST 각 **77 passed**, 격리 위반 0. 방어 코드 변이 37개 중 36개 kill(생존 1 = 고아 루프 관문; 리뷰 반영분의 가드 — 재검증 2곳·분할 한정·방향 필터·하트비트 — 도 각각 kill 확인). 엔진·스케줄러·ExitManager·하트비트를 다루는 기존 시험 묶음 KST 570 passed / 2 known xfailed. 새 장부 속성 때문에 `object.__new__(RiskManager)` 하네스 4곳에 초기화 한 줄 추가(단언 변경 0). 리뷰 반영 전 전체 suite KST 1910 passed / 2 known xfailed(격리 0); 반영 후 전체는 PR 에 기록한다. 작업 중 PR #81 head 가 `ba966d1` → `9c9d6dc` 로 이동해 그 위로 rebase 했다.
