...
- **검증:** 신규 `tests/test_exit_exempt_sell_guard.py` 16건 — 첫 10건은 수정 전 **10 failed**(전부 "면제 종목 SELL 이 `OrderEvent`/emit 이 된다" 단언)로 RED 고정 후 커밋, 수정 후 GREEN. 리뷰 반영분 3건(스로틀 분리·stale 폴백·`on_order` 재검사)은 직전 커밋의 `engine.py` 로 돌려 **3 failed** 확인 후 GREEN, 스케줄러 조기 반환 시험은 기존 가드의 특성화(전후 모두 통과). 재리뷰 반영분 2건(취소 실패 시 pending 유지·취소 중 면제 등록)도 직전 `engine.py` 에서 **2 failed** 확인 후 GREEN, 비면제 폴백 대조군 1건 추가. 브로커 대역은 실제 KIS 계약(취소 성공분만 추적에서 제거, 실패는 0건)을 따른다. 실제 `GapAndGoStrategy` → `StrategyManager` → `on_signal` 연결, 발행처 5종 × 중앙 가드, 비면제 SELL 통과·live set 해제 추종, 코어 조기경보(알림 포함)·리밸런싱 폴백·교체(매수 교착 없음). 인접 엔진·batch_analyzer·스케줄러 시험 10파일 UTC/KST 각각 206 passed·격리 0건. 전체 suite 는 PR `verify`(첫 판 1858 passed / 2 xfailed) — 로컬은 다른 세션의 pytest·스왑 포화로 미실행.
