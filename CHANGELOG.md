...
- **검증:** RED(기준 `ba966d1`) 19건 실패(엔진 11·스케줄러 8)/회귀 가드 8건 통과 → 수정. 리뷰 5회 반영·main 병합 후 `tests/test_stale_sell_cancel_failure.py` 73건 + PR #81 시험 30건 + PR #83 면제 시험 17건 UTC/KST 각 **120 passed**, 격리 위반 0. 방어 코드 변이 58개 중 57개 kill(생존 1 = 고아 루프 관문; 리뷰 반영분의 가드 — 엔진 재검증 2곳·스케줄러 세대 재검증·등록 시점 의도·방향 필터·하트비트 선택·백오프·롤백 없는 해제 — 도 각각 kill 확인). 엔진·스케줄러·ExitManager·하트비트를 다루는 기존 시험 묶음 KST 586 passed / 2 known xfailed. 새 장부 속성 때문에 `object.__new__(RiskManager)` 하네스 4곳에 초기화 한 줄 추가(단언 변경 0). 전체 suite KST: 1회차 반영 전 1910 → 1회차 반영 후(`9206fe1`) 1925 passed / 2 known xfailed(격리 0); 2회차 반영 후(`83a3db4`) 1941 passed; 3회차 반영 후(`859f7dc`) 1946 passed; 4회차 반영 후(`f816d09`) 1948 passed; main 병합 뒤 전체는 PR 에 기록한다. 작업 중 PR #81 head 가 `ba966d1` → `9c9d6dc` 로 이동해 그 위로 rebase 했다.
