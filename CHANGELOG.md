...
- 기존 실패 시 이전 SHA 복구와 exit1/복구 실패 exit2 의미를 유지한다. 테스트용 lock 재정의를 추가해 운영 lock에 접근하지 않는다.
- 새 인증·배포·복구 시험7건과 기존4건이 통과했다. 실 sudoers/systemd를 실행한 시험은 아니며 인증정보 교체·Git 이력 정리는 별도 후속이다.
- PR #90은 main `4db5141`에 병합했다. 전체 engine 개발선은 설치/전체 C/F/G/R 차단 때문에 운영 후보에서 제외한다. 주문·전략·위험 설정 유지. 실제 배포 결과는 [제한 릴리스 원장](docs/operations/release-2026-09-23.md)에 구분해 기록한다.
