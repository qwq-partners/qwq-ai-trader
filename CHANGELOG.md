...
- **적용 범위 = 분할 매도만, 판정은 등록 시점의 의도:** 과매도는 `원 주문 수량 < 보유 수량` 일 때만 가능하다. 전량 청산(손절·트레일링·EOD, 수량상 전량인 익절)은 원 주문이 살아 있으면 KIS 가 주문가능수량 0 으로 거절한다 → **두 경로 모두 종전 그대로 두어 손절 지연을 만들지 않는다.** 분할 여부는 발행 의도로 남긴다 — 엔진은 SELL 수량을 정하는 **같은 스냅샷**에서 `metadata.quantity < 보유` 이고 `exit_action` 이 없거나 `sell_partial` 일 때만 분할(`sell_all`·`replacement_exit`·수량 미지정은 전량)로 보고 `_pending_signal_cache[종목]["sell_partial_intent"]` 에 남긴다. 스케줄러는 `_partial_exit_pending[종목] = 등록 시각`(`sell_partial` 발행 시). 현재 수량 비교·`pending_stage`·호가 조회 await 뒤의 보유량으로 추정하면 늦은 잔고 스냅샷이나 '롤백 없는 해제' 뒤의 새 손절을 분할로 오분류한다.
...
- **엔진(분할):** 포지션 없음 → 종전 해제. 취소 실패 가능 첫 회는 추적 여부와 무관하게 조회 없이 대기(장부는 완전 체결·취소 성공에서만 빠진다 — 미추적인데 pending 이 남았다면 FillEvent 가 오는 중), 20초 뒤 취소 재시도. 그 뒤 거래소에 있으면 재주문 없이 유지(상한 없음, 재시도 간격 60초로 백오프, ERROR 로그 1회 + 5분 넘으면 헬스 모니터 교착 경보), 없거나 브로커 미추적이면 종전 시장가 폴백(원 주문 수량), 판단 불가가 **연속** 9회(처음부터면 270초 = 종전 최장 보유 90초×3, 생존 확인 뒤 전환이면 그로부터 220초)면 **재주문 없이** 해제. 재시도 간격은 직전 판정을 따른다(생존 확인 직후만 60초).
...
- **재시도 구동:** stale 루프는 SIGNAL 이 와야 돌고 스케줄러 장부는 부분체결만 돼도 지워진다 → 유지분의 재시도만은 엔진 하트비트(10초, 직렬 큐)의 `RiskManager.on_heartbeat` 가 구동한다(간격이 지난 것 중 가장 오래 기다린 한 종목씩). SELL 폴백 한 종목 처리를 `_fallback_stale_sell` 로 옮겨(내용 불변) on_signal 루프와 하트비트가 같이 부른다. 유지분이 없으면 하트비트는 아무것도 하지 않는다.
...
- **스로틀:** 엔진은 pending 시각을 되감지 않고 별도 장부 `_pending_cancel_keep`(횟수, 마지막 시도 시각, 생존 로그 여부)로 만든다 — 되감으면 헬스 모니터의 5분 교착 경보가 '손절이 막힌 SELL 유지'를 보지 못한다. 폴백 횟수 장부와 나눠 유지가 시장가 폴백 예산(2회)을 깎지 않는다. 스케줄러는 60초 스로틀 표식을 생존 판정이 끝날 때까지 유지해 틱·REST 폴링·독립 정리 루프의 중복 취소·조회를 막고, 유지 표식(연속 판단불가 횟수·경보 여부)을 pending 등록 시각에 묶는다.
...
- **검증:** RED(기준 `ba966d1`) 19건 실패(엔진 11·스케줄러 8)/회귀 가드 8건 통과 → 수정. 리뷰 3회 반영 후 `tests/test_stale_sell_cancel_failure.py` 68건 + PR #81 시험 30건 UTC/KST 각 **98 passed**, 격리 위반 0. 방어 코드 변이 53개 중 52개 kill(생존 1 = 고아 루프 관문; 리뷰 반영분의 가드 — 엔진 재검증 2곳·스케줄러 세대 재검증·등록 시점 의도·방향 필터·하트비트 선택·백오프·롤백 없는 해제 — 도 각각 kill 확인). 엔진·스케줄러·ExitManager·하트비트를 다루는 기존 시험 묶음 KST 586 passed / 2 known xfailed. 새 장부 속성 때문에 `object.__new__(RiskManager)` 하네스 4곳에 초기화 한 줄 추가(단언 변경 0). 전체 suite KST: 1회차 반영 전 1910 → 1회차 반영 후(`9206fe1`) 1925 passed / 2 known xfailed(격리 0); 2회차 반영 후(`83a3db4`) 1941 passed; 3회차 반영 후 전체는 PR 에 기록한다. 작업 중 PR #81 head 가 `ba966d1` → `9c9d6dc` 로 이동해 그 위로 rebase 했다.
