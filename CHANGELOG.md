...
## 2026-09-23 — fix(safety): main 보호 동작 정합화·수동 복구 진단 (운영 미설치)

- main의 TR 전환/원장 페이지/면제·stale 보호 계약을 단일 owner 개발선에 통합한다. 기존 QueryResponse·획득별 lease·attached writer 차단을 함께 보존한다.
- 새 main heartbeat가 attached 모드에서 legacy 취소를 호출하던 경계와, 일반 면제 SELL이 legacy 사유/경고 장부를 변경하던 경계를 RED 재현 후 차단한다. 면제 매도 거부는 유지한다.
- 구 특성화26개는 동시호가 취소 금지·면제 재주문 금지2건만 기대 교체하고 나머지의 한계/호환 범위를 명시한다. 기존 main 안전 테스트의 단언은 그대로다.
- 읽기 전용 [수동 복구 진단 절차](docs/operations/protection-recovery-diagnostics.md): 부분 설치·불안정 RAM·게시 불일치·근거 부재를 구분하며 자동 복구/예약 해제/재주문/원문 공유를 금지한다. exporter/CLI 구현은 미완이다.
- 검증·독립 리뷰·잔여 범위는 [N2 원장](docs/reviews/main-safety-alignment-2026-09-23.md)이 정본이다. 운영 배포/재시작/주문/설정 변경0, readiness=False 유지.
...
- [사전 검토 보고서](docs/reviews/p1-handoff-preflight-2026-09-22.md)와 P1 계획 §6에 구현 전 처분 제안을 남겼다. **Do 미착수, 제품/시험 변경 0, 신규 pytest·변이 재현 미실행.** 이전 5,116건 통과를 이번 실행 결과로 재사용하지 않는다. 사용자 정책 결정·예산 및 착수 계획 확인 대기, 운영 변경 없음.
...
- **검증:** 조사 2 → 설계 → 심사 2관점 REVISE ×2(must-fix 6+9) → 처분. 구현 3 ∥ 독립 재현 3(S-A CHANGES_REQUIRED P1 1 · S-B/S-C CHANGES_REQUIRED P2 3 · S-D APPROVE P2 2 — 전부 제품 0줄로 처분, 생존 변이 3종 이제 kill) → **Codex 17차 APPROVE**(P2 docstring 1). 전체 suite 단독 직렬 **UTC 5116 / KST 5116 passed**(각 기존 xfail 2·경고 4·격리 0 — UTC 는 시계 창을 피해 13:01 UTC 시작). 신규 시험 3파일 + 3파일 추가.
...
- **무엇이 되었나:** runtime 이 붙은 엔진에서 실제 큐의 SIGNAL 이 기존 후보 판단(`on_signal`)을 그대로 거친 뒤 **owner 의 prepare → final 재검사 → dispatch 한 길로만** 송신된다(정상 MARKET BUY·LIMIT/MARKET SELL). 송신하지 못한 요청은 예약을 남기지 않고 사유 코드가 남는다. 제품 7파일 +330/−6(그중 기존 줄 변경은 `commands.py` 와 `engine.py` 의 조건 한 줄), 새 시험 8파일 약 2,700줄.
- **wave 1 — `lifecycle.py` `abandon_candidate`(+33)·`risk_policy.py` `hybrid_enabled`(+3):** 송신을 시작한 적 없는 prepared SUBMIT 을 한 transaction 으로 끝낸다(가드: `prepared`·`claim_id None`·`command_status None`·`order_ref None`·체결 0·증거 충돌 없음 → 기존 `_release_resources` 재사용, 터미널은 기존 `FINAL_REJECTED`). `trading_ready` 가 항상 False 라 gateway 를 붙이는 순간 모든 dispatch 가 claim 이전에 실패하므로 **이것이 없으면 예약이 100% 누수**된다 — 게다가 미claim prepared 가 하나라도 남으면 다음 날 일자 전환이 `unresolved_submit` 로 시작되지 않는다. `EffectiveRiskPolicy.hybrid_enabled` 는 기본값 없는 필수 bool(기존 시험은 helper 2곳의 인자 1줄씩만 수정).
- **wave 2 — `commands.py`·`day_recovery.py`:** `_dispatch` 의 claim 이전 실패를 뭉개던 `claim_not_available` 을 풀어 **사유 보존** + SUBMIT 은 같은 호출 안에서 abandon(자식 명령은 부모 `order_ref` 때문에 구조적으로 불가 → S4), 종료 중은 `command_admission_closed`, 저장 장애는 `dispatch_failed`(예약 무변). `decision_hybrid_mismatch`(설정 on·신고 off) 대조, `stale_decision_config_version` 의 첫 실효 RED. 일자 전환은 전일 `entry_decision_facts` **만** 비운다(출처 행은 이름으로 키잉돼 유계). **계약 귀결:** claim 이전의 일시 차단도 그 attempt 를 끝낸다 — 재송신은 같은 intent 의 새 prepare(기존 시험 2파일의 기대값 갱신은 이 누수를 현행으로 고정했던 단언).
- **wave 3 — `safety/gateway.py`(신규)·`runtime.install_gateway`:** `SignalGateway.submit(event, order, evidence)` 가 (BUY) 진입 시세 → `publish_qualification` → 감사 로그 → `prepare` → `dispatch`. engine import 0·owner 판정 복제 0·같은 attempt 재 dispatch 0. gateway 는 정책 맥락을 **쓰지 않는다**(config 축을 덮어쓰면 owner 의 대조가 자기 자신과 비교하게 된다). 읽기 helper `reserved_cash()`·`pending_strategy_notional()` 은 fail-closed. **독립 재현이 찾은 P1:** prepare 와 dispatch 사이에 세션 경계가 닫히면 예외가 abandon 을 거치지 않고 탈출해 예약 101,500원이 남았다 → `_dispatch` 의 try 밖 중복 세션 검사 제거(신원·권한 위반은 계속 raise). **Codex 2차의 P1 2건**(claim 이전 취소·prepare 중 shutdown)은 같은 부류이고 crash 까지 덮는 필요충분한 해법은 다음 기동의 sweep 이라 **`recover_unsent()`** 를 부품으로 추가(엔진 실행 중 거부, 제품 호출은 10A3 factory 의 몫).
- **전체 suite(HEAD `10d2ca7`, 단독 직렬):** UTC **4758 passed / 2 xfailed / 경고 4**(324.28초), KST **4758 / 2 / 4**(319.85초), 격리 0. S2 마감 4632 대비 +126. 기존 xfail 2·경고 4 불변.
- **Codex 3차(대상 `3ff04c8`): CHANGES_REQUIRED — P1 1·P2 1.** legacy 불변·증거 순서·H2 의 fail-closed·2차 처분의 타당성은 "결함 없음". P1: H6 은 attach **시점**만 봐서, 그 뒤 legacy 장부가 남은 RiskManager 가 연결되면 진입부 stale 루프가 owner 를 거치지 않고 직접 취소·재주문한다 → **H7**(on_signal 의 stale 루프 직전에서 attach + 장부 잔류면 그 SIGNAL 거부). P2: `_submit_signal` 의 `finally` 가 `symbol` 없는 이벤트에서 다시 던져 루프를 끝낸다 → `getattr`. 둘 다 행동 RED → 수정(`10d2ca7`). 이 처분의 Codex 재확인은 S5 범위.
- **wave 4·5 — `src/core/engine.py`(운영 live 파일, 최종 +98/−1, 추가 실행 줄은 전부 attach 가드 안):** H1 `_process_event` 가 SIGNAL 만 `_submit_signal` 로 넘긴다(`on_signal` await 직후 **다음 await 이전에** 증거를 지역값으로, 예외는 핸들러 루프와 같은 의미로 흡수 — 안 하면 owner 예외 한 건이 엔진 루프를 내린다, `finally` 에서 `_pending_sector_map.pop`, OrderEvent 를 큐에 되싣지 않음) · H4 attach 에서 legacy 장부 7종 미기록·`_last_signal_time` 은 유지 · H5 eviction 미호출(안 막으면 그 SELL 이 실제 POST 경로를 얻는다) · H2 `_reserved_cash`(property 유지)·`_pending_strategy_notional` 이 owner 예약을 읽는다(B 가 47주 예약 뒤 30주로 줄어 POST 1 — 수정 전에는 `decision_quantity_unjustified`) · H3 sector 조회 **예외**는 attach 에서 거부(`G3_sector`) · H6 legacy 장부가 남은 채로는 bind 거부.
- **검증:** 단계마다 단일 writer(요청 claude-opus-5/high) → 독립 재현(요청 opus/xhigh, 다른 실행) → coordinator 의 diff 직접 확인·변이 재적용. **독립 재현은 7단계 중 6단계에서 CHANGES_REQUIRED** 였다 — 실제 제품 결함 1건(wave 3 의 세션 경계 누수)과 시험 공백 다수(독립 재현에서 살아남은 변이 15종을 전부 시험 보강 후 재적용해 kill 확인). Codex(요청 gpt-6-astra/xhigh, 포그라운드 read-only) 1차 APPROVE(S2 마감 수정의 P2 도 닫힘)·2차 CHANGES_REQUIRED(P1 2·P2 1 → 처분)·3차 CHANGES_REQUIRED(P1 1·P2 1 → 처분, 위). 관측 모델은 전부 metadata 미노출(미검증).
- **coordinator 의 오판 2건(원장에 정정 기록):** ① wave 1 뒤 전체 suite 의 실패 2건을 전부 다른 세션의 부하 탓으로 봤으나 1건은 새 시험이 SQLite store 를 닫지 않아 다음 파일의 fd 계수 시험을 깬 **순서 의존**이었다 ② wave 5 는 지정 파일 GREEN·독립 재현 통과 뒤에도 **전체 suite 만이** legacy 회귀(`object.__new__(RiskManager)` 인스턴스에서 `self.engine` 을 새로 읽음)를 잡았다.
- **하지 않은 것:** 직접 SELL 폴백·취소0건 해제·eviction 의 owner 경로 이관과 미claim 자식 명령의 종료(S4) · 독립 실큐 인수·최종 broad 리뷰(S5 — ORDER 이벤트를 큐에 싣지 않는다는 해석을 상위 계획과의 차이로 확인받는다) · factory·`config_version` 5축 조립·`recover_unsent()` 호출·`trading_ready` 실제 대사·attach 인지형 대시보드(10A3) · entry quote 의 출처가 SIGNAL 자체라는 한계 · `_risk_validator`·팩터 버킷 게이트는 attach 경로 시험에서 아직 스텁.
- main/운영·배포·재시작·주문·설정·Toss grant 무변경.
...
- **S2-4 `src/execution/safety/qualification.py`(신규, I/O·시계 읽기 0 — now 는 전부 인자):** `build_decision_facts` 가 CV 증거 12키 + 사이징 입력 14키 + CV 가 받은 regime → `EntryDecisionFacts` 와 게시할 출처(`PendingSource`). 판정을 바꾼 출처만 인용(규칙11·12·4·5·trade_wiki 미인용, regime 상시 인용), `RULE_IDS` 15쌍이 CV 의 실제 `penalties.append` 15곳을 빠짐없이 덮음(Codex 전수 대조), LLM 8어휘, overlay 예외 표식, 0 배율·hybrid 는 **보정 없이 facts 미생성**, 시계 교차검증(`decision_clock_disagreement`), `entry_expires_at` = CV 다음 경계·SEPA 14:30·패널 +6h·당일 말의 최소, `config_version` 5축 digest. **SEPA 상한 두 구멍을 닫음:** CV 는 14:30 전인데 `decided_at` 이 14:30 뒤면(LLM await 가 경계를 넘으면) 거부, 양쪽 시계가 모두 14:30 을 넘으면 상한 항이 소멸해 당일 말까지 유효해지던 것을 `sepa_entry_deadline_passed` 로 거부.
- **S2-5 `src/core/engine.py`(on_signal 에 +48 안팎, runtime 이 붙어 있을 때만 — legacy 0줄 실행) + `src/execution/safety/qualification_publisher.py`(신규):** on_signal 은 **증거 캡처만** 한다 — validate 직전 관측 시각, 자기 `llm_second_check` await 직후 token **동등** 대조 뒤 `last_decision`(deepcopy)·`last_llm_reason` 을 추가 await 없이 함께 복사, 사이징 성공 직후 입력 복사, 섹터 조회 뒤 `QualificationEvidence` 조립. `publish_qualification(commands, evidence, *, intent_id, config_version, decided_at)` 는 출처(idempotent 재사용) → facts 순서로 게시한다. **둘을 잇는 호출·intent_id 발급·config_version 조립은 S3 gateway** — 제품 호출자는 0건이다.
- **통합 수정(Codex wave B + 독립 재검증):** 출처 이름을 소비 범위로 분리(`panel_outlook:<symbol>`·`trade_memory:<strategy>:<sector>`) — 같은 패널로 두 종목을 잇달아 게시하면 첫 요청이 `stale_qualification_source` 가 되던 설계 결함을 **실제 owner 에서 행동 RED 로 재현**한 뒤 수정. 출처 `as_of` 를 CV 가 실제로 읽은 시각(`observed_at`)으로.
- Plan→Do→See: 단계별 단일 writer(요청 opus/high) → 독립 재검증·재현(요청 opus/xhigh, 계획서 변이 + 자체 변이) → Codex 교차 리뷰(요청 astra/xhigh, read-only). 다섯 단계 모두 **제품 가드는 맞고 시험 공백이 변이 생존으로 드러나는** 양상이었고, 매번 시험을 보강해 같은 변이로 kill 을 확인했다. Codex wave B P1(prepare 뒤 **claim 이전** 최종 검사가 실패하면 결과가 기록되지 않아 attempt 가 `prepared` 로 남고 예약이 유지된다 — lifecycle 에 prepared 종료 경로가 없다)은 Task 9 부터의 공백으로 확인해 **S3 인수 조건**으로 이관.
- 검증: HEAD `83baa2a` **전체 UTC 4628 passed/기존 xfail2/경고4(301.43초)·KST 4628/2/4(282.50초)**, 각 exit0·격리0. S1 뒤 4442 대비 +186 = builder 69 + CV 특성화 55 + 사이징 반출 27 + publishers 25 + regime 재검사 10.
- **정직한 성과 문장:** 실효 stale 축은 regime 1개, panel·trade_memory 는 replay 구속 전용, config 축은 S3 에서 실효. S3 로 넘긴 조건 12항은 `docs/reviews/b2b3-stage-ledger-2026-09-20.md` 의 "S3 인수 조건(누적)". main/운영·배포·재시작·주문·설정·Toss grant 변경0, `trading_ready=False`·MODIFY 미지원 유지.
...
- **왜:** 공식 저장소(`koreainvestment/open-trading-api`)의 모든 연속조회 예제는 다음 페이지 요청에 `tr_cont="N"` 을 보낸다. 제품은 응답 헤더로 종료만 판정하고 요청에는 아무것도 싣지 않아, **2페이지 요청이 1페이지를 다시 받을 수 있었다** — 그러면 "ctx 키가 이전과 같으면 중단" 가드가 조용히 잘린 목록으로 끝낸다(잔고·체결 누락).
- **무엇 ②(형제 루프의 종료 판정 통일):** `get_positions_for_account` 의 페이지 루프에만 없던 응답 헤더 종료 판정(`_tr_cont` 가 `D`/`E` 면 중단)을 다른 두 루프와 **같은 한 줄**로 넣었다. 이 루프는 빈 ctx 키만 보고 끝나는데 KIS 는 마지막 페이지에도 ctx 키를 채워 보내므로(2026-09-15 EGW00215 반복의 원인과 같은 것), 마지막 페이지 뒤에 원장 호출이 한 번 더 나가고 있었다.
- **미체결 조회(`get_exchange_open_orders`)의 잘림 처리는 PR #81 이 같은 함수에서 다룬다 — 이 PR 은 건드리지 않는다.**
- **무엇 ③(취소 POST):** **코드 0줄** — 취소의 `retry=True` 를 유지하고 근거를 "멱등이라서"에서 효과 한정 논증으로 고쳐 적었다(재전송 대상 5xx 다수는 접수 전 거절 · 전량 취소는 `ORGN_ODNO` 하나를 겨냥해 두 번 닿아도 새 노출 불가 · 응답 유실 시 `False` 는 `retry=False` 에서도 같다). 재시도를 빼면 보호 취소(90초 SELL 폴백·10분 BUY 정리) 성공률만 떨어진다. 특성화 시험으로 고정.
- **운영에서 실제로 바뀌는 것:** **`get_positions`·`_query_daily_fills` 는 1페이지로 끝나는 계좌(현행 운영 계좌)에서 요청이 한 바이트도 바뀌지 않는다** — 헤더·params·호출 횟수 전부 동일(헤더 dict·params dict 전량 등식으로 고정). 달라지는 것은 둘이다: ⓐ 보유 종목/당일 체결이 2페이지를 넘을 때 2페이지째 요청에 `tr_cont: N` 헤더 한 줄이 붙는다, ⓑ **`get_positions_for_account`(외부 계좌 표시용)는 마지막 페이지 뒤의 불필요한 원장 호출 1회가 없어진다** — 헤더가 `D`/`E` 인데 ctx 키가 채워져 오던 응답에서 호출이 2회 → 1회로 줄어든다(원장 TR 초당 1건 제한을 덜 쓴다).
- **검증:** 신규 `tests/test_kis_pagination_protocol.py` 10건(가짜 HTTP·실 KIS 0, 진짜 `_api_get`·진짜 `_get_headers` 를 통과시켜 요청을 스냅샷) — 세 루프의 1페이지 요청이 **헤더 dict·params dict 전량 등식**으로 기준선과 같다(기대 dict 는 제품 코드에서 옮긴 리터럴), F→M→D 3페이지의 `(미송신)→N→N` 과 행 합산, 같은 페이지 재시도(HTTP 500)에서도 헤더 유지, `get_positions_for_account` 의 "헤더 D + 채워진 ctx" 1페이지 호출 1회와 F→D 2페이지 합산, 취소 재시도 특성화(두 전송의 본문·`hashkey` 헤더 동일). ②는 **RED 먼저** 고정했다(같은 표본에서 호출 2회 → 1회, 2페이지 표본 3회 → 2회). 기존 가짜 `_api_get` 4곳(`test_kis_ledger_pagination_2026_09_15.py` 2 · `test_kis_tr_switch.py` 1 · `test_review_fixes_2026_09.py` 1)은 **기대값 그대로** 시그니처만 `tr_cont=""` 를 받게 넓혔다. 지정 5파일 UTC/KST 각 **117 passed**·격리 0건. 변이 3종(F3 의 종료 한 줄 삭제 · 1페이지 params 에 키 추가 · `_api_get` 이 1페이지에도 `tr_cont` 를 실음) 전부 kill.
- **하지 않은 것:** 미체결 조회(`get_exchange_open_orders`)는 손대지 않았다 — 잘림 처리는 PR #81 이 같은 함수에서 이미 한다(`symbol` 을 물은 호출이 첫 페이지에 없고 헤더가 F/M 이면 `None`). 페이지 루프 자체, `custtype` 헤더도 그대로. 배포·재시작·`.env` 변경 0.

## 2026-09-22 — ops: main `2358936` 운영 반영 (PR #84 병합·배포·재시작)

- **지시·조건:** 사용자 지시 "#84 교차 리뷰 통과하면 머지하고 운영 반영". PR #84(다른 세션 작성 — 취소 0건인 '살아 있는 SELL' 위 재발행 금지, PR #81 교차 리뷰의 잔존 결함 P1-5)의 교차 공급자 리뷰(Codex, 요청 gpt-6-astra/xhigh, 세션 기록상 같은 값·리뷰어 자신은 실효 모델·effort 미확인으로 보고, 전부 정적)는 1~6회차 병합 불가 → **7회차 병합 가능(순증 P0·P1·P2 0, 최종 head `9fe95ed`·기준 main `d3954a7`)**. 6회차의 P1 은 #83(면제 가드)과의 병합 상호작용이었고 처분 뒤 7회차가 해소를 확인했다. 리뷰어가 수용한 순증 위험은 문서화된 '지연'뿐(분할 매도 첫 회 대기 20/60초, 판단 불가 180초 예산, 거래소 생존 확인 시 상한 없는 유지 + 사람 경보). 회차별 기록은 PR #84 코멘트.
- **통합 전 확인(이 세션):** 리뷰를 중복으로 돌리지 않고 작성 세션의 판정을 기다렸다. 최종 head 의 필수 verify SUCCESS·`CLEAN`·Draft 해제 확인. 제품 diff(`src/core/engine.py` +400·`src/schedulers/kr_scheduler.py` +112)의 구조를 직접 읽음 — 새 `EventType.HEARTBEAT` 핸들러는 엔진이 이미 10초마다 발행하던 이벤트에 붙고, 유지분(`_pending_cancel_keep`)이 없으면 즉시 반환·처리 5초 상한. `7ce54b4` 의 첫 verify 실패는 #84 와 무관한 `tests/test_toss_observer_service.py::test_stop_during_startup_does_not_wait_for_job_timeout`(실시간 타이밍 시험, #79 가 잔여로 기록한 파일) 1건 — 실패 job 재실행으로 SUCCESS.
- **배포 전 점검(00:49 KST, 장 마감 후):** pending `[]`, 브로커 연결, 운영 트리 청결(`d3954a7`), 설정3파일·킬스위치4경로 지문이 09-21 20:37 기록과 동일, 경합 pytest 없음·부하 1.0.
- **배포:** `scripts/deploy/local_deploy.sh 2358936` 00:49:40 시작 → 운영 서버 verify **1972 passed / 2 known xfailed / 기존 warning 1**(74.7초) → 재시작 → 헬스 통과, 00:51:12 `[완료]`(자동 롤백 없음). 이전 PID1409857(09-21 23:06:23 기동) → **PID1453603, 00:51:02 기동.** 운영 checkout 은 `main` 으로 복귀(트리 동일).
- **사후 점검:** `KIS API 연결 완료`·`KIS TR 세트: legacy`·`통합 트레이딩 엔진 시작`. 새 PID 의 ERROR/CRITICAL/Traceback 0(집계된 1건은 종료되던 이전 PID 의 `Unclosed client session`). 160초 시점 `ops_check`: 원장 EGW00215 0·토큰 오류 0, 기동 직후 시세 TR(FHKST01010100) EGW00201 2건은 재시도 성공·반복 없음(09-21 20:44 배포 때와 같은 시작 버스트), 루프 정체·실패 누적 없음, pending `[]`. 지문 7경로 전후 동일, Toss 관측 서비스 PID3335469 무변경. `.env`·설정·킬스위치·주문 변경 0.
- **운영 메모:** 작성 세션의 6회차 Codex 러너가 시한 없는 샌드박스 프로브의 종료 단계에서 49분 멈췄다(PR #81 4회차와 같은 증상 — Codex 장애 아님, 대조 실험 9초 정상). 이 세션이 원인·해법을 알렸고 작성 세션이 자기 프로브 프로세스만 종료해 진행했다. 이후 러너에는 프로브 180초 시한이 들어갔다.
- **남은 것:** 이번 경로는 분할 매도 SELL pending 이 있어야 돌므로 현금 고갈 중에는 관측되지 않는다 — 확인 항목은 `docs/operations/monitoring-checkpoints.md` 맨 위. 주문 ID 단위 대사 부재·첫 10분 BUY pending 의 청산 차단·거래소 조회 첫 페이지 한계는 `docs/risk/risk-and-exit.md` 의 잔존 위험 그대로.

## 2026-09-21 — fix(engine,scheduler): 취소 실패한 '살아 있는 분할 SELL' 위 재발행 금지 — 분할 매도 이중 발행 (미배포, PR #84)

PR #81 교차 리뷰 P1-5(당시 '최우선 후속 과제'로 기록)의 수정이다. PR #81 위에 쌓아 작업했고, #81·#83 이 main 에 병합된 뒤 최신 main 을 병합했다 — #83 이 SELL 폴백 루프 머리에 넣은 자동매도 금지 종목 분기와 제출 직전 면제 재검사는 이 PR 이 옮긴 `_fallback_stale_sell` 안으로 그대로 이식했고(main 의 루프 본문과 정규화 비교로 누락 0 확인), 스케줄러에서는 #83 의 면제 SELL 생존 확인(`await`) **뒤에** 이 PR 의 pending 세대 재검증이 온다. **배포·재시작·주문·설정 변경 0.** 현금 고갈로 봇 매매가 없어 두 경로 모두 현재는 실행되지 않으며, 매수 재개 시점부터 의미가 있다.

- **결함:** `KISKRBroker.cancel_all_for_symbol` 은 미연결·hashkey·rt_cd≠0·예외를 전부 삼키고 0 을 돌려준다. 100주 보유·1차 익절 10주 지정가 SELL 활성·취소 실패에서 ① 엔진 90초 폴백이 반환값을 보지 않고 시장가 10주를 추가 제출(합계 20주는 보유 수량 안이라 KIS 가 거절하지 않는다) ② 스케줄러 `_cleanup_stale_pending`(별도 장부, 3분)이 양쪽 pending 을 풀고 `rollback_stage` → ExitManager 가 같은 10주를 재발행. 체결됐으나 미반영인 경우엔 롤백이 `ExitManager.on_fill` 의 stage 승격까지 지운다. 두 경로 모두 RED 시험으로 재현했다.
- **적용 범위 = 분할 매도만, 판정은 등록 시점의 의도:** 과매도는 `원 주문 수량 < 보유 수량` 일 때만 가능하다. 발행 의도가 전량인 청산(손절·트레일링·EOD)은 원 주문이 살아 있으면 KIS 가 주문가능수량 0 으로 거절한다 → **두 경로 모두 종전 그대로 두어 손절 지연을 만들지 않는다**(분할로 발행됐다가 채택 시점에 수량상 전량이 된 경우는 엔진만 전량으로 보고 스케줄러는 분할 표식대로 관문을 탄다 — 결과는 과매도가 아니라 스케줄러 쪽 해제 지연, 한계에 기록). 분할 여부는 발행 의도로 남긴다 — 엔진은 SELL 수량을 정하는 **같은 스냅샷**에서 `metadata.quantity < 보유` 이고 `exit_action` 이 없거나 `sell_partial` 일 때만 분할(`sell_all`·`replacement_exit`·수량 미지정은 전량)로 보고 `_pending_signal_cache[종목]["sell_partial_intent"]` 에 남긴다. 스케줄러는 `_partial_exit_pending[종목] = 등록 시각`(`sell_partial` 발행 시). 현재 수량 비교·`pending_stage`·호가 조회 await 뒤의 보유량으로 추정하면 늦은 잔고 스냅샷이나 '롤백 없는 해제' 뒤의 새 손절을 분할로 오분류한다.
- **설계:** 주문 생존의 근거는 거래소 실 미체결(브로커 인메모리 장부는 1차 힌트)이고, 엔진은 '취소→시장가 재발행', 스케줄러는 '청산 의도(stage) 롤백'의 유일한 실행자다. 둘 다 같은 판정 함수 `RiskManager.stale_order_still_live(symbol, side, confirm)`(PR #81 의 `_stale_buy_still_live` 를 방향 인자를 받도록 일반화)로 **생존이 부정되기 전에는 자기 행동을 하지 않는다.** 장부 확인은 방향까지 본다 — 취소 건수는 종목 단위라 같은 종목의 다른 주문(해제된 BUY 잔량 등)이 취소된 1건을 SELL 취소로 읽지 않도록 "취소 뒤에도 활성 SELL 이 장부에 남았는가"로 판정한다. 남지 않았으면(취소 성공) 두 경로 모두 종전 그대로(지연 0).
- **엔진(분할):** 포지션 없음 → 종전 해제. 취소 실패 가능 첫 회는 추적 여부와 무관하게 조회 없이 대기(장부는 완전 체결·취소 성공에서만 빠진다 — 미추적인데 pending 이 남았다면 FillEvent 가 오는 중), 20초 뒤 취소 재시도. 그 뒤 거래소에 있으면 재주문 없이 유지(상한 없음, 재시도 간격 60초로 백오프, ERROR 로그 1회 + 5분 넘으면 헬스 모니터 교착 경보), 없거나 브로커 미추적이면 종전 시장가 폴백(원 주문 수량), 마지막 비-판단불가 판정 뒤 **180초**가 지나도록 판단 불가뿐이면(처음부터면 등록 후 270초 = 종전 최장 보유 90초×3) **재주문 없이** 해제 — 횟수가 아니라 시간 예산이다. 재시도 간격은 직전 판정을 따른다(생존 확인 직후만 60초).
- **스케줄러(분할, stale·고아 루프 공통):** 브로커 미추적 → 종전 즉시 해제. 추적 중 첫 회 → 60초 스로틀만큼 대기. 그 뒤 거래소에 있으면 해제·롤백 없이 유지(상한 없음) + 텔레그램 CRITICAL(pending 당 1회), 없으면 종전 해제+롤백, 판단 불가가 **연속** 2회(등록 후 약 5분 — 엔진 270초와 같은 예산)면 이 장부와 엔진 pending 만 풀고 **stage 는 롤백하지 않는다**: 손절·트레일링 판정은 다시 열리고 같은 분할 익절의 재발행은 ExitManager pending 검증자가 계속 막는다.
- **재시도 구동:** stale 루프는 SIGNAL 이 와야 돌고 스케줄러 장부는 부분체결만 돼도 지워진다 → 유지분의 재시도만은 엔진 하트비트(10초, 직렬 큐)의 `RiskManager.on_heartbeat` 가 구동한다(간격이 지난 유지분을 오래 기다린 순으로 모두 — 하트비트당 한 종목이면 간격이 10초×종목 수로 늘어난다). SELL 폴백 한 종목 처리를 `_fallback_stale_sell` 로 옮겨(내용 불변) on_signal 루프와 하트비트가 같이 부른다. 유지분이 없으면 하트비트는 아무것도 하지 않는다.
- **await 뒤 세대 재검증(스케줄러):** 정리 루프는 취소 전에 pending 의 등록 시각을 잡고 해제·롤백 직전과 생존 조회 직후에 같은 세대인지 확인한다 — 그 사이 옛 익절이 체결로 끝나고 같은 종목의 다음 익절이 등록되면 옛 주문의 '소멸' 판정이 새 pending·stage 를 풀어 중복 발행된다.
- **await 뒤 재검증(엔진):** 엔진은 취소·생존 조회 `await` 를 건넌 뒤 같은 SELL pending 인지 확인하고 아니면 멈춘다 — 그 사이 스케줄러가 pending 을 풀면 수량 장부가 비어 폴백이 보유 전량으로 번진다(10주 익절 → 100주 시장가). 같은 가드가 기존 취소 `await` 의 경쟁(엔진 2회차 폴백과 스케줄러 3분 정리가 모두 ~180초)도 닫는다.
- **스로틀:** 엔진은 pending 시각을 되감지 않고 별도 장부 `_pending_cancel_keep`(`_SellKeep`: 횟수, 마지막 시도 시각, 생존 로그 1회 여부, 직전 판정이 생존인가, 마지막 비-판단불가 시각)로 만든다 — 되감으면 헬스 모니터의 5분 교착 경보가 '손절이 막힌 SELL 유지'를 보지 못한다. 폴백 횟수 장부와 나눠 유지가 시장가 폴백 예산(2회)을 깎지 않는다. 스케줄러는 60초 스로틀 표식을 생존 판정이 끝날 때까지 유지해 틱·REST 폴링·독립 정리 루프의 중복 취소·조회를 막고, 유지 표식(연속 판단불가 횟수·경보 여부)을 pending 등록 시각에 묶는다.
- **브로커 계약 변경안은 채택하지 않았다:** 호출처 6곳이 전부 try/except 라 '예외 = 유지' 분기가 한꺼번에 살아나고 운영 스크립트 동작까지 바뀐다 · 실패를 정확히 알아도 '취소 거절'은 체결/생존을 말해 주지 않아 거래소 조회는 어차피 필요하다 · 필요한 힌트는 현 계약에서 건수보다 정확히 유도된다(취소 성공은 장부에서 빼므로 취소 뒤 남은 그 방향의 활성 주문 = 취소 실패 주문) · 엔진 실행 안전성 이행 브랜치가 그 계약을 새로 설계 중이다(중복·충돌 회피).
- **중복 구현 확인:** `feature/engine-safety-design-20260917` 은 이 결함을 S1 으로 인식하나 해법은 owner 경로 재설계이고 legacy 루프는 '현행 고정' 특성화 시험만 있으며 스케줄러 정리는 10C 로 미뤄져 있다(미병합·운영 승격 차단). 운영은 legacy 경로라 이 수정이 필요하다. 그 브랜치가 main 을 병합할 때 `test_engine_legacy_stale_eviction_characterization.py` 의 '0건 → 시장가 재주문' 기대(분할 수량 표본)와 `object.__new__(RiskManager)` 하네스(`_pending_cancel_keep = {}`)를 갱신해야 한다.
- **리뷰:** 독립 리뷰(요청 claude-opus-5/high, 같은 공급자 — 실제 모델 미확인) **조건부 승인**: P1 1(판단 불가에서 스케줄러 장부가 엔진보다 ~12분 오래 남아 손절 신호 자체가 생성되지 않음)·P2 7. 교차 공급자 리뷰(Codex, 세션 기록상 gpt-6-astra/xhigh, 정적 — 샌드박스에서 pytest 불가) **병합 불가**: P1 2(조회 await 중 pending 해제 시 10주→100주 전량 확대 / 첫 회 대기가 후속 SIGNAL 을 요구하는데 부분체결 뒤엔 받쳐 줄 장부가 없음)·P2 2(판정 await 중 스로틀 공백 / 경보 표식이 종목 단위로 남음). **전부 실코드로 재현·확인해 반영**했다(위 '적용 범위'·'재시도 구동'·'await 뒤 재검증'·'스로틀'·스케줄러 상한). 문서 오류(체결 확인 주기 5초→2초/15초, '20/60초 안에 복구' 표현, 취소 일부 성공 설명, CHANGELOG 의 '전량 매도' 주장 범위)도 정정. 미반영으로 남긴 것은 한계에 기록. **2회차(대상 `9206fe1`)**: 독립 리뷰 **승인**(P0 0·P1 0·P2 7 — 장마감 리셋에 새 장부 누락, 생존 확정 재시도의 백오프 부재·하트비트가 유지분 전부를 직렬 처리, '롤백 없는 해제 뒤 손절 재개' 시험 공백, 문서 한 줄 등 → 반영). 교차 공급자 리뷰 **병합 불가**(1회차 지적은 전부 해소 확인, 새 P1 2: 스케줄러 생존 조회 await 중 pending 세대가 바뀌면 새 pending 을 해제 / '분할만' 판정이 현재 수량·`pending_stage` 추정이라 실제 전량 청산에도 대기 — P2 1: 문서의 시간식·검사 순서·'지연 0' 표현) → 세대 재검증·등록 시점 의도 기록으로 반영. **3회차(대상 `83a3db4`)**: 교차 공급자 리뷰 **병합 불가**(2회차 세대 교체 지적은 해소 확인. 새 P1 2: 분할 의도를 수량을 정한 뒤 호가 조회 await 를 건넌 등록 시점의 보유량으로 판정해 전량 청산이 분할로 굳을 수 있고 `replacement_exit` 이 전량으로 인정되지 않음 / 재시도 간격이 '직전 판정'이 아니라 '과거 생존 로그 여부'를 읽어 생존→판단 불가 전환 뒤 해제가 9분 — P2 1: 한계 절의 옛 설명) → 의도를 수량 결정 스냅샷에서 정하고, 유지 장부를 `_SellKeep`(직전 판정과 로그 1회 여부 분리)로 바꿔 반영. **4회차(대상 `859f7dc`)**: 교차 공급자 리뷰 **병합 불가**(3회차 P1 2건 해소 확인, 신규 과매도·중복 발행 경로 없음. 새 P1 1: 2회차에 넣은 '하트비트당 한 종목' + 횟수 상한 때문에 다종목 유지 시 해제가 종목 수에 비례해 밀림 — 8종목이면 등록 후 820초 / P2 1: 엔진·스케줄러의 두 분할 표식이 어긋날 수 있어 '수량상 전량도 지연 0'은 성립하지 않음) → 상한을 시간 예산(180초)으로 바꾸고 하트비트가 때가 된 유지분을 모두 처리, 표식 불일치는 정책·한계로 명시. **5회차(대상 `f816d09`)**: 교차 공급자 리뷰 **병합 불가**(4회차 지적 해소 확인. 새 P1 2: 유지 → 소멸 판정 → 시장가 제출 실패(접수 불명 포함) 뒤 유지 장부가 남아 하트비트가 SIGNAL 없이 재제출 / 하트비트가 때가 된 종목을 모두 직렬 처리하면 조회 타임아웃 시 엔진 큐의 FILL·ORDER 가 수 분 밀림) → 유지 장부를 폴백으로 넘어가는 순간 닫고, 하트비트 처리에 5초 상한(첫 종목은 항상, 나머지는 다음 하트비트)·종목별 실제 시각 기록으로 반영. **6회차(대상 `7ce54b4` = main 병합본, 기준 origin/main `d3954a7`)**: 교차 공급자 리뷰 **병합 불가**(5회차 P1 2건 해소 확인, main 병합의 이식은 정규화·AST 비교로 정확 확인. 새 P1 1 — 병합 상호작용: 분할 표식 + 자동매도 금지 + 브로커에 활성 SELL + 연속 판단 불가에서 스케줄러 관문이 직접 풀고 True 를 돌려줘 #83 의 면제 확인을 건너뜀 → 면제 SELL 의 취소 추적 소실, 엔진의 판단 불가 예산 해제도 await 중 등록된 면제를 재확인하지 않음 / P2 2: 면제 분기가 유지 장부의 `tried_at` 을 갱신하지 않아 하트비트 정렬에서 뒤 종목이 굶을 수 있음, 문서 3곳) → 면제 종목은 #83 의 면제 처리가 맡도록 반영(면제 분기 진입 시 유지 장부 종료, 생존 조회 await 뒤 면제 재확인: 엔진은 아무것도 풀지 않고 넘김·스케줄러는 False 로 돌려 호출측 면제 확인이 결정), 문서 정정. 7회차 결과는 PR #84 에 기록한다.
- **한계(잔존):** 하트비트는 유지분의 재시도만 구동(첫 90초 폴백·BUY 정리는 SIGNAL 의존 그대로) · 죽은 주문이 장부에 남은 종목은 분할 매도의 시장가 전환이 첫 회 대기만큼 늦음 · 판정이 종목·방향 단위(사람의 MTS 매도 미체결, 50건 초과 시 다음 페이지) · '거래소에 없음'은 체결 확인이 밀리면 '체결 미반영'과 구분 불가 · 판단 불가 해제 뒤 ExitManager 밖 발행처의 SELL 은 막지 못함 · 폴백 2회 상한 해제·`on_fill` 의 방향 무구분 수량 차감은 기존 그대로 · 고아 루프 관문은 현재 도달하지 않는 심층 방어. 상세 `docs/risk/risk-and-exit.md`.
- **검증:** RED(기준 `ba966d1`) 19건 실패(엔진 11·스케줄러 8)/회귀 가드 8건 통과 → 수정. 리뷰 6회 반영·main 병합 후 `tests/test_stale_sell_cancel_failure.py` 77건 + PR #81 시험 30건 + PR #83 면제 시험 17건 UTC/KST 각 **124 passed**, 격리 위반 0. 방어 코드 변이 61개 중 60개 kill(생존 1 = 고아 루프 관문; 리뷰 반영분의 가드 — 엔진 재검증 2곳·스케줄러 세대 재검증·등록 시점 의도·방향 필터·하트비트 선택·백오프·롤백 없는 해제 — 도 각각 kill 확인). 엔진·스케줄러·ExitManager·하트비트를 다루는 기존 시험 묶음 KST 586 passed / 2 known xfailed. 새 장부 속성 때문에 `object.__new__(RiskManager)` 하네스 4곳에 초기화 한 줄 추가(단언 변경 0). 전체 suite KST: 1회차 반영 전 1910 → 1회차 반영 후(`9206fe1`) 1925 passed / 2 known xfailed(격리 0); 2회차 반영 후(`83a3db4`) 1941 passed; 3회차 반영 후(`859f7dc`) 1946 passed; 4회차 반영 후(`f816d09`) 1948 passed; main 병합 뒤(`7ce54b4`) 1968 passed; 6회차 반영 후 전체는 PR 에 기록한다. 작업 중 PR #81 head 가 `ba966d1` → `9c9d6dc` 로 이동해 그 위로 rebase 했다.

수정: `src/core/engine.py`, `src/schedulers/kr_scheduler.py`, `tests/test_stale_sell_cancel_failure.py`(신규), 하네스 한 줄(`tests/test_engine_stale_pending_fixes.py`·`test_entry_risk_lifecycle.py`·`test_t11_acceptance.py`·`test_t11_entry_plan.py`), `docs/risk/risk-and-exit.md`, `docs/operations/monitoring-checkpoints.md`, `CLAUDE.md`
...
- **리뷰 반영(같은 PR):** 독립 리뷰(claude-opus-5/xhigh) P0 0·P1 0·P2 7 → APPROVE, 교차 리뷰(Codex gpt-6-astra/xhigh, rollout 으로 실제 모델 확인) P0 0·P1 3·P2 3 → REQUEST_CHANGES. 지적을 실코드와 대조해 수용 6·기각 4.
  - 수용 ① `on_signal` 을 거치지 않는 직접 제출 경로: **stale SELL 시장가 폴백**은 면제가 런타임에 등록되기 전(`run_manual_buy_orders` 의 `add_exit_exempt`)에 나간 지정가를 시장가로 재주문할 수 있었다 → 면제면 취소·pending 해제만. `on_order` 는 `submit_order` 직전에 재검사. 첫 판의 "면제 종목은 pending SELL 이 생기지 않아 도달 불가"는 **면제가 먼저 등록된 경우에만 참**인 논증이었다(두 리뷰 공통 지적).
  - 수용 ② **안전자산 자동 청산**은 브로커 직접 제출이고 대상이 'KOFR 고정'이 아니라 후보 4종 중 하나였다(첫 판 문서의 단정은 오류 — 교차 리뷰가 반증) → 면제면 청산 스킵.
  - 수용 ③ `no_auto_exit_symbols` 등록을 `str().zfill(6)` 으로 포지션 키와 맞춤(따옴표 없는 `87010`). 8진수로 읽혀 복구 불가한 경우(`000660`→432, PyYAML 실측)는 기동 시 `logger.error`.
  - 수용 ④ 경고 스로틀이 `_last_signal_time` 을 써서 면제 해제 직후 30초간 정상 SELL 이 쿨다운에 걸렸고, 시험이 `.clear()` 로 그 부작용을 가리고 있었다 → 전용 dict 로 분리, 시험은 초기화 없이 통과를 확인. ⑤ 시험의 `candidates or []` → `is not None`. ⑥ 고아 pending 없음(주장 C)을 시험으로 고정.
  - **재리뷰(Codex, 델타 `11ba796..a09ea9a`) REQUEST_CHANGES P1 2 → 둘 다 수용:** ⓐ stale 면제 분기가 취소 반환값을 보지 않고 pending 을 풀었다 — 브로커 취소는 실패를 예외로 올리지 않고 0 을 주므로(메모리의 KIS 브로커 계약과 동일) 거래소에 지정가가 살아 있는 채 추적만 잃을 수 있다 → 취소 뒤 `get_open_orders` 에 그 종목 활성 주문이 남아 있으면 pending 유지·60초 뒤 재취소, 없을 때만 해제. ⓑ 비면제로 루프에 들어온 뒤 취소 await 도중 면제가 등록되면 시장가 폴백이 그대로 나갔다 → 제출 직전 재검사. 면제 판정은 `RiskManager._is_exit_exempt()` 한 곳으로 모음.
  - **2차 재리뷰(Codex, 델타 `a09ea9a..5c1fa81`) REQUEST_CHANGES P1 1·P2 1 → 둘 다 수용:** ⓒ 엔진이 취소 실패로 유지한 pending 을 `kr_scheduler._cleanup_stale_pending` 이 3분 뒤 취소 0건을 '소멸'로 보고 다시 풀었다(스케줄러 발행 SELL 이 대부분이라 ⓐ가 사실상 무력) → 타임아웃·고아 해제 두 경로 모두 면제 종목이면 브로커 추적을 확인해 남아 있으면 양쪽 pending 보존·60초 뒤 재취소(`_exempt_sell_still_open`). 비면제 종목의 해제 동작은 불변. ⓓ ⓐ의 재시도 간격을 pending 시각 되감기(now−30초)로 만들어 헬스 모니터의 300초 교착 경보를 가렸다 → 전용 스로틀 `_exempt_cancel_last_try` 로 바꾸고 시각은 보존.
  - 기각: 차단 결과를 발행처에 되돌리는 콜백(발행~소비 수 ms·기동 시점 한정, 기존 `_cleanup_stale_pending` 이 회수 — 새 추상화 불필요), SELL 차단의 `signal_events` 원장 기록(`_log_sig` 는 설계상 BUY 전용, 다른 SELL 차단과 동일), 상류 틱 로그 소음(기존 동작·실발생 0), Tier1 stale 알림까지 빠지는 것(의도 — "침묵이 사용자 지시" 선례).
- **그대로 둔 것:** `gap_and_go`/`theme_chasing` 이 남의 전략 포지션(면제 아님)에도 자체 청산을 내는 동작(별도 판단). 금요 트림·안전자산 청산의 발행처 검사는 스케줄러 무한 루프 안이라 단위 시험이 없다(트림은 중앙 가드 시험 `core_trim` 이 주문을 받는다). `run_trader` 정규화도 기동 경로라 단위 시험 없음.
- **검증:** 신규 `tests/test_exit_exempt_sell_guard.py` 17건 — 첫 10건은 수정 전 **10 failed**(전부 "면제 종목 SELL 이 `OrderEvent`/emit 이 된다" 단언)로 RED 고정 후 커밋, 수정 후 GREEN. 리뷰 반영분 3건(스로틀 분리·stale 폴백·`on_order` 재검사)은 직전 커밋의 `engine.py` 로 돌려 **3 failed** 확인 후 GREEN, 스케줄러 조기 반환 시험은 기존 가드의 특성화(전후 모두 통과). 재리뷰 반영분 2건(취소 실패 시 pending 유지·취소 중 면제 등록)도 직전 `engine.py` 에서 **2 failed** 확인 후 GREEN, 비면제 폴백 대조군 1건 추가. 브로커 대역은 실제 KIS 계약(취소 성공분만 추적에서 제거, 실패는 0건)을 따른다. 2차 재리뷰 반영분(스케줄러 정리 회귀 시험 신규 + 취소 실패 시험을 고정 시계 59초/61초 전진으로 재작성 — 시각 직접 덮어쓰기 제거)도 직전 제품 코드에서 **2 failed** 확인 후 GREEN. PR #81 과의 병합 트리에서 양쪽 시험 46 passed(당시 16+30건). 실제 `GapAndGoStrategy` → `StrategyManager` → `on_signal` 연결, 발행처 5종 × 중앙 가드, 비면제 SELL 통과·live set 해제 추종, 코어 조기경보(알림 포함)·리밸런싱 폴백·교체(매수 교착 없음). 인접 엔진·batch_analyzer·스케줄러 시험 10파일 UTC/KST 각각 207 passed·격리 0건. 전체 suite 는 PR `verify`(첫 판 1858 passed / 2 xfailed) — 로컬은 다른 세션의 pytest·스왑 포화로 미실행.
...
- **고침 ⑤ 10분 BUY 정리의 '취소 0건도 해제'**: 조사 중 확인한 근본 원인 — `KISKRBroker.cancel_all_for_symbol`/`cancel_order` 는 실패를 예외로 올리지 않고 0/False 를 돌려준다. 2026-08-04 P1 의 "0건은 해제, API 예외만 유지-재시도"에서 뒤 절반은 도달한 적이 없고, 0건에 "추적 주문 없음(소멸)"과 "취소 실패(방금 체결/거래소 생존)"가 섞여 예약현금이 풀리고 즉시 재매수가 허용됐다. 이제 0건이면 ⓞ 부분체결로 포지션이 이미 있으면 종전대로 해제(pending 이 그 종목의 손절 신호까지 막으므로 청산 우선) ⓐ 브로커 미추적 → 종전대로 해제(예약 잠김 방지 유지) ⓑ 추적 중 첫 회 → 유지·60초 뒤 재시도(체결이면 5초 주기 FillEvent 가 지움) ⓒ 그 뒤에도 남으면 거래소 실 미체결(TTTC8036R, ExitManager pending 검증자와 같은 조회) — 생존·판단 불가는 유지, 없으면 해제(조회는 첫 페이지만 읽으므로, 종목을 지정한 호출에 한해 '첫 페이지에 없음 + 헤더 `tr_cont` F/M'이면 브로커가 `None` 반환 — 찾은 생존 증거는 버리지 않고, 종목 미지정 호출인 ExitManager 검증자는 종전 동작 그대로) ⓓ 유지 상한은 **연속 판단 불가 15회**(60초 간격, 최소 15분)에만 — 예약현금이 자정까지 잠기지 않게 CRITICAL 로그와 함께 강제 해제. **거래소 생존이 확인된 주문은 상한과 무관하게 유지** ⓔ 유지 중이던 BUY 가 나중에 부분체결되면 `on_fill` 이 즉시 해제하고, FillEvent 없이 잔고 동기화로만 포지션이 반영된 경우는 `_check_exit_signal` 이 pending 검사 직전에 `release_kept_stale_buy` 로 해제(다음 SIGNAL 까지 보유분의 손절 신호가 막히지 않게) ⓕ 0건 해제 시에는 언제나 `block_symbol` 5분(BUY 전용) — '미추적'이 방금 체결(FillEvent 대기)일 수 있어 포지션 반영 전 재매수를 막는다. 새 장부 없이 기존 `_pending_fallback_count` 를 유지 횟수로 같이 쓰고(락 안·pending 존재 확인 후 기록, 새 pending 등록 시 0회로 초기화 — 잔존 값이 SELL 폴백 예산을 깎지 않게), 추가 KIS 호출은 ⓒ 에서만. 60초 스로틀은 pending 시각 되감기라 유지 중 모니터링 경과 표시가 9~10분에 머문다(기록된 한계).
- **두고 기록 ② exit_exempt 가드 없음**: 현재 상태(면제 = 계속 보유 중인 `manual` 087010)에서는 면제 종목이 엔진 SELL pending 에 들어오는 경로를 찾지 못했다(SELL 발행처 전수 확인 — 면제 가드 뒤이거나 `core_holding`/전략 자체 포지션 한정, 수동 매도는 별도 브로커 인스턴스). 이 루프의 가드는 위치도 맞지 않는다(그 시점엔 지정가가 이미 나감). 단 구조적 불변식은 아니다 — 코어 경로에는 면제 가드가 없고, 전략 관리자는 보유 포지션을 모든 활성 전략에 넘기므로 같은 날 갭 후보 등록 뒤 수동 매수된 면제 종목은 `gap_and_go` 가 SELL 을 낼 수 있다(발행처의 공백, 중앙 가드는 후속 과제).
- **두고 기록 ③ 폴백 `submit_order` 예외 시 `clear_pending`**: `KISKRBroker.submit_order` 가 POST 이후 예외를 전부 삼켜 `(False, msg)` 로 돌려주므로 이 `except` 는 POST 전 예외만 닿는다(미발주 확실). 응답 유실은 일반 실패 분기로 들어오는데, `on_order`(즉시 해제)와 달리 폴백의 첫 실패는 횟수만 올리고 다음 SIGNAL 에서 다시 제출한다 — 분할 매도에서는 중복 여지가 남는 잔존 위험. 3상태 구분은 브로커 계약 변경이라 범위 밖.
- **두고 기록 ④ 폴백 상한 뒤 방치**: 횟수는 제출 실패에도 오르므로 '시장가 2건 각 90초 미체결' 또는 '제출 실패 1회 + 시장가 1건 90초 미체결'로 도달. 살아 있는 주문은 브로커가 계속 추적해 체결은 반영되고 전량 중복은 KIS 가 수량 초과로 거절하나 분할 매도 중복은 막지 못한다.
- **교차 공급자 리뷰(Codex, 세션 기록상 gpt-6-astra / xhigh, 정적 — 샌드박스에서 pytest 임시 디렉터리 생성 불가로 실행 실패)**: 판정 **병합 불가**(P0 0 · P1 5 · P2 2). 새 코드에 대한 P1 3건(상한에서 생존 확인된 주문까지 해제 / 부분체결 포지션의 손절 차단 연장 / 첫 페이지만으로 소멸 판정)과 P1-4(미추적 = 방금 체결 경쟁)의 완화, P2 2건(상한 도달 조건·면제 경로 서술 오류)을 위 ⓞⓒⓓⓕ 와 문서 정정으로 반영했다. **재리뷰(같은 요청 모델·effort, 정적)도 병합 불가** — 반영이 만든 순증 위험 P1 2건(유지 후 부분체결 시 다음 SIGNAL 까지 청산 차단 / 브로커가 첫 페이지에서 찾은 생존 증거까지 `None` 으로 낮춰 ExitManager 30분 하드 만료 뒤 재발행 가능)과 P2 1건(상한 카운터가 '연속'이 아님)을 ⓔ·ⓒ 의 종목 지정 3상태·ⓓ 의 연속 횟수로 다시 반영했다. **3회차도 병합 불가**(순증 위험 P1 1건 — 나머지 두 건은 해소 확인): 유지 중 BUY 의 포지션이 FillEvent 없이 잔고 동기화로 먼저 반영되면 `on_fill` 해제가 돌지 않는다 → ⓔ 의 청산 검사 직전 해제로 반영. **4회차: 병합 가능**(신규 순증 위험 P0·P1·P2 0, 대상 `9c9d6dc`·기준 main `d8d78fe`, 세션 기록상 gpt-6-astra/xhigh, 정적). 4회차 뒤 main 에 PR #83(exit_exempt 중앙 가드 — 같은 stale SELL 루프 머리에 면제 분기 추가)이 들어와 병합했다: 텍스트 충돌 0, 면제 취소 분기 → 폴백 상한 → 동시호가 유지 순서라 면제 종목은 동시호가에도 취소가 우선하고 동시호가 유지는 비면제 종목에만 적용된다. 두 PR 의 시험 47건 동시 통과, 결합 상태의 전체 suite 는 PR 의 필수 verify 로 확인(결합 상태에 대한 교차 리뷰는 별도로 받지 않았다). **미반영 잔존**: P1-5 매도 쪽 취소 실패 뒤 재발행(분할 매도 이중 매도 — 엔진 폴백 + 스케줄러 정리, 이번 PR 의 회귀는 아님)과 주문 ID 단위 종료·체결 대사 부재는 매도 경로 재설계 범위라 최우선 후속 과제로 기록. 반영분에 대한 교차 재리뷰 결과는 PR 코멘트.
...
- **검증**: RED(목표 3건 실패/회귀 가드 4건 통과) → 수정. 독립 리뷰(claude-opus-5, 요청 effort high·실효 effort 미확인, 같은 제공자라 교차 제공자 리뷰 아님) 조건부 승인 — P1 1건(유지 상한 부재)·P2 중 횟수 기록 경쟁·동시호가 분기 범위·시험 누락을 반영, 시각 되감기 표시 왜곡은 한계로 기록. 재검토 **승인**(P1 해소) — 추가 P2(강제 해제 뒤 즉시 재매수 가능, 공유 장부 누출 시 SELL 폴백 예산 소진) 2건도 각 한 줄로 반영, 예외 경로의 상한 미적용은 한계로 기록. 교차 리뷰 2회 반영 후 `tests/test_engine_stale_pending_fixes.py` UTC/KST 각 30 passed(누적 방어 코드 12곳 변이 시 해당 시험 실패 확인 — 같은 호출의 같은 종목 BUY 차단, `on_fill` 부분체결 해제, 생존 증거 보존, 동기화 반영 포지션의 SIGNAL 없는 손절 발행 포함), 엔진·스케줄러·브로커·ExitManager 의존 시험 561 passed / 2 known xfailed, 격리 위반 0. 교차 리뷰 반영 전 전체 suite KST 1826 passed / 2 known xfailed(기준선 1810 + 신규 16); 반영 후 전체는 PR 의 필수 verify 로 확인.

수정: `src/core/engine.py`, `src/execution/broker/kis_kr.py`(`get_exchange_open_orders(symbol=…)` 종목 지정 3상태, 미지정 호출 동작 불변), `src/schedulers/kr_scheduler.py`(`_check_exit_signal` 의 pending 검사 직전 1호출), `tests/test_engine_stale_pending_fixes.py`(신규), `docs/risk/risk-and-exit.md`
...
- 정책 ABA와 완료 전 검증/지속 소비 권한의 구별을 `MEMORY.md`의 재사용 가능한 교훈으로 기록했다. 변경 이력이나 운영 승인으로 쓰지 않는다.
...
- 최종 UTC/KST 순차 전체 검증 각각 **1809 passed/2 known xfailed/1 기존 warning**, 격리0·비밀정보 검사 통과. 독립 broad 및 후속 테스트 한정 리뷰 C/I/M0. 초기 CI의 시험 Python patch 불일치를 `sys.executable -I -S`로 수정하고 운영 exact-version 거부 검사는 유지했다. CI/main 병합 이력은 PR #74, 실제 ON은 일정 확정 및 운영 인수 후 별도 기록한다.
...
- **T11 팀 근거·EntryPlan shadow**: 10:30 슬롯 원장 6행 append. `success_probability` 6/6 None, `data_sufficiency` partial 6/6, `stance_v2` hold 6/6, `execution_state` candidate 6/6, 기권 0. `shadow_plan_check` 0건 — 현금 0.49% 라 매수 신호가 엔진 현금 검사에서 먼저 걸리므로 **정상**(훅이 그 뒤에 있다).
- **KIS 원장 연속조회 종료**: 같은 시간대 비교로 원장 거절 **236건 → 46건(80% 감소)**. 다만 페이지 수·스냅샷 재사용 로그가 `logger.debug` 라 운영 로그에 안 남아 **직접 증거는 미확보**(간접 증거만). 동기화 잔고 실패 2회는 재시도로 복구.
- **T12 Phase 0 스크리닝**: 거래량 급증 채널이 **0개 → 1~3개로 복구**(배포 전 30일간 2,474회 전부 0). 0개인 경우도 새 계측으로 '응답 30행·게이트 탈락'이 구분된다. 네이버 경고 0건.
  - **정정**: PR #60 본문의 "배포 전 후보 26~27개"는 30일 평균이었고 어제 같은 시간대는 이미 34개였다. 오늘 33개로 **통합 후보 수 증가는 확인되지 않았다.** 개선 효과 판정은 같은 시간대 비교로만 한다(문서에 주의사항 기재).
- 미확인 이월: 16:00 수급 5일 배치 실행 여부(저녁 재확인), 장외 300초 동기화 게이트, 채널별 기여 수 계측.
...
- 기존 KIS 투자자 일별 조회·공급자 미주입 시 일일 수급 폴백, 일반 pykrx, 네이버 뉴스·DART 직접 검증은 유지한다. 공개 검증 결과의 수급/공매도/버즈 기본값은 계속 미획득·무가산이며 `approved=True`를 실제 검증 성공이나 위험 해소로 바꾸지 않는다. 확인된 공시 위험 차단·뉴스/공시 조정은 보존한다.
...
- **거래량 급증 순위 (30일간 2,474회 전부 0개)** — `kr_screener.screen_volume_surge` 의 `FID_COND_SCR_DIV_CODE` 가 `20101` 이었다. 이 TR(FHPST01710000)의 화면코드가 아니라서 KIS 가 `rt_cd=0 "정상처리"` 와 함께 **output 0행**을 돌려줬다(오류 로그가 0건이던 이유). **라이브 프로브로 확정**: 같은 토큰·같은 시각에 `20101` → 0행, `20171` → 30행. `20171`(HTS [0171])로 교정하고 `FID_TRGT_EXLS_CLS_CODE` 10자리·미사용 조건 공백으로 스펙을 맞췄다.
  - 진단이 함께 제안한 `FID_BLNG_CLS_CODE=1`(거래량증가율순)은 **채택하지 않았다** — 같은 프로브에서 증가율순 상위 3종목이 전부 전일 거래량 2~6주짜리 채권형 ETF·ETN(증가율 9999.99%)이었다. 평균거래량순(`0`)을 유지한다.
  - `vol_inrt` 결측·비숫자 시 같은 응답의 `prdy_vol` 로 자체 산출하고, 둘 다 없으면 **1.0 을 지어내지 않고 건너뛴다**(1.0 은 `min_volume_ratio=2.0` 미만이라 조용한 전량 탈락이 된다). 0개일 때 응답 행수·비율산출불가 건수를 WARNING 으로 남겨 "빈 응답"과 "전량 탈락"을 구분한다.
- **전종목 5일 수급 배치 (30일간 0회 실행)** — `kr_scheduler` 가 존재하지 않는 `..data.providers.supply_score_provider` 를 import 해 매 거래일 16:00 `ModuleNotFoundError` 가 나고 `except` 가 WARNING 한 줄로 삼켰다. 실제 모듈은 `supply_score`. 함수 본문 안 lazy import 라 기동 시엔 드러나지 않았다.
- **네이버 금융 크롤링** — (1) `_naver_crawl` 이 `kis_rate_limit.acquire()` 를 호출해 **KIS 초당 예산을 소모**하고 있었다(2026-09-03 에 같은 파일의 진짜 KIS 호출 2곳과 일괄 삽입된 줄). 외부 사이트이므로 제거. (2) 2026-09-11 테이블 구조 변경 이후 파싱 성공 0건(796회), KIS 실패 시 주력으로 승격되는 경로도 29회 전부 0종목이라 `screen_all(use_naver=...)` 기본값을 **False** 로 내렸다. 일 265건의 WARNING 노이즈가 사라진다. 파싱 코드는 토스 도입 작업과 충돌을 피해 남겨 둔다.
- 테스트: `tests/test_phase0_screener_supply_2026_09_15.py` 12건 — 화면코드·정렬·스펙 파라미터 단언, vol_inrt/prdy_vol/비숫자/산출불가 4경로, 네이버 리미터 미사용·기본 OFF, **내부 import 해석 검사**(4개 핵심 파일의 상대 import 를 `find_spec` 으로 전수 확인 — 같은 유형의 오타 재발 차단).
- 돈 경로: 발굴 후보가 늘어나는 방향이라 **신규 매수 후보 수가 바뀐다**. 현재 현금 0.4%로 실주문은 나가지 않지만, 스크리닝 점수 분포 변화는 배포 후 관측 대상.

## 2026-09-15 — docs: 토스증권 Open API 2차 데이터 소스 도입 설계 (T12, 구현 미착수)

계기: KIS 초당 한도·HTTP 500 반복에 더해, 조사 과정에서 **백업 채널이 이미 사망**한 것이 확인됐다 — 거래량 급증 순위 30일간 2,474회 전부 0건(`vol_inrt` 파싱), 신고가 포착 2,469회 전부 0건, 네이버 크롤링 09-11부터 파싱 0건, pykrx WICS 30일 성공 0건, 전종목 수급 배치는 `supply_score_provider` 모듈명 오타로 한 번도 실행되지 않음. 스크리닝 후보가 35개 → 26개로 약 25% 감소.
- 설계서 `docs/superpowers/plans/2026-09-15-toss-securities-fallback.md` 신규 — 경계(주문·잔고는 KIS 단독), 커버리지 매핑, 토큰 단일성·리미터·라우팅 아키텍처, Phase 0~4 단계 도입, 위험(가격 기준 차이·토큰 상호 무효화·세션 자동 교체 금지)·보류 항목.
- 실측 확인(운영 서버에서 직접 호출): 토큰 발급 200(`expires_in` 86399, 클라이언트당 1개), 허용 IP 통과, 시세·캔들·랭킹·수급·지수·캘린더·종목 마스터 전부 200. rate limit 그룹별 TPS 헤더 확인.
- 장 운영시간 실측(KRX+NXT 통합): 프리 08:00~09:00(08:50~ 단일가), 정규 09:00~15:30(15:20~ 종가 단일가), 애프터 15:30~20:00(15:40 단일가 종료). 코드 `utils/session.py` 는 단일가 구간을 휴장으로 빼는 보수적 매핑이라 주문 안전상 어긋나지 않으나 하드코딩이라 캘린더 대조가 필요.
- 문서: `docs/integrations/external-apis.md` 에 토스 절 추가(미구현 명시).
- **구현·배포 없음.** Phase 0(자체 버그 3건)·약관 확인이 선행 조건.
...
- **최종 리뷰 후속(blocking 2 + advisory)**: (1) `shadow_plan_check` 행이 `signal_events` 총계·전략별·일별 집계(block_rate 분모)·`/api/signal-events` 기본 조회·SSE 실시간 피드에 섞여 '감점' 으로 오독되던 계측 오염 → 집계·기본 조회는 `event_type IN ('passed','blocked','penalized')` 로 한정, shadow 행은 SSE 미발송(`type=shadow_plan_check` 명시 조회만). (2) `judgment` 의 '검증 통과' +10 취소를 positive_basis 유무와 무관하게 **항상 적용**으로 확정(계약 §2.2 문언, 미결 주석 제거). advisory: 계획으로 읽을 수 없는 입력은 allow 가 아니라 `wait INPUT_MISSING:plan`; 원장 마스킹에서 순수 숫자열 규칙 제거(숫자만인 plan_id 보호, 계좌 8-2 형식·키 기반 마스킹 유지); shadow 검증기에 넘기는 급락 수준은 당일 갱신분만(`intraday_state_as_of` 메타 추가, 기준선 키 `intraday_state` 불변); 죽은 `_quote_at` 제거; `/engine` 카드 `plan_rejected` 라벨; timing 러너 `reports_without_age` 카운터 + 일봉 자정 판정 시각 → PLAN_EXPIRED 한계 명시.
...
## 2026-09-15 — fix(broker): KIS 원장 8434R 연속조회 종료 판정·잔고 스냅샷 재사용·동기화 세션 게이트 (EGW00215 반복 원인)

계기: 배포 후 ops-check 에서 원장 초당 한도 초과(EGW00215) 경고가 2시간에 200건대로 반복. 4관점 조사(호출자·리미터·기능 영향·이력, 24건 지적 → 적대적 검증 18건 확정)로 원인 확정 — 전부 잔고조회 `TTTC8434R` 이며 호출자는 30초 포트폴리오 동기화 단독.
- **원인 1** (`kis_kr.get_positions`): KIS 는 마지막 페이지에도 `ctx_area_fk100/nk100` 을 채워 보내는데 루프가 키 비움만 종료 조건으로 써서 **보유 1종목 계좌가 8434R 을 페이지 상한 10회까지 호출**(동기화 1틱당 11회). `_api_get` 이 응답 헤더 `tr_cont` 를 `data["_tr_cont"]` 로 실어 D/E(마지막)면 즉시 종료하고, 헤더가 없으면 `get_positions_for_account` 의 동일 ctx 키 가드로 2회에 종료. 체결 조회 `_query_daily_fills`(8001R)에도 같은 종료 판정(주문이 열린 날 원장 포화·체결 인지 지연의 구조적 원인).
- **원인 2** (`get_account_balance`): 2026-09-11 잔고 스냅샷 재사용의 '다음 페이지 있음' 판정이 같은 ctx 키 기반이라 **항상 True → 운영에서 한 번도 발동하지 않음**. 헤더 F/M 로 판정(헤더 없으면 기존 키 기반 유지). 효과: 1틱당 8434R 11회 → 1회.
- **낭비** (`kr_scheduler.run_portfolio_sync`): 세션 게이트 없이 24시간 30초 → 야간·주말 8434R 1,440건/일(일요일에도 거절 315건). **장외 CLOSED(거래일 15:40 이후 NXT 종료 뒤·08:00 이전·휴장일)만 300초**, 전환 직후 1회는 30초 유지. KRSession 이 CLOSED 로 보는 08:50~09:00·15:20~15:40 동시호가 구간은 지정가 이월 체결이 있어 30초를 유지하고, 300초 수면이 09:00 개장을 가로지르지 않는다(리뷰 반영). 장외에서는 `engine.is_trading_hours()` 가 신호 주문을 차단하고 직접 호출 경로(안전자산·미체결 폴백)는 시장가라 브로커가 거부하므로 재정합할 신규 체결이 없다. **채택하지 않은 안**: 장중 pending 유무로 30→180초 연장 — `_pending_orders` 는 POST 성공 응답 뒤에만 등록되므로 응답 유실(체결은 됨) 시나리오에서 정확히 fail-open 하여 미등록 체결이 손절 없이 방치된다.
- 계측: `_api_get` 의 3번째 500 도 EGW00201/00215 분류·리미터 계측·경고 로그를 거친다(이전엔 마지막 시도분이 계측·백오프에서 누락돼 거절 카운트가 실제보다 1/3 적게 집계). **호출자 분기는 불변** — 본문이 파싱되면 그 KIS 오류 본문(rt_cd≠0)을 반환하고 파싱 실패·429/502/503 만 합성 `rt_cd -1`. '포지션 0건' 표현은 `get_positions` 가 rt_cd 실패를 빈 dict 로 돌려주는 기존 계약이며 방어는 동기화의 평가액>0 + 5초 재조회 가드가 담당한다. 매수가능조회(8908R) 실패의 예수금 폴백을 DEBUG→WARNING(그 값이 `portfolio.cash` 에 들어간다).
- 기능 영향(오늘 실측): 3회 소진 4건 → 동기화 스킵 2회, '포지션 0건' 오판 2회(5초 재시도로 복구, 유령 삭제·매수 차단·주문 영향 0). 기각된 가설: 리미터 동시성·acquire 누락·다중 프로세스·백오프 부족(평일 09~15시 거절 잔여분은 KIS 측 요인, 재시도가 흡수).
- 테스트: `tests/test_kis_ledger_pagination_2026_09_15.py` 10건(헤더 D 1회 종료·동일 키 2회 종료·진짜 2페이지 병합·체결 루프 동일·스냅샷 발동/미발동/헤더 없음 폴백·3번째 500 계측·헤더 캡처·동기화 300초 전환 규칙). 문서: runbook 동기화 주기·'해결' 문구 정정, system-overview 주기표.
- **배포**: PR #56 머지 후 18:31 KST `local_deploy.sh 678e132` 성공(verify 796 passed / 2 xfailed) — KIS 연결 18:31:46·엔진 시작 18:31:51, ERROR 0, pending 0, 운영 체크아웃 main 복귀(트리 동일). 재기동 150초 후 ops-check: 재기동 이후 ERROR 0·동기화 경고 0·하트비트 정체/실패 없음·pending 0, **재기동 후 원장 거절(EGW00215) 0건**(재기동 전 같은 날 360건).
- 적대적 리뷰(3관점 15건 → 8건 확정, blocking 0): 동기화 게이트 전제 오류·개장 경계 가로지르기(위 반영), CHANGELOG 과장 문구 정정, 테스트 범위 문구 한정. **후속(별도 PR)**: KR 브로커는 연속조회 요청 헤더 `tr_cont: N` 을 보내지 않아 다중 페이지 계좌(8434R 50건/페이지 초과 보유)의 2페이지 이후를 받은 적이 없다(초기 커밋부터, 현 계좌 ≤9종목이라 미발화) — `_api_get` 에 extra_headers 를 받아 2회차부터 `N` 을 싣고 마지막 페이지가 가득 차면 WARNING. 서버가 커서를 무시 중이면 원장 호출이 늘 수 있어 이 핫픽스와 분리.
- 미채택 advisory(기록): TTTS3007R 원장 셋 추가(해외 미운용), kis_us 공용 리미터 미사용(US 미운용), ledger_busy 예외 누수(10초 타임아웃 방어 존재), 주문 POST 원장 게이트 밖(관측만 제안, 재전송 금지 계약 유지), 로그 문구 2건.
...
- `src/utils/data_freshness.py` `kr_night_futures_as_of`: 세션 존재를 **개시일 기준 KRX 거래일**로 판정(`src.utils.session.is_kr_market_holiday` 연동 — 주말+공휴일, 동적 조회 우선·내장 폴백). 거래일 저녁이면 익일이 공휴일이어도 개장(연휴 전날), 공휴일 당일 저녁은 휴장. 직전 세션 역산·다음 개장 계산도 같은 캘린더를 쓴다. 세션 종료 시각을 05:00→**06:00** 으로 정정(KRX 파생상품시장 야간거래 안내·FAQ 2025-04-28: 거래시간 18:00~익일 06:00, 호가 17:50~) — 사실 정정이며 승인된 규칙 구조는 그대로.
- 의미 변경: 공휴일 전날·당일·연휴 중 조회의 as_of/유효기간이 달력대로 계산된다(예: 추석 09-24 20:00 조회 → as_of 09-24 06:00, 09-28 18:00 까지 유효). 05:00~06:00 구간은 이제 세션 중(실시간)으로 본다.
- 테스트: `tests/test_t10_data_validity.py` 공휴일 사례(추석 전날·당일·연휴 중·한글날·월요일 새벽·05:30/06:30 경계) 추가, 05:00 기대값 3건을 06:00 으로 정정(근거 docstring). 전체 553 passed / 2 xfailed / 격리 0.
- 문서: `docs/agents/expert-system.md` 세션 규칙 절, 계획서 T10 정책 항목 승인 표기. 운영 미배포(별도 승인).
...
- **통합 최종 리뷰 반영** (INT-1 돈 경로 승인·INT-2 자료→평가 blocking 1건): 지수 4종이 전부 결측(개별 종목만 응답)이면 `avg_pct=0` 이 "미국 지수 평균 +0.00% 보합권 마감" 을 세 발송 경로로 내보내던 0 포장 제거(`⚠️ 지수 시세 미수집 — 마감 방향 판단 불가`, `get_overnight_signal` summary·`indices_missing`), 5분 급락 루프가 등락률 결측(NaN)이면 감지기 이전 상태를 어댑터에 now 로 재각인하지 않음, 07:30 발송 기록 `valid_n` 을 `data_status_summary` 커버리지 기준으로, 아카이브 경로 상수(`MORNING_BRIEF_ARCHIVE_DIR`) 이중 출처 제거. D 통합 E2E `tests/test_t10_e2e_flow.py` 8건(07:00→07:30→12:00→20:30 고정 시계·메모리 공급자, 전일 상태·역순 도착·조회 실패·발송 실패·부분 응답·재시작·curl_cffi 차단).
- **배포 1차 시도 롤백(03:03 KST) → conftest 수정**: 운영 체크아웃에서 `verify.sh` 가 돌 때 pytest 의 rootdir 스캔이 `<prod>/.env/pyvenv.cfg`·`logs/` 를 stat 하는데 격리 가드가 운영 `.env`/logs/results 의 메타데이터 접근까지 차단해 수집이 실패 → `local_deploy.sh` 자동 롤백(de111b7, 서비스 active 유지). 운영 루트 항목은 내용 읽기(open)·변경만 차단하고 stat/scandir 은 허용하도록 정정(`BLOCKED_CONTENT_PREFIXES`), 워크트리에 `.env`/`logs` 를 만들어 `QWQ_PROD_ROOT` 로 재현·검증.
- **테스트**: 전체 스위트 **552 passed / 2 xfailed**, 테스트 격리(`tests/conftest.py`, socket+curl_cffi+운영 상태 파일 차단) 위반 0건. D 독립 재현(`tests/test_t10_repro_{a,b,c}.py`, 기준 SHA `a59e29f`) 14건 통과. Codex 교차 리뷰는 bwrap 샌드박스 오류로 **미실행**(Claude opus/xhigh 독립 리뷰로 대체).
...
- **배포**: PR #47 머지(main `8c8a6bf`) → 1차 배포 롤백 → PR #48 핫픽스 머지(main `3f6b1bf`) → **2026-09-15 03:08 KST `local_deploy.sh 3f6b1bf` 성공·재기동 검증**(KIS 연결 03:08:57, 엔진 시작 03:09:24, 헬스 통과, ERROR 는 구 프로세스 종료 잡음 1건). 운영 서버는 이제 F1~F22 전부 포함한 `3f6b1bf`. 상세: `docs/reviews/remediation-2026-09-14.md` §11.8, 계획서 T10 절.
...
## 2026-09-14 — fix: 장중 레짐 입력 신선도·결측 정직성 + B·C 인터페이스 배선 (계획서 T9 A, F9·F10)

계기: 09-14 12:00 장중 레짐 재분류가 08:20 스크리너 메모리(KOSPI 5일 +3.3%/20일 +1.4%)를 재사용해 당일 -3.34% 급락(09:05 crash) 중 `trending_bull` 0.85; LLM 입력이 `SP500/SOX/VIX` 키를 찾지만 공급자는 `S&P500/반도체(SOX)`·VIX 없음 → 0 으로 전달.
- `src/schedulers/kr_scheduler.py`(`_run_llm_regime_classifier` 입력 경로): 12:00 실행은 분류 전 `fetch_index_price` 로 KOSPI/KOSDAQ 당일 등락을 재조회하고 스크리너 종가열에 덧붙여 5/20일 재계산(마지막 봉이 오늘이거나 날짜 미상(KIS 폴백)이면 덧붙이지 않음 — 이중 계상 방지). 조회 실패 시 스크리너 **로드 시각** + "장중 갱신 실패" 표기.
  `_pct_change` 는 표본 부족·기준가 0 을 None 으로(get_kospi_change 의 0.0 불사용) → 빈 캐시는 `missing_fields` `KOSPI_c5/c20`·`스크리너 캐시 없음`. 급락 감지 상태는 `batch_analyzer._intraday_updated_at` 당일 게이트(`_intraday_crash_snapshot`, as_of=갱신 시각; 전일 상태는 결측)로 프롬프트에 포함, crash/severe 면 LLM `trending_bull` 을 적용하지 않음 — `market_regime.cap_regime_by_intraday_risk` 단일 출처(스케줄러 캡 표 삭제), `_resolve_regime_conflict(kospi, llm, crash_level)` 도 동일. 모든 입력에 as_of·source, 결측은 '결측'(0 금지), `llm_regime_today.json` 에 `input_meta`(us/kr as_of·source·kospi_c5/c20·kospi_today_pct·intraday_crash_level·missing_fields)·`regime_capped`·`confidence_raw`(임계값 불변).
- F10: `_index_field(indices, *aliases, fields)` 별칭 조회(없으면 None) + C 의 `overnight["indices_normalized"]` 우선 소비(VIX 는 price, missing 가드), `us_as_of` 를 마감 시각/조회 시각으로 구분 표기.
- 배선: 5분 급락 루프 → `bot.engine._regime_adapter.set_intraday_risk(level, pct, as_of)`(**운영에서 유효 레짐 캡이 처음 활성화** — crash/severe 에서 bull→sideways 강등 전용, 어댑터 `effective_regime` 도 as_of 당일 게이트·시계 주입 `_now`); 07:30 전문가 브리핑에 `build_expert_conflict_note(tone|text, {"score": agg})` 문구(브리프가 이미 붙였으면 중복 방지)·`data_status_summary()`("자료 부족 N명", 커버리지 부족 시 "체제 점수 무보정"); 20:30 진화 잡에 `evaluate_morning_brief(date)` 훅(`asyncio.wait_for` 120초, 하트비트 note, 날짜당 1회). KOFR 안전자산 루프도 당일 게이트 스냅샷 사용.
- 테스트 `tests/test_regime_llm_inputs.py` 27건(수정 전 15건 실패 확인) + regime 1건. runbook "장중 레짐 입력 as_of·결측 확인", system-overview 레짐 입력 흐름, monitoring-checkpoints 항목.
- 의미 불변: 주문·청산·급락 임계값(update_intraday_state 분기)·REGIME_EXIT_PARAMS·게이트 코드 무변경. 12:00 에 KIS 지수 TR 2건 추가(원장 TR 아님). 잔여: `kr_inputs` 미전달(야간선물 시장 시각 없음), swing_screener KIS 폴백은 마지막 봉 날짜 미기록, `_morning_brief_eval_date` 는 타임아웃에도 소진(1일 1회). 운영 미배포.
...
- A 배선은 같은 날 후속 항목(장중 레짐 입력 신선도, T9 A)에서 반영: `adapter.set_intraday_risk` 주입, 저녁 `evaluate_morning_brief` 훅, 07:30 결합부 `build_expert_conflict_note`·`data_status_summary` 부착. `kr_inputs` 전달은 미배선(야간선물은 시장 시각이 없어 as_of 없음 → 운영 scope 는 여전히 us_close_only). brief_tone·claims 는 패턴 휴리스틱(없는 주장을 만들지 않는 방향, claims=None 비율로 확장 판단). `update_regime` 의 결측 0 채움(기존 코드)은 별도. 배포 없음.
...
- 잔여: `data_status_summary` 07:30 브리핑 노출과 `indices_normalized` 소비는 같은 날 T9 A 항목에서 반영(팀 컨텍스트 노출은 미반영); macro/us_market/kr_economy/global_micro/weekend 전문가의 data_status 미설정; v8 spark 폴백의 시장 시각 미매핑. 배포 없음.
...
- **리뷰** `docs/reviews/strategy-architecture-review-2026-09.md` (4개 독립 분석 교차 검증): 실거래 266건
  수수료 전 총손익 ≈ 0(차감 후 -139만, t=-0.18), KOSPI +37% 구간 자산 -7.5%. 구조 원인 5(회전·청산 사다리·
  명목 사이징·근거 없는 배분·레짐 4겹), 게이트 스택 판정의 베타 혼동(04-23 G4 약화·08-20 팀 부스트 근거가
  초과수익으로는 반대), 코드 건전성 4/10. 권고 8건(청산 단일화·회전 억제·위험 기반 사이징·배분 재편·판정 기준
  교체·계측·레짐 단일화·테스트)과 금지 목록.
- **계측 결함 수정** (`src/analytics/`): ① counterfactual 추적기가 삽입순 상위 50건만 처리해 08-24 이후 126건을
  한 번도 가격 조회하지 않던 기아 → 미가격 우선·오래된 순, 상한 150, KODEX200 대비 초과수익 `x1/x5/x20` +
  summary 초과 라인. ② gate_performance가 `penalized`를 제외해 G4 LLM 2차 검증을 한 번도 측정하지 않음(9/13
  `|wiki` 버킷도 도달 불가) → `PEN_<gate>` 버킷, (게이트,종목,일) dedup(G1 5분 재기록 13배 부풀림 제거),
  KODEX200 초과·손절 클립 병기, 판정은 초과수익 기준, 용량 게이트(G5_) 판정 제외, 최소 표본 30.
- 검증 규율 5항 추가(evolution-system.md), CLAUDE.md 리뷰 요약, docs/README Reviews 절. 테스트 3건.
...
- `/ops-check` 하루치 집계에서 EGW00215 996라인 발견 → 시간대 분포 재조사: 9/3부터 매일 09:00~15:30
  시간당 70~140건, 장외 0. 9/8의 "개장 직후 ~20건, 코드로 해결 불가" 결론은 저녁·주말 창만 본 오판이었고
  직렬화(9/7)·백오프(9/8)는 악화도 개선도 아니었다.
- 새 단서: 오늘 495건 **전부 `TTTC8434R`**, 1회 재시도 387·2회 108. 동기화가 30초마다 잔고→포지션으로
  8434R을 두 번 호출(≈780쌍)하고 두 번째가 ~50% 거절 → inquire-balance만 ~2초 간격 규칙 가설.
- `kis_rate_limit.LEDGER_TR_INTERVALS = {"TTTC8434R": 2.1}` (다른 원장 TR은 1.05 유지). 테스트 1건.
  9/11 시간대별 건수로 검증 후 맞으면 `_sync_portfolio`의 8434R 중복 호출 제거.
- runbook 알려진 이슈 정정.
...
편입·유휴 체결확인 15초·EGW00215 2초 백오프까지 적용해 우리 원장 트래픽을 초당 0.2건·완전
직렬로 낮췼는데도 개장 직후(09:00~09:15) `TTTC8434R` 원장 초과는 9/4 18 → 9/7 29 → 9/8 38
라인(이벤트 20건)으로 무관하게 발생했고 그 외 시간대는 0건이다. 원인 후보는 개장 시 KIS 원장
부하 또는 같은 계좌를 조회하는 외부 클라이언트(MTS/HTS 잔고 화면). 영향은 재시도 1~2초와
간헐적 동기화 1회 스킵뿐 → **리미터를 더 손대지 않고 관측 유지**로 결론 (runbook 알려진 이슈).
참고 실측: 게이트웨이 EGW00201은 원장 제거·10/s 페이싱으로 HTTP 500 총량 51→31건/개장 구간.
...
- **08:47 실전 점검 후속 — 프로세스 공용 KIS 리미터** (`src/utils/kis_rate_limit.py`):
  배포 후 HTTP 500이 136→23건/시(같은 시간대 전일 대비)으로 줄었으나 잔여분은 전부 시세 TR
  (FHKST01010100/03010100) `EGW00201` — `kis_market_data`·`kr_screener`가 브로커와 별도 세션·
  세마포어로 같은 appkey를 써 버스트가 합산 20/s를 넘긴 것. 시세 모듈은 재시도가 없어 업종지수·
  외국인동향 조회가 그대로 실패했다. 슬라이딩 윈도우 + 원장 간격을 한 모듈로 통합해
  브로커(`_rate_limit` 위임)·시세 10곳·스크리너 3곳이 공유. 15:41 배포 실측: 원장 EGW00215
  0건, 게이트웨이 EGW00201은 재시작 스크리닝 버스트에서 7건/3분(전일 26건) 잔존 → 전송
  기준 18/s 윈도우가 도착 지터로 서버 1초 버킷 20을 넘는 것으로 보고 15/s로 하향 → 그래도
  8건 재현(15:46). 총량이 아니라 **집중도**(gather 버스트가 허용치를 수십 ms에 소진)가 원인
  → 호출 간 최소 간격 페이싱 추가 → 그래도 15:52 스크리닝에서 8건이 1~2초 간격으로 고르게
  재현(~250건 중 3%) → 10/s로 하향 → 스크리닝 21초에 여전히 3건. 송신률이 원인이 아닐
  가능성(일봉 TR 집중) → 거절 시점의 우리 측 밀도(최근 1초 건수·직전 간격)를 `[KIS리미터]`
  경고로 계측 + 거절 후 1초 전역 hold(파일온 방지). 계측 결과(9/3 16~19시, 9/4 08~09시):
  거절 시점 우리 송신이 5~6건/초일 때도 거절 → 송신률 문제 아님(같은 appkey의 다른 소비자
  또는 서버측 한도) — 추가 하향 없이 관측. 시간당 HTTP 500은 전일 대비 −78%.
- **원장 직렬화 (9/4 09:13 점검)**: 개장 직후 09:01~09:13에만 EGW00215 13건 재발 — 잔고 응답이
  1초를 넘기면 전송 간격 1.05초로는 다음 원장 호출이 **응답 전에** 나가 겹친다 →
...
- **개장 직후 원장 초과 (9/7 점검)**: 직렬화 후에도 09:00~09:13에 EGW00215 29건(9/4 18건) —
  우리 원장 간격이 병목이 아니라 개장 시 KIS 원장 부하 또는 같은 계좌를 조회하는 외부 클라이언트
  (MTS/HTS 잔고 화면 등)로 판단. 우리가 줄일 수 있는 것: 체결확인 루프가 미체결 주문 없이도
  5초마다 TTTC8001R을 치던 것을 **유휴 15초**로(주문 있으면 2초 유지), EGW00215 수신 시 원장
  호출 **2초 백오프** + 건수 계측(`[KIS리미터] EGW00215`).
  원장 세트에 매수가능조회
...
- **배포 (2026-09-03 07:49 KST)**: PR 머지 전 운영 서버에서 `remote_deploy.sh` 절차를 수동
  ...
  이후 장 마감 후 15:41→15:46→15:51→15:57→16:02 다섯 차례 재배포(a544f03 → efaf91c →
  af9a0cb → fd4adb8 → **4256105 현재 운영**). 재시작 직후 스크리닝 구간 EGW00201: 8·8·8·3·**0**건.
  원장 EGW00215는 15:41 이후 0건, 토큰 락아웃 0건. 다음 판단 근거는 `[KIS리미터]` 계측 로그.
...
저널 경고 48시간 8,746건 중 8,120건이 `[API] HTTP 500, 1회 재시도`(14일 연속
일 ~4,000건)였고, 그 뒤에 아래 구조적 원인이 있었다.

- **P1 KIS 원장 HTTP 500 폭주** (`kis_kr.py`): `run_portfolio_sync`(30초)가
  잔고+포지션을 같은 원장 TR(TTTC8434R)로 연속 호출 → KIS 원장 서버의 계좌당
  초당 1건 제한(EGW00201 "원장에서 허용 가능한 초당 거래건수를 초과")에 걸려
  30초마다 HTTP 500 → 1초 대기 재시도. 전역 초당 18건 리미터는 이를 못 막는다.
  → `_rate_limit(tr_id)`에 원장 TR(잔고/체결/미체결) 1.05초 간격 강제. 주문 POST
  경로는 미적용(청산 지연 회피). 경고 문구에 `tr_id` + 오류 본문(`msg_cd msg1`)
  포함 — 다음 장애는 로그만으로 호출 주체 식별 가능.
- **P2 모니터링 SQL 오차단** (`monitoring_runner.py`): `-- 주석`으로 시작하는
  체크포인트 SQL 3/12건이 read-only 검사(`^SELECT|WITH`)에 매주 차단되던 문제 →
  선행 주석 줄 제거 후 검사 (`_is_read_only_sql`).
- **P2 pykrx 업종분류 반복 실패** (`sector_momentum.py`): KRX 인증/차단으로
  `get_market_sector_classifications`가 항상 JSON 오류 → 5분마다 재시도하며 pykrx
  내부 stderr 스팸(저녁 배치 시간대 시간당 40건). → 실패 시 6시간 백오프
  (키워드/파일 캐시 매핑은 그대로 동작).
- **P2 종목 마스터 폴백 부재** (`dashboard/data_collector.py`): pykrx 실패 + 캐시
  72h 만료 시 `no valid cache` 경고만 남기던 경로에 FDR `StockListing("KRX")`
  폴백 추가 (`storage/stock_master.py`의 2026-04-21 패턴과 동일).
- **P3 캐시 누적** (`utils/logger.py`): `daily_entry_count_*.json`이 3월분부터
  정리 대상에서 빠져 있던 것 → `cleanup_old_cache` 패턴 추가.
- 회귀 테스트 `tests/test_review_fixes_2026_09.py` 10건 (원장 간격/비원장 무간격,
  SQL 주석 허용·쓰기 차단, pykrx 백오프). 전체 51건 통과.

### 운영 상태 기록 (코드 변경 아님 — 판단 필요)
- **현금 78,238원 (자산의 0.4%)** — 펩트론 087010 120주(`manual`, exit_exempt,
  7/1 진입)가 자산 18.98M의 99.6%. 미실현 **-18.3% (-422만)**, 8/31→9/2 자산
  22.06M→18.98M(-14%). 봇 신규 매수는 7월 이후 구조적으로 불가(8·9월 주문 0건,
  감사 원장 파일 없음). 진화·CF·승격 표본 축적이 모두 이 상태에 묶여 있다.
  코어홀딩 스케줄러는 매일 3회 "빈슬롯 매수 시도(예산 잔여 2.9M)"를 로그하지만
  실제 현금이 없어 항상 0건 — 예산이 equity 기준이라 생기는 잡음.
- 섀도우 승격 현황(실측): 규칙#11 CF 9/20건·r5 56%, 밸류코어 3/8주, 수확 G3
  2/30체결(누적 -3.2R), 팩터 버킷 노출 스냅샷은 포지션 0건이라 전부 0% (8/19의
  "2주 후 재판단"은 판단 불가 → 매수 재개 후로 이월).
- 8/29 토요일 승격 점검·주간 shadow 블록은 저널에 흔적 없음 (8/30 가시성 수정이
  9/2 배포됐으므로 **9/5 토요일 09:30 저널/텔레그램으로 첫 확인**).
- `mcp` 패키지 미설치로 pykrx/naver_search MCP 서버 연결 실패(무시 처리됨),
  KOSPI200 야간선물 -3.49%·실현변동성 45%(×0.55 사이징) 고변동 약세 국면.
...
부검 핵심: 손실은 14개 거래(17%)에 집중, 나머지 83%는 +56.4만 흑자.
최대 원인은 미문서화 상태였던 core_holding 결함(-117만)으로 알려진 두 사고
(-44.8만)보다 컸음. 상세 분석은 아래 각 항목 및 docs 참조.

- **T1** `kr_scheduler.run_core_rebalance_scheduler`: 코어홀딩 빈슬롯 매수
  15:00~15:10 윈도우 제거 — 6/5·6/8 종가매수 3건이 익일 갭하락으로 손절선(10%)
  관통(-15.9~-19.2%, 합계 -772k). 종가 진입 금지, 오전~13:30 윈도우만 사용.
- **T2** `kr_scheduler` 저녁 품질검증 잡: 손절선 초과 방치 워치독 신설 —
  미실현 -12% 미만 포지션 감지 시 WARNING + 텔레그램. 6월 sync_detected 급증
  (5월 3건→13건, 코어 2건 5~13일 방치) 재발 감시. 근본 원인(재시작 등록 누락·
  코어 파라미터 소실)은 8월 초 exit_manager 보강으로 기해소 확인.
- **T3** `cross_validator`: 규칙 3-4(gap_and_go 14:30+ 진입 일 2건 캡) **폐지** —
  부검 실측: 14:30+ 진입 5건은 +138k 흑자, 실제 사고(6/11)의 진입은 이틀 전
  오전. 대체: `kr_scheduler`에 **갭EOD 가드** — 15:10 이후 손실(<0%) 상태인
  gap_and_go 포지션 장마감 전 청산 (손실 오버나잇 금지, 수익 포지션은 허용,
  테마EOD와 동일 패턴/pending 등록 포함).
- **리서치**: `docs/research/ai-trading-research-2026-08.md` 신설 — 2024~26
  AI/LLM 트레이딩 문헌 조사 (실증 유효: 변동성 타게팅·수급/잔차 모멘텀·공시
  텍스트 해석 / 무효: 토론 구조 알파·고회전 센티멘트. 적용 Top 5 + 금지 6항).
- 문서: `docs/strategies/kr-strategies.md`(코어 종가매수 금지·갭EOD),
  `docs/risk/risk-and-exit.md`(워치독·규칙 3-4 폐지).
...
동안은 **아무 것도 오지 않았다**. 사용자 입장에서 "표본이 아직 안 쌓인 것"과
"스캐너가 멈춘 것"이 똑같이 침묵으로 보이는 상태 — 8/15 리포트 미발송이 실제
사례였다(정상 동작이었으나 확인에 로그 조회가 필요했음).

- **`harvest_shadow.weekly_progress(now=None)` 신규**: 항상 요약 문자열 반환.
  누적 체결 수(30 게이트 진행률)·보유·대기·금주 성과·**최근 스캔일 stale 가드**
  (3일 초과 또는 기록 없음/파손 시 ⚠️). 리포트가 항상 오게 되므로 스캐너 정지를
  본문에 명시해야 한다.
- **`kr_scheduler`**: 인라인 집계 30줄 → `weekly_progress()` 호출 3줄로 축소.
  스케줄러가 원장 포맷을 알 이유가 없고, 인라인이면 이번에도 테스트 불가였다.
- 기존 코드가 누적을 계산하지 않으면서 "※ G4 승격 기준: 누적 30체결"을 안내하던
  불일치도 해소 (이제 실제 누적 표시).
- 테스트 3건 추가 (총 12건): 청산 0건 발송 / stale 경고(오래됨·기록 없음) /
  누적↔금주 7일 창 분리 + 깨진 원장 줄 무시.
- 검증: pytest 12건 통과, 실제 상태 파일 렌더링 확인(`진행 0/30체결 · 보유 2 ·
  대기 0 / 금주 청산 없음 / 최근 스캔 2026-08-14`), 재시작 정상(traceback 0).
...
- **`disclosure_feed.py` 신규**: 공개 JSON(무키) 공시 피드에서 최근 2일
  중요도 상위 5건 요약 — 유형 라벨·장구분 포함. 개인 운영 무료 서비스라
  **fail-open** (실패 시 빈 문자열, 브리핑 섹션 생략, 매매 비영향).
- **아침 브리핑(07:30 슬롯)**: 섹터 동향 아래 "최근 공시 (중요도 상위)" 섹션.
  뉴스 큐레이터에 없던 공시 스트림 공백 보완.
- 미채택: 시세(T+1 — KIS 실시간 보유), stock master 폴백(1,463종목 커버 부족),
  MCP(봇 무관). 매매 판단 경로 연결 금지 조건 문서화 (external-apis.md).
- 검증: 실 엔드포인트 셀프체크 (실데이터 5건 포맷 확인), py_compile, 재시작 정상.
...
- 정량 축은 정확 — KODEX 반도체(091160) 실측 20일 -26.39% (146,380→107,750원,
  KIS 일봉 재계산 일치). 괴리의 1차 원인은 설계상 미장 신호 미입력 (KR 전용 점수).
- **결함**: 정성 축(Perplexity)이 프롬프트의 "주가 등락 채점 금지" 규칙을 무시하고
  급락 자체를 -70으로 채점 → 정량 -100과 **이중 계산**되어 -88로 증폭
  (재료 기준이면 -60대 중반). 철강·IT·증권 reason도 가격 서술로 동일 위반.
- **수정 (A안, 사용자 선택)**: `_fetch_qual_signals` 파싱 시 정성 점수를
  ±100 → **±50 클램프** — 최종 영향 폭 ±20(×0.4)으로 제한. LLM이 규칙을
  어겨도 이중 계산 피해가 절반 이하.
- 기각 대안: B) 프롬프트 few-shot 보강 (LLM 준수 불확실), C) 현행 유지.
...
(사용자 승인 "전체를 순서대로 진행해"). 커밋: 1단계 7dfea88, 2단계 6fd5485, 3단계 본 커밋.

### 1단계 — 이중 매도 계열 (exit_manager, P0+P1 3)
- pending_stage/목표·체결수량 **영속화+복원** — 재시작 직후 거래소에 살아있는 주문
  위에 재발행하던 P0 (기존 주석이 "자동 재발행 보장"이라 명시하던 설계 결함)
- on_fill **부분 체결 누적** — 1주 체결로 단계 승격되던 문제, 목표수량 도달 시에만 승격
- 5분 만료를 **거래소 SELL 미체결 확인 후에만 해제** (`maybe_expire_pending` +
  run_trader 검증자 배선, 판단 불가 시 유지 fail-safe, verifier 없으면 레거시 유지)
- 재시작 정합성: 단계별 누적 예상 잔고로 "가장 깊은 확실한 단계" 강등 (기존: 1차만
  검사 + 무조건 NONE 리셋 → TP1 재발행 위험). 오프라인 시뮬레이션 5종 통과.

### 2단계 — 수익 로직·엔진 P1 10건
- REGIME neutral 익절 12/20 → **15/25** (문서·ExitConfig 계약 정렬), ATR max_stop 7→8
- SEPA 목표가 ×1.5 → **×2.0** (min_rr=2.0 게이트 정합 — ATR 2~3.5% 후보 전멸 해소)
- 스크리너: 첫 소스에도 가중치 적용 (호출 순서 의존 제거), min-max 정규화는
  **최고점 100 초과 시만** (저품질 장에서 1등을 100점 승격시켜 절대 게이트 무력화하던 문제)
- 안전자산(KOFR) 매수 `can_open_position` 게이트 경유 (기존: 리스크 검증 2종 우회)
- 전문가 브리핑 실패 시 3회 재시도 후에만 슬롯 완료 처리 (fail-closed, 8/5 사고 패턴)
- 야간선물 **night(CM) 세션만** 오버나이트 신호로 사용 — F(주간) 폴백이 정규장 중
  "밤사이 신호"로 오용되던 구멍 (스크리닝 + kr_market/weekend 전문가 3곳)
- 섹터 동기 조회 미스 메모이즈 + ETF 전체 실패 2분 네거티브 캐시 (API 폭주 방지)
- 위키 리서치 페이지 카운터 메모리화 (신규 페이지마다 200파일 동기 순회 제거)

### 3단계 — P2 7건 반영 + 2건 반박
- Gap&Go ORB 돌파 보너스: 갱신 전 고점 보존 — `price > price` 비교로 **한 번도
  발동한 적 없던** 보너스 10점 복구
- 최대 보유일 `>` → `>=` (N+1일째 발동하던 오프바이원)
- 실현 일일손익에 매도분 귀속 **매수 수수료 차감** (일일손실 게이트 낙관 보정)
- R/R 게이트 **순비용 기준** (왕복 ~0.227% 반영 — gross 2.0 경계 과대평가 보정,
  US는 zero-commission이라 영향 없음)
- LLM 폴백 호출 계수 추가 (적대검증 실패 후 폴백 미계수), fire-and-forget 태스크
  예외 관찰 콜백 (orchestrator 2곳 + engine 좀비 경보)
- **반박 2건**: PER/PBR 0→None은 의도된 결측 처리 (주석 명시), 정합성 검증의
  0-falsy 지적은 해당 패턴 아님 확인.
...
(전략/섹터/체제 3차원)에 `wiki/symbols/<종목코드>.md` 4번째 차원 추가.

- **거래 교훈 축**: 매도 ingest 시 종목 페이지에 거래 이력 테이블(전략·수익률·청산유형·
  보유일·체제) + LLM 교훈 기록. 파일명은 종목코드 고정(이름은 frontmatter) —
  이름 미상 시 이중 생성 방지.
- **리서치 노트 축**: `note_research()` — orchestrator가 전문가 9명(뉴스 큐레이터·
  어닝·섹터카운슬 포함)의 `affected_symbols`(전문가당 최대 5종목)를 fire-and-forget
  기록. 같은 날 같은 출처 dedup, 30개 롤링, 리서치 전용 신규 페이지 상한 200
  (거래 페이지는 무제한·상한 계수 제외).
- **매수 검증 주입**: cross_validator LLM 2차 검증 프롬프트에 `query_symbol()`
  (거래 통계 + 교훈 3개 + 최근 리서치 5개, 600자) "종목 노트" 추가 — 과거 교훈과
  최근 리서치가 매수 판단에 직접 반영.
- **크기 관리**: 페이지 200줄 제한 동일, 토요일 Lint가 180일 미갱신 종목 페이지를
  `symbols/archive/`로 이동 (락 보유), 인덱스에 종목 카테고리 추가.
- **배선**: run_trader에서 엔진 TradeWiki 단일 인스턴스를 orchestrator에 공유
  (이중 인스턴스 시 asyncio.Lock 무력화 방지).
- **코드리뷰 (P0 0·P1 2·P2 6 → P1 전량+P2 4건 수정)**: ① 페이지 상한이 거래
  페이지까지 계수해 장기적으로 리서치 생성이 영구 차단되던 결함 → 리서치 전용
  (trade_count=0)만 계수. ② LLM 자유 텍스트의 내부 개행이 마크다운 행 구조를 깨고
  롤링 트림을 우회하던 결함 → 개행 정규화 (기존 `_extract_lesson`도 동일 수정).
  ③ lint 아카이브 rename을 락 보유로 (ingest와 TOCTOU 경합 방지). ④ dedup 마커에
  연도 포함. ⑤ `_append_to_section` 데드코드 제거. ⑥ 드라이브바이: 기존 `query()`가
  "최근 3개 교훈" 주석과 달리 가장 오래된 3개를 집던 정렬 버그 수정.
  보류 P2 2건: ingest 락이 LLM 호출 포함(지연 스멜), query 무락 sync 읽기(기존 패턴).
- 검증: 오프라인 기능 테스트 12항목 (생성·dedup·개행 방어·상한 계수·정렬·인덱스·
  lint) + 재시작 후 "[전문가] 종목 위키 리서치 노트 배선 완료" 확인.
