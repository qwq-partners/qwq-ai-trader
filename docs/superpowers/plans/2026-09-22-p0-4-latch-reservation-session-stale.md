...
- 보유도 같은 commit 에서 줄어든다(`economics.py:437`) → **`held − reserved` 는 음수가 되지 않는다**(100/40 이면 60−60=0). 결정 문서 §4-2 항목 4 의 "`reserved_quantity` 는 terminal 전까지 줄지 않는다 / `held − reserved < 0`" 은 **틀렸다**. 결론(잔여 60 재매도 불가)만 맞다.
