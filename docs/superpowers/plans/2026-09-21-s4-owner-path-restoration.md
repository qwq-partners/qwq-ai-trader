...
| ③ | LIMIT SELL 지정가의 dispatch 직전 재확인 | **하지 않는다**(설계의 S4-3 기각). 원장 "S4 진입 조건 4" 는 "재확인하지 않는다"로 닫는다 | 심사 must-fix 1 — "불리한 방향만 막는다"는 안전장치가 곧 손절 방향이다. 하락장에서 호가가 한 틱만 내려가도 보호 SELL 이 체계적으로 거부되고, ①로 에스컬레이션도 없으니 손절이 거래소에 닿을 길이 없어진다(S3 현재보다 나빠진다). 또 가격 판정을 gateway 에 새로 만드는 것은 S3 계약 1(owner 판정 복제 금지)과 어긋나고, prepare~dispatch 사이에 새 network await 를 넣어 wave 3 의 P1(세션 경계 누수) 창을 넓힌다. 지정가가 안 붙는 문제의 해법은 에스컬레이션이고 그것은 ①의 전제 뒤다. **범위 한정(S5 Codex 5차-B):** 이 결정은 "**거부형** 가격 재검사를 도입하지 않는 절충"이다 — 가격 위험을 닫은 것이 아니라 보호 SELL 의 송신 기회를 지킨 것이다. 취소 최종성의 부재가 모든 가격 개선을 막는 것도 아니다(최초 송신 전의 가격 선택을 개선하는 설계에는 기존 주문의 취소 최종성이 필요 없다 — 별도 설계 과제). 현재 SELL 은 호가 조회가 **정상 성공했어도** 판단→final 사이에 내려간 호가를 다시 보지 않고, 미체결이어도 에스컬레이션이 없다(인계 문서 차단 사유 10) |
