...
4. checkpoint 읽기: **store 파일이 없으면 만들지 않고** `startup_checkpoint_required`(`store.load()` 는 없는 파일을 생성·초기화한다 — 읽기가 아니다). 있으면 `store.load()` → 필수 root 집합이 없으면 같은 사유. **store 의 수명은 호출자 소유다 — 설치기는 닫지 않고, _호출자_ 가 성공·거부·실패 모든 경로에서 `await store.close()` 한다**(정정: 초안 문장은 "설치기가 닫는다"로 읽혔다 — 구간 1 거부 뒤 legacy 로 계속 가는 호출자의 store 를 설치기가 닫으면 안 된다). 그래서 "구간 1 은 아무것도 바꾸지 않는다"의 정확한 뜻은 **기존 store 의 내용을 바꾸지 않는다**이다 — `load()` 이후의 거부에서는 sqlite 연결이 열린 채 남고 `-wal`/`-shm` 파일이 생긴다(독립 재현이 바이트 단위로 실측: db 의 size·mtime·sha256 불변).
...
최초 checkpoint·레짐 baseline·일자 전환을 만드는 앞 단계(공식 증거 뒤) · 추세 출처 갱신(설치 뒤 첫 prepare 가 `regime_source_not_current` 로 막힐 수 있다 — 갱신 주체는 스케줄러 배선, 10C). **구현·재현이 함께 찾은 제품 사실(수정하지 않음 — 10C 배선의 계약):** 성공한 추세 갱신은 VIX 가 없거나 6시간이 지났으면 VIX 갱신을 **뒤에** 건다(`regime_owner.py` `refresh_trend` 끝의 `_schedule_vix`). 그 VIX 행이 commit 되면 방금 수락된 추세 판단의 input seal(읽기 사실에 `vix_regime` lane 이 들어 있다)이 `source_read_changed` 로 stale 이 되어 **다음 추세 갱신까지 모든 prepare 가 `regime_source_not_current` 로 막힌다**(fail-closed). 2분 루프가 돌고 있으면 다음 주기에 VIX 가 신선해 다시 current 가 될 것으로 보이나(추정 — 미검증), 루프가 없으면 풀리지 않는다. 10C 는 "VIX 갱신 뒤 추세를 즉시 다시 평가한다"(또는 VIX → 추세 순서)를 계약으로 넣어야 한다. 시험 하네스는 seed 에서 VIX 를 먼저 수락시켜 이 순서를 피했다 · 진입 시세의 실제 source 요구(차단 사유 6) · 시계 주입·관측·사이징 표(§3-1 에서 미룸) · 런타임 면제 추가의 처분(10C) · 계좌 lease 배선 · 실패 뒤 해체 절차(`runtime.shutdown()` 은 owner 가 blocked 면 다시 실패할 수 있다 — 호출자는 store 를 닫고 프로세스를 세운다).
