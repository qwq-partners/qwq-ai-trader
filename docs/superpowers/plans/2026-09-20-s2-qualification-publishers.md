...
19. **trade_memory 출처는 CV 가 메모리에 실제로 넘긴 섹터로 귀속한다(S2 마감 수정, Codex S2 재리뷰 P2).** 규칙9 는 신호 metadata 의 섹터(`sector or ""` — 빈 문자열일 수 있다)로 `get_score_adjustment` 를 부르는데, 증거의 `sector` 는 그 뒤 `_sector_lookup` 이 돌려준 값이다. 두 값이 다르면 출처가 CV 가 쓰지 않은 섹터에 달리고 서로 다른 소비 범위가 같은 행을 덮는다(행동 RED 로 재현: 빈 섹터 −7 과 '반도체' −3 이 같은 이름을 공유). 그래서 CV 가 `last_decision['memory_sector']`(**13번째 키**, 보정이 실제로 붙었을 때만 넘긴 값 그대로·아니면 None)로 보고하고, builder 는 그 값으로 이름(`''` → `'-'`)과 digest 를 만든다. `memory_adj != 0` 인데 str 이 아니거나, `memory_adj == 0` 인데 None 이 아니면 `unexpected_cv_decision`. `facts.sector` 는 계속 조회 후 섹터(섹터 한도 판정용)이며 출처 이름과 분리됐다 — 그래서 `facts.sector` 의 ':' 는 더 이상 `invalid_source_scope` 로 막히지 않고 DTO 계약(`invalid_decision_sector`: 빈 값·공백)만 남는다.
...
- 고정 인터페이스: `validate(symbol, side, strategy, score, metadata, market_regime='neutral', count_stats=True, request_token: str | None = None) -> tuple[bool, float, str]`(새 kwarg 는 맨 뒤·기본 None) · `last_decision: dict | None`(진입부 무조건 None, 통과 return 직전 1곳에서만 대입; 키 `token, symbol, side, strategy, regime, penalties(tuple), cap_applied, original, adjusted, memory_adj, memory_sector(None|str — 계약 19, S2 마감 수정에서 추가된 13번째 키), panel(None|{created_at, conviction, bonus, loaded_at}), now_hm`) · `last_llm_reason: str | None`(계약 9 의 어휘)
...
- **trade_memory digest** 는 `{strategy, sector, memory_adj}` 다 — 여기서 `sector` 는 계약 19 의 `memory_sector`(CV 가 메모리에 넘긴 값, 미상은 `''`)이지 조회 후 섹터가 아니다(계획 표의 `rule·score_delta` 는 S2-1 의 `last_decision` 에 없고 CV 수정이 금지라 얻을 수 없다 — replay 구속 전용이라 실효성 손실 없음).
