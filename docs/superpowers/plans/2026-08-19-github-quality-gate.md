...
- [x] `main` PR/push와 수동 실행 trigger를 정의한다.
- [x] 읽기 전용 권한, concurrency, 15분 timeout을 정의한다.
- [x] `actions/checkout@v7`과 `actions/setup-python@v7`으로 Python 3.12와 pip cache를 준비한다.
- [x] `pip install -r requirements.txt` 뒤 `bash scripts/dev/verify.sh`를 실행한다.
- [x] YAML 구조와 로컬 검증을 실행한다.
- [x] `git commit -m "ci: PR 검증 workflow 추가"`로 커밋한다.
...
- [x] workflow trigger, check 이름, 실패 확인법과 재실행 절차를 문서화한다.
- [x] GitHub UI와 API의 `main` 보호 설정값을 문서화한다.
- [x] 문서 인덱스와 변경 이력을 갱신한다.
- [x] `bash scripts/dev/verify.sh`를 실행한다.
- [x] `git commit -m "docs: GitHub 품질 관문 운영 절차 추가"`로 커밋한다.

### Task 3: Remote Verification and Protection

- [x] feature 브랜치를 push한다.
- [x] `main` 대상 PR을 생성한다.
- [x] `gh pr checks --watch`로 `verify` 성공을 확인한다.
- [x] `main` 보호 API에 PR 필수, `verify` 필수, strict 최신화, 대화 해결, force push·삭제 금지를 적용한다.
- [x] 보호 설정을 다시 조회해 실제 값을 검증한다.
- [x] 권한 부족 시 PR과 브랜치를 유지하고 수동 설정 절차를 인계한다.
