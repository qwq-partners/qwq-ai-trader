...
- [ ] main afa6e1e의5파일을 같은 내용으로 격리 test-only worktree에 추가한다.
  `git show afa6e1e:tests/test_kis_tr_switch.py`로 읽고 apply_patch로 추가한다.
- [ ] 기존 legacy stale26·session5를 실행해 기준을 확인한다.
- [ ] 새5파일 실행. 수집 오류면 `--continue-on-collection-errors`로 가능한 행동도 측정한다.
...
- [ ] assertion/xfail 무변경·main blob 동일성을 검증하고 RED임을 명시해 test-only commit한다.
...
- [ ] coordinator 전용 통합 worktree에서 검토된 관측 후보를 no-commit merge한다.
  문서 충돌은 양쪽 이력을 보존한다. `_attached_gateway`와 `_SellKeep`는 모두 필요하다.
- [ ] KIS는 QueryResponse/result wrapper와 획득별 lease를 보존하면서 main TR/페이지 및
  관측 래퍼를 결합한다. 실제 첫 페이지/다음 페이지 헤더 시험을 먼저 읽는다.
- [ ] attached heartbeat 회귀를 RED로 추가한다. 실제 engine/owner fixture에 legacy
...
- [ ] closing auction 취소 금지/면제 재송신 금지2개 구 기대를 교체한다. cancel0 BUY는
  tracked-live/undecidable/vanished/구 호환 fake로 분리한다. 나머지 기대는 근거 없이 바꾸지 않는다.
- [ ] Task1 main5파일 + KIS execution query/integration/evidence + owner/gateway/P1/lease/install
  시험을 실행한다. Expected: main 행동과 attached writer/sender 격리 모두 GREEN.
- [ ] diff·시험·26개 처분표를 검토하고 통합 커밋한다. 미해결이면 merge 완료로 보고하지 않는다.

## Task3: 수동 복구 진단 절차

File: `docs/operations/protection-recovery-diagnostics.md`.

- [ ] 모드(legacy/attached)·code SHA·owner/published/engine version·health·미적용 inbox를
  읽는 단계와 증거 없는 판단을 명확히 분리한다. legacy에 owner가 없으면 N/A로 표기한다.
- [ ] 원장/감사/의도/예약/입력 source/RAM 연결의 분류표와 필요한 추가 증거를 작성한다.
- [ ] `_audit_recovery` 등 변경 함수를 읽기 전용으로 부르지 않는지 코드와 대조한다.
- [ ] 두 안정 스냅샷과 별도 승인된 브로커 증거 없이는 어떠한 수동 변경도 제시하지 않는다.
  도구가 아직 배선되지 않은 항목은 미지원으로 표시하고 운영 실행 명령을 발명하지 않는다.
- [ ] 독립 리뷰에 절차의 파괴·권한 확대·증거 추정 가능성을 함께 검토시킨다.

## Task4: See

- [ ] native critical + tools0 Opus 교차 리뷰. 실제 모델·비용·차단·범위를 기록한다.
- [ ] 모든 worker 종료 후 env-i 격리에서 UTC→KST 전체 `pytest tests -q -p no:cacheprovider --tb=short`.
...
- [ ] 문법·비밀패턴·git diff --check. 전체 통과 후 feature 문서·커밋/푸시.
- [ ] main/운영 전환과 C/F/G/R 미완을 별도로 보고한다. 진단 구현/배포를 과장하지 않는다.
