...
- **허용 hunk:**
  - **H2** `_reserved_cash` property(1468-1471)와 `_pending_strategy_notional`(2426-2432): attach + gateway 설치 시 각각 `gateway.reserved_cash()`·`gateway.pending_strategy_notional(strategy_name)` 를 돌려준다. `_calculate_position_size` 본문·소비 지점 5곳은 불변.
