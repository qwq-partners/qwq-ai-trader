...
- **현재 변경은 문서뿐이다.** 이 설계는 실자료 수집 승인서가 아니다. 인증 API·실토큰·운영 캐시·배포·재시작·주문·설정 변경 권한을 부여하지 않는다.
...
확인할 핵심은 **#67 보존, 기본 OFF, 실제 승인값 미발행, 추가 KIS 조회 0, Toss 전용 worker 격리, 불완전 자료는 비교 제외, 일봉/소비자 승격 별도**다. 이 설계 확인 후에만 상세 구현 계획과 런타임 수정을 시작한다. 설계 문서 작성·테스트 기준선 통과는 위 R01~R14의 구현 완료 증거가 아니다.
