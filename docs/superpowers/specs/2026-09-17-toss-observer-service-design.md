...
**상세 설계 검토용이며 아직 구현·설치·활성화하지 않았다.** 현재 Toss OFF와 관측 서비스 부재를 ON 완료로 보고하지 않는다. 이전 배포의 실행 코드/PID는 [운영 기록](../../reviews/toss-pr-integration-2026-09-17.md)으로 구분한다. 이 문서에서 명시하는 수치·권한·기간은 이번 제한 관측을 위한 제안 정책이지 기존 운영값이나 공급자 권장값이 아니다.
...
이번 문서만 저장·커밋·푸시한다. 상세 설계를 사용자가 확인하면 파일/API 소유권과 인수 테스트를 분리한 실행 계획을 작성한다. 같은 경로의 열린 PR/main을 착수 전과 병렬 작업 종료 후 확인한다.

### Do — 역할별 병렬 구현, 접점은 부모 통합

- A 보안·launcher/승인·시작 영수증: **Astra/high**. 토큰/HTTP 정본을 보존하며 설치 권한 경계를 담당한다.
- B 입력 adapter·관측 서비스·상태·테스트: **Terra/high**. 인증 파일을 수정하지 않고 확정된 Deployment/입력 계약을 소비한다.
- 부모: 계획·의존성 packaging/설치·문서·통합·운영 실행. 같은 파일 동시 편집을 금지한다.
