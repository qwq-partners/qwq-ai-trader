...
**사용자가 09-17 후속 `ㄱㄱ`로 상세 설계 및 구현·검증 후 별도 서비스 ON을 승인했고, 이어서 관측일 09/18·21·22 및 09/22 18:00 KST 만료를 명시 확정했다.** [실행 계획](../plans/2026-09-17-toss-observer-service.md)의 실제 설치/ON 단계로 진행한다. 실제 실행 결과는 [관측 서비스 원장](../../reviews/toss-observer-service-2026-09-17.md), 기존 거래 봇 배포 이력은 [이전 운영 기록](../../reviews/toss-pr-integration-2026-09-17.md)으로 구분한다. 아래 수치·권한은 이번 제한 관측 정책이지 기존 운영값이나 공급자 권장값이 아니다. 일정 변경은 사용자 승인에 따른 것이며 추후 자동 연장하지 않는다.
...
| 항목 | 값 |
|---|---|
| plan/dataset | `toss-observer-holdings-20260918-v1` / `live` |
| 예정 날짜 | `2026-09-18`, `2026-09-21`, `2026-09-22` |
| 가격 창·주기 | KST `regular` 09:00 이상 15:20 미만, 5분 정각 격자(일 76슬롯) |
| 캘린더 | 각 예정일 08:55, 현재가와 별개; 지연해도 해당일만 실행 |
| grant 기간 | 설치·최초 시작 직전 확정한 UTC not_before부터 2026-09-22 18:00 KST 미만, 최대 7일 |
...
## 9. 설계 준비 당시 검증 기록 (이후 실제 ON 결과는 운영 원장 참조)
