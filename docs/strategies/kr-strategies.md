...
- **갭EOD 손실 오버나잇 가드 (2026-08-19~)**: 15:10 이후 미실현손익 < 0%인
  gap_and_go 포지션은 장마감 전 청산 (`kr_scheduler`, source=gap_eod).
  6월 부검 근거 — 6/11 오버나잇 사고(-175k)의 원인은 진입 시각이 아니라
  손실 상태로 밤을 넘긴 것 (갭하락은 장중 스톱을 관통). 수익 포지션은
  오버나잇 허용. 기존 규칙 3-4(14:30+ 진입 캡)는 과녁 오류로 폐지
  (14:30+ 진입 5건 +138k 흑자 실측).
...
- ATR 동적 손절 비활성 (고정 SL)
- **종가매수 금지 (2026-08-19~)**: 빈슬롯 매수 윈도우에서 15:00~15:10 제거
  (`kr_scheduler.run_core_rebalance_scheduler` fill_windows).
  6월 부검 근거 — 6/5·6/8 15:00 종가매수 3건이 익일 갭하락으로 명목 손절선
  (10%)을 관통해 -15.9%~-19.2% 청산 (합계 -772k, 6월 최대 단일 손실원).
  종가 진입은 장중 검증 시간 없이 오버나잇 갭에 전면 노출되므로 오전~13:30
  윈도우에서만 매수.
...
## 전략 배분 (2026-09-13 종합 리뷰 반영)

| 전략 | 배분 | 근거 (실거래 3~7월, 포지션 원장 정본) |
|---|---|---|
| sepa_trend | 40% | 백테·실거래 데이터가 있는 유일한 라인 (기대값 ≈ 0 — 청산 정책 A/B 대상) |
