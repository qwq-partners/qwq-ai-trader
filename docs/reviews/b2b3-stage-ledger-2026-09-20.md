...
현재 P1 후속 단계는 [P1 S2~S5 결과 원장](p1-producer-wiring-2026-09-23.md)과
[후속 실행 계획](../superpowers/plans/2026-09-23-p1-producer-wiring.md)이 정본이다.
아래 표와 말미 S1·S1′의 '정책/예산 답변 대기'는 당시 이력이다. 이후 개발 순서는
[다음 작업 인계](../operations/p1-next-steps-2026-09-23.md)에 분리한다.
...
- **P0-3 의 방법론 기록:** ① 독립 재현이 둘 다 CHANGES_REQUIRED 였고 그중 하나(S-D 전면 skip 이 보호 청산 구동기를 전부 없앤다)는 **설계(Q-3)가 놓친 결과**였다 — live writer 를 막는 결정은 그 writer 가 겸하던 emit 까지 같이 사라지는지 반드시 묻는다. ② 처분 구현자가 처분의 전제를 코드로 반증하고(무기한 `Event.wait()`) 더 큰 변경을 택한 것은 옳았다 — 처분은 명령이 아니라 가설이다. ③ Codex 의 "미배선 통과" P1 은 기각했다: 근거는 "유일한 제품 경로가 강제한다 + 술어에 걸면 176건 회귀" 이고 16차가 경로 부재를 확인했다 — 기각은 코드 사실로만 한다. ④ cross_validator 의 시계 창은 09:00~10:30·12:30~13:00 **프로세스 지역시각**이다(UTC 전체 suite 는 18:00~19:30 KST 를 피한다). **(2026-09-22 해소 — 시험만, 제품 0줄)** 원인은 시계의 한 축만 동결한 것이었다: `engine_clock` 은 `src.core.engine` 모듈의 `datetime` 만 밀고, CV 시간 가드는 `validate` 안에서 다시 import 한 시계를 읽는다. 두 축을 묶어야 하는데 `freeze`(`test_cross_validator_characterization`)는 **팩토리 fixture** 라 인자로 받기만 하고 부르지 않으면 아무것도 동결되지 않는다 — 유령 RED 4건은 전부 `freeze` 를 시그니처에 달고 한 번도 부르지 않았다. 노출된 생성 지점 3곳(`eviction.legacy()`·parity 섹터 폴백·wiring legacy 축출)에서 기존 헬퍼를 `ENGINE_NOW` 와 같은 순간으로 부르게 고쳤고(새 헬퍼 0), 쌍 불변식은 공유 지점 `engine_clock` docstring 에 적었다. 검증은 `TZ` 로 프로세스 지역시각을 5개 버킷(08:45·09:15·09:45·12:15·12:45)에 고정해 수정 전 재현(09:15 4 failed·09:45 3 failed) → 수정 후 전 버킷 green. **보고와 달랐던 것:** 네 번째는 `test_execution_p03_wiring.py` 가 아니라 `test_execution_signal_gateway_wiring.py::test_eviction_still_runs_on_the_legacy_path` 였다(p03 은 5개 버킷 전부 18 passed 로 시계 무관). `test_engine_legacy_stale_eviction_characterization.py` 도 `engine_clock` 을 단독으로 쓰지만 5개 버킷 전부 26 passed 라 손대지 않았다 — 유령 RED 는 CV 가드에 닿는 legacy BUY 시험에만 붙는다. **회피 지침 자체는 남긴다** — 확인한 것은 이 4파일(+특성화 1파일)뿐이고 전체 suite 를 시계 창 안에서 쓸어 보지는 않았다(다른 세션이 호스트에서 전체 suite 를 돌고 있어 "단독 직렬" 규칙상 미실행). 중립 버킷 전체 suite 는 **5116 passed / 2 xfailed / 경고 4**(423초, P0-4 기준선과 동일).
- **P0-4 완료(기준 `9c36068` → merge `90cfc20` → 처분 `6eaee68` → `a4f8585`, 제품 변경 2곳·legacy 바이트 동일):** Plan(조사 2 → 설계 → 심사 2관점 **REVISE ×2**: 설계 초안의 래치 재개가 게이트 개방 — 아무도 받지 않는 보호 결정 커밋·낡은 진입 증거로 BUY 개방·교차 종목 fail-open·결정적 실패 미치료 → 폐기, S-A 는 admit 사전 거부 면제만) → Do 구현 3 ∥ 독립 재현 3(S-A CHANGES_REQUIRED P1 1: 래치 단독의 SUBMIT 게이트 미고정 / S-B·S-C CHANGES_REQUIRED P2 3: 인용 오기·side 무관 잠금 미고정·주석 반사실 / S-D APPROVE P2 2: 고아 블록 미경유·S5 docstring) → 처분 전부 제품 0줄, 생존 변이 3종(래치 항 제거·side 인지 잠금·가드를 첫 루프에만) 이제 kill → **Codex 17차 APPROVE**(P2 docstring 1). 전체 **UTC 5116 / KST 5116 passed**·격리 0. 차단 사유 **22·23 등록**. 상세 계획서 §5. 다음은 P1(보호 SELL 의 main 동등 — 한 설계로: owner quote 생산자·취소 3분류·F8 ①·재준비 소유자·래치 해제).
- **P0-4 의 방법론 기록:** ① "해제 경로를 만든다"는 목표 자체가 심사에서 뒤집혔다 — 닫힌 게이트를 여는 변경은 그 게이트가 막고 있던 결함(소비자 없는 결정·낡은 증거)이 무엇인지 먼저 묻는다; 목표를 좁혀 마감하고 나머지는 차단 사유로 적는 것이 "해제 경로가 생겼다"고 쓰는 것보다 낫다. ② 세 재현 중 둘이 "제품은 옳은데 **시험이 불변식을 안 잡는다**"(래치 단독 게이트·side 무관 잠금·고아 블록)였다 — 계약 고정 단계의 인수는 "이 변경이 없으면 죽는가"가 아니라 "**이 변경이 지키는 불변식을 지우면 죽는가**"로 쓴다. ③ 인용 오기(`:437`)가 조사 → 계획서 → 결정 문서로 전파됐다 — 인용을 옮길 때는 옮기는 사람이 줄을 다시 연다. ④ 3 작업자 병렬은 pytest 동시 ≤2 규칙을 넘었다(구현 3 이 각자 파일 묶음을 돌림) — 부하 플레이크는 없었으나 다음부터 구현 단계는 2+1 로 나눈다.
- **P1 Plan 완료·Do 대기(2026-09-22 밤, 제품 0줄):** 조사 3(main 기준선 28사실·owner 부품 44·생산자 입력 33) → 설계(시장가 보호 SELL·취소 없음·MARKET_DATA 핸들러 생산자·에피소드 intent·pending 해제·재개) → 심사 REVISE ×2(P0 6·P1 7·P2 3) → 처분 확정(계획서 §3). **Do 는 §4-1(시장가 전환) 사용자 승인 뒤** — 단계 S1‖S1'(runtime·gateway 부품) → S2(생산자) → S3(engine.py) → S4(factory) → S5(주석). 새 차단 사유 후보 24(MARKET_DATA 틱마다 전략 평가 사망)·25(두 번째 보호 SELL 영구 불가)·26(시간외 보호 SELL 부재)·27(선제 stale 결정 소비자 0). 방법론: 설계가 "더 안전"이라고 적은 항목 2개(#2 지표·#17 면제)가 심사에서 **존재하지 않는 훅·미기록 참조**로 드러났다 — "더 안전" 판정은 그 경로의 생산자·writer 를 코드로 찾은 뒤에만 적는다.
...
## P1 S1·S1′ — 보호 기반 부품 (2026-09-23)

- **Plan:** 사용자 "전체 진행해"로 P1 개발 범위를 이어받았다. 4-1 미답이면 S2부터 미착수라는 인계 게이트는 유지한다. 사전 충돌5건과 실행 중 반례는 [P1 계획 §6-1](../superpowers/plans/2026-09-22-p1-protective-sell-parity.md)의 P1-A~G/P2-E로 처분했다. 원 source 무효화·실패 command/generation 귀속·순수 계산과 저장 실패 분리·한 행씩 결정 인계·관측 체결이 있는 pending 보존을 고정했다.
- **Do:** 같은 `6ee6e20` base의 격리 worktree에서 S1 runtime(`a3f522b`→`af6c5e3`)과 S1′ gateway(`8b2b7fa`→`7a8dff0`)를 병렬 구현했다. 제품2파일·신규3시험파일67개이며 기존 회귀 파일/legacy 본문은 변경하지 않았다. 요청 Astra/high. 통합은 coordinator만 수행한다.
- **See:** 별도 Astra/xhigh가 S1′에서 캐시 spy의 `in/[]` 누락 변이 생존을 찾아 시험 보강, S1에서 실제 FINAL_FILLED/observed10/applied0→pending 해제→실큐 적용 후 first 단계 유실을 찾아 제품 수정했다. 두 후보 모두 수정 후 한정 승인이다. S1 작성자5+독립3 변이와 추가 관측 guard, S1′ 작성자3+독립3 변이를 검증했다. S1 관련243·S1′ 관련42 GREEN, 실제모델/effort는 metadata 미노출로 미검증이다. **UTC/KST 각 5183 passed·기존 2 xfailed·경고4·격리 위반0, 종료0. 개발 통합 `77a3641`.**
- **보증 한계:** 외부 Claude 예산 미답으로 교차 공급자 리뷰 미실행. 인계 §10의 부품 축소 보증 허용만 적용했으며 S3/S4 통합의 교차 공급자 게이트를 완화하지 않는다. 정상 quote와 재개 계산의 흐름 복제는 비차단 P3 관리 항목이다. 원문 민감행 필터 누락의 절차오류도 값 없이 [상세 보고](p1-components-2026-09-23.md)에 기록했다.
- **다음 체크리스트:** ① 정책4-1~4-4와 외부 리뷰 예산 답변 ② S2 sweep이 한 재개 결과를 원 intent로 보관한 후 다음 호출, 공개 sweep/on_market_data 공유 lock·shield·종료 drain ③ S3 engine 한 파일·S4 factory 한 파일씩 legacy 불변/교차 리뷰/전체 UTC·KST ④ S5 주석·범위 통합리뷰. 차단23 잔여·24~27 및 기존 설치 전제는 열려 있다. main/운영 전환은 별도다.
