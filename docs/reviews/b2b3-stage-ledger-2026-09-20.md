...
- **이어받는 에이전트 체크리스트:** ① `gh pr view 88` — 병합됐으면 engine 에 main 을 들일 때 S4-0 특성화 26건의 "현행 결함 그대로" 단언이 #81·#84 때문에 뒤집힌다(결정으로 기록) ② 결정 문서의 인용 중 "coordinator 확인"이 없는 것은 에이전트 보고다 — 구현 전에 그 줄을 다시 읽는다 ③ KIS 의 답을 기다리는 것을 다음 행동으로 제시하지 않는다.
