...
| 단계 | 구현 범위 | 검증/통합 상태 |
|---|---|---|
| S2 | 신규 보호 생산자와 실제 owner/store 시험 | 후보 `8a539b4`, 작성자54/관련267건. 독립 재현 Important4건 확인, 수정 중·통합 전 |
| S3 | engine attach 가격 writer·매도 수량/유형 | 후보 `0bb1d43`, 작성자32+37+26건. await 뒤 휴장 재검사 누락1건 수정 중; 통합은 S2 다음 |
...
- S2: 이전 부분10주 체결 뒤 보류 full100이 잔량90 보호를 영구 차단함.
  충돌하는 새 EOD 결정 생성 시점을 막고, 원결정 수량을 임의 보정하지 않는다.
