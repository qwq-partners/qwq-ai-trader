...
2. **신선도 가중치 감쇠** — `AnalystReport.freshness_decayed_confidence()`가
   반감기 60분으로 지수 감쇠시킨다. 실측: 30분 0.57배, 2시간 0.25배, 6시간 0.016배.
3. **프롬프트에 나이 명시** — 토론 컨텍스트에 각 근거의 나이를 붙이고
   "오래된 근거는 할인해서 판단하라"고 지시한다. 나이를 감추면 모델은 전부 현재로 취급한다.
4. **스크리닝 시각 전달** — `bot._last_screened_at`을 `indicators_as_of`로 넘겨
   지표 나이를 실측한다.
...
- [ ] **포트폴리오 단위 allocator** — 현재는 종목을 독립 심의한다. 동시 승인 건들의
      상관·집중을 한 번에 최적화하는 계층이 없다. 실주문 연결 전 필수.
