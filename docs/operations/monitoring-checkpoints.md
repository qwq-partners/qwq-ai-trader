...
### 배포 후 (매수 재개 시점부터 유효) — 분할 매도 취소 실패 처리 (2026-09-21 수정, 미배포)
...
- [ ] `journalctl -u qwq-ai-trader | grep -E "매도 취소 실패 가능|취소 실패 가능 — 원 주문 생존 가능"` — 발생 빈도. 대부분 다음 체결 확인에서 FillEvent 로 끝나야 한다(뒤따르는 "시장가 폴백 주문 제출"이 없어야 정상).
- [ ] 같은 종목에서 `시장가 폴백 주문 제출` 이 `매도 취소 실패 가능` **직후 같은 시각에** 나오거나, 분할 매도인데 폴백 수량이 보유 전량이면 회귀다(이중 매도·전량 확대 경로).
- [ ] 텔레그램 "청산 주문 취소 불가" 또는 헬스 모니터 `교착 pending` 경보 수신 시: MTS 에서 해당 미체결 매도를 확인·취소 → 다음 재시도(생존 확정 뒤에는 엔진·스케줄러 모두 60초 간격)에 소멸로 판정돼 재판단되는지 확인. 봇 재시작은 필요 없다(재시작하면 인메모리 장부가 비어 판정이 '미추적'으로 바뀐다).
- [ ] CRITICAL `stale 매도 확인 불가 N회 — 재주문 없이 pending 해제` / ERROR `주문 생존 확인 불가 연속 N회 — stage 롤백 없이 pending 해제` 는 거래소 조회(TTTC8036R) 연속 실패를 뜻한다 — EGW00215·다음 페이지(`tr_cont` F/M) 로그와 함께 본다. 이후 그 종목의 손절 판정이 재개됐는지(ExitManager 로그) 확인.
...
- [x] **2026-09-16 10:52 1차 관측 완료** — 10:30 슬롯 원장 6행 append 확인.
- [x] `TeamAssessment` 실측: `abstained` 0/6, `data_sufficiency` partial 6/6, `stance_v2` hold 6/6, **`success_probability` 6/6 None**(계약대로), `execution_state` candidate 6/6.
- [x] `shadow_plan_check` **0건** — 현금 0.49% 라 매수 신호가 엔진 현금 검사에서 먼저 걸린다(훅이 그 뒤). 위 ⚠️ 대로 정상이며 결함이 아니다. status/reasons 분포는 매수 재개 후 재관측.
- [ ] (표본 확보 후) 팀 심의 `entry_ready` 가 대부분 wait(QUOTE_STALE) 이면 후보 현재가 재조회 배선을 검토(KIS 호출 증가 → 별도 승인).
...
### 2026-09-15~ — KIS 원장 연속조회 종료 판정·동기화 세션 게이트 (18:31 KST main 678e132 배포)

- [x] **2026-09-16 관측: 장중 원장 거절 80% 감소.** 같은 시간대(09:00~10:52) 비교 — 09-15 **236건** → 09-16 **46건**. 거절 TR 은 여전히 전부 `TTTC8434R`.
- ⚠️ **직접 증거는 잡히지 않는다.** 페이지 수(`포지션 조회 완료: N개 (M페이지)`)와 `잔고 스냅샷 재사용` 로그가 둘 다 `logger.debug` 라 운영 로그(INFO)에 남지 않는다. 거절 감소는 **간접 증거**다. 직접 확인이 필요하면 일시적으로 로그 레벨을 올리거나 해당 2줄을 INFO 로 승격하는 별도 변경이 필요하다.
- [x] 동기화 잔고 조회 실패 2회(09:08·09:18) — 둘 다 재시도로 복구, '재시도에도 포지션 0건' 0건.
- [ ] 장외 CLOSED 300초 게이트가 실제로 적용되는지(08:00~15:40 은 30초 유지) — 장외 시간대 로그로 재확인.

### 2026-09-15~ — T12 Phase 0 스크리닝 채널 복구 (21:20 KST main 4b70a34 배포)

- [x] **거래량 급증 채널 복구 확인** — 배포 전 30일간 2,474회 호출 전부 0개였다. 09-16 09:00~10:52: 3개×12회, 2개×5회, 1개×2회, 0개×2회.
- [x] 0개인 경우도 새 계측으로 원인이 구분된다 — `거래량 급증 0개 — 응답 30행, 비율산출불가 0행, 게이트 min_ratio=2.0`(응답은 왔고 게이트 탈락). 이전에는 빈 응답과 전량 탈락이 똑같이 "0개 발굴"이었다.
- [x] 네이버 테이블 구조 변경 경고 **0건**(기본 OFF 적용).
- [ ] **통합 후보 수 증가는 확인되지 않았다.** 09-16 33개 vs 09-15 같은 시간대 32~37개로 동일 수준. 거래량 급증이 찾은 1~3종목이 다른 채널과 중복되거나 병합 후 점수 컷에서 걸러진 것으로 보인다 — 채널별 기여 수 계측이 필요하다(T12 Phase 2 인수 조건의 'Phase 0 기준선').
- ⚠️ **비교는 반드시 같은 시간대로 할 것.** PR #60 본문의 "배포 전 26~27개"는 30일 평균이었고 어제 오전 시간대는 이미 34개였다. 일 평균과 특정 시간대를 섞어 비교하면 개선 효과를 오판한다.
- [ ] 16:00 수급 5일 누적 배치(`[수급5일] 갱신`)가 실제로 실행되는지 — import 오타 수정 후 첫 실행은 09-16 저녁.
