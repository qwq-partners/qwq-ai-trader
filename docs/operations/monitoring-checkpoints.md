...
- [ ] 텔레그램 "청산 주문 취소 불가" 또는 헬스 모니터 `교착 pending` 경보 수신 시: MTS 에서 해당 미체결 매도를 확인·취소 → 다음 재시도(엔진 20초·스케줄러 60초 간격)에 소멸로 판정돼 재판단되는지 확인. 봇 재시작은 필요 없다(재시작하면 인메모리 장부가 비어 판정이 '미추적'으로 바뀐다).
