...
- **개장 직후 원장 초과(EGW00215, 09:00~09:15, `TTTC8434R`) ~20건/일은 코드로 해결 불가**
  (2026-09-08 결론): 원장 호출을 응답 후 1.05초로 직렬화하고 유휴 체결확인을 15초로 줄여
  초당 0.2건 수준까지 낮춰도 9/4 18 → 9/7 29 → 9/8 38라인으로 무관하게 발생. 그 외 시간대는
  0건. 원인 후보는 개장 시 KIS 원장 부하 또는 같은 계좌를 조회하는 외부 클라이언트(MTS/HTS
  잔고 화면). 재시도 1~2초·간헐 동기화 1회 스킵 외 영향 없음 — **더 줄이려 리미터를 손대지
  말고 관측만 유지**. `[KIS리미터] EGW00215` 계측 라인으로 건수 확인.
