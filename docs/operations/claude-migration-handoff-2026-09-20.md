...
> **현재 진행 정본(2026-09-23 P1 후속):** [P1 결과 원장](../reviews/p1-producer-wiring-2026-09-23.md)에 후보·검증·통합 상태를 기록한다. 사용자 위임으로 정책/제한된 리뷰 예산은 확정됐으므로 아래 '미답/미착수'는 이력이다. [다음 개발 순서](p1-next-steps-2026-09-23.md)와 설치 차단 표를 함께 확인한다. 개발 기능/설치기 인수와 운영 설치를 구분하며 운영 호출자는 여전히 없다.

> **2026-09-23 현재:** P1 S1·S1′ 부품의 독립 한정 승인을 받고 UTC/KST 전체 검증(각 5183 passed·기존 2 xfailed·격리 위반0)을 통과해 개발 브랜치 `77a3641`에 통합했다. 현재 정본은 [P1 부품 보고서](../reviews/p1-components-2026-09-23.md)와 P1 계획 §6-1 이후다. **S2 이후 미착수·운영 미설치**, 차단23은 일부 남으며 24~27을 아래 표에 등록했다. S2 정책·외부 리뷰 예산은 사용자 미답이다. 아래 상태 줄은 과거 기록이다.
...
> **2026-09-22 갱신 — 임계 경로가 바뀌었다.** KIS 는 질문지에 답하지 않으며 사용자가 "우리가 임의로 판단해서, 제일 이득이 되는 방향으로" 진행하라고 지시했다. 31문항의 판단과 설계 결정(D1~D10), 그 적대적 심사 결과는 **[KIS 미확정 문항의 판단 결정](../superpowers/plans/2026-09-22-kis-judgement-decisions.md)** 이 정본이다. 아래 표의 1·2·12 는 더 이상 "공식 증거 대기"가 아니다: **1 → (1a) 취소 최종성은 "알 수 없다"로 확정하고 설계로 회피 / (1b) 보호 SELL 의 에스컬레이션 부재는 그대로 차단 — 목표는 09-21 밤 main 이 도달한 수준과의 동등** · **2 → 공개 계약으로는 증명 불가로 판정, "차가운 시작" 술어로 대체(설계 확정·구현 대기)** · **12 → 좁히는 것은 `blocked_unknown` 하나, 해제는 운영자 절차(해제 경로는 여전히 0)**. 같은 심사가 **KIS 답과 무관한 새 차단 사유 16~18 과 attach 가 현행보다 약한 지점 10곳**을 찾았다 — 표 맨 아래. 다음 구현 단계는 결정 문서 §5 의 **P0-1(attach transport 의 킬스위치·감사 원장)** 이고, 게이트를 여는 결정(D4·D6)은 P0 전제 **뒤**다.
...
| 16 | **attach 의 주문 POST 가 킬스위치와 감사 원장을 우회한다**(2026-09-22 발견) | `GuardedKISTransport.send_prepared` 가 `broker._session.post` 로 직접 보낸다 — `KISBroker.submit_order` 의 `kill_switch.check`·`audit_log.record` 를 타지 않는다. `src/execution/safety/` 전체에 두 모듈 참조 0건(coordinator 확인). attach 가 설치되면 **긴급 정지 수단이 없고 시도된 주문이 원장에 남지 않는다** | **닫힘 — legacy 와 같은 best-effort 수준까지(2026-09-22, P0-1, 부품·제품 호출자 0건).** `send_prepared` 가 최종 guard 뒤·POST 앞의 동기 구간에서 킬스위치(SUBMIT/MODIFY)를 검사하고 시도·결과를 원장에 남긴다(독립 재현 APPROVE, 런타임 시험으로 위치 고정). **남는 전제:** 킬스위치는 플래그 디렉터리 접근 실패 시 fail-open(현행도 동일) — 설치 시 `~/.cache/ai_trader` 기동 점검 필요. 결정 문서 §5-1 |
| 17 | **30초 legacy writer(`_sync_portfolio`)에 attach 분기가 없다**(2026-09-22 발견) | attach 에서도 포지션·수량·평단·현금을 직접 쓴다 → `legacy_portfolio_writer_conflict` 가 상시 조건이고 한 번 쓰면 다음 owner 게시까지 **전 종목 SELL 이 막힌다**. 반대로 owner 는 사용자의 수동 매도를 배울 경로가 없다(같은 수량 SELL 이 영원히 거절만 반복) | **닫힘(2026-09-22, P0-3, engine `80571eb`)** — attach 에서 `_sync_portfolio` 는 live 를 쓰지 않고 KIS 응답과 owner 게시본을 읽기 전용으로 대조(종목 대칭차·수량·현금 1,000원 임계)해 불일치면 `set_sync_status(False)`·하트비트 실패·error, 0 이면 legacy 와 같이 성공 표시. 분기는 빈응답·부분누락 재시도 방어 **뒤**(시험이 위치 고정), 미설치 경로 바이트 동일. **남는 것:** 불일치의 해소는 여전히 owner 밖(수동 매매를 owner 에 반영하는 경로 없음 — 알람뿐) · 수동 매도 뒤 매 주기 `partial_missing` 재시도 비용(P1 후보). `docs/superpowers/plans/2026-09-22-p0-3-wiring-sync-gate.md` §7 |
| 18 | **attach 에는 체결·종결 증거의 생산자가 0건이다**(2026-09-22 — 3항의 실체) | attach 는 `submit_order` 를 쓰지 않아 `broker._pending_orders` 가 비고 `run_fill_check` 의 `if open_orders:` 가 거짓 → `check_fills()` 가 돌지 않는다. legacy FILL 은 engine 이 거부한다. **ACK 된 주문이 terminal 에 가지 못하고, 매수 체결이 포지션이 되지 않아 손절 신호 자체가 생기지 않는다.** 이 상태에서 게이트를 여는 변경(D4·D6)은 하지 않는다 | **부품 완성(2026-09-22, P0-2, engine `7500d3c`)** — runtime 이 소유하는 주기 task 가 일별체결조회 → 파서 → `reconcile` → 저장 상태 재처리 → `apply_execution_observation` 로 넘기는 경로가 코드로 존재한다(Codex 4라운드 → APPROVE, 전체 5057 passed). **설치기 기준 닫힘(2026-09-22, P0-3, engine `80571eb`)** — `install_attached_runtime(…, collect)` 가 attach 직후 `start_reconciler(collect)` 를 부르고(구간 1 거부 3종: `invalid_execution_collector`·`execution_query_environment_required`·`execution_producer_already_wired`), 자동·수동 BUY 는 생산자 생존 전제(주기 task 생존·굶지 않음·오늘 주기 완주 ≤15초·대상 있으면 오늘 조회 완료 ≤15초) 없이는 `reconciler_unavailable`, SELL·CANCEL 은 통과. 설치~run 창은 `engine_not_running` skip. Codex 15차 P1 2·P2 1 → 처분(미배선 통과는 결정으로 유지 — 설치기가 유일한 attach 경로) → 16차 APPROVE, 전체 5086 passed. **설치기 자체의 제품 호출자는 여전히 0건**이고 실계좌 스모크 4항(P0-3 계획서 §5) 전에는 env 거부·파서 skip·BUY 게이트가 막는다. `docs/superpowers/plans/2026-09-22-p0-3-wiring-sync-gate.md` §7 |
| 19 | **정상 경로에서도 체결마다 전 종목 명령 정지 창이 생긴다**(2026-09-22, P0-2 P-12) | 미적용 inbox 행이 있는 동안 `_owner_ready` 가 `unapplied_execution_observation` 으로 모든 새 명령을 막는다 — 급락 때 체결과 보호가 겹치는 순간에 터진다(legacy `check_fills` 는 매도를 막지 않는다) | P0-2 가 창을 유한하게(주기 안 재적용·`REAPPLY_RESERVE`) 만들고 health 에 노출했다. **`_owner_ready` 의 범위 축소는 P2(게이트 범위) 결정** |
| 20 | **`entry_policy_context` 의 재게시자가 0건이다**(2026-09-22, P0-3 Q-5) | attach 에서 sync·추세·거시 정책 사실이 **설치 시점에 얼어붙는다** — legacy 는 30초 sync 와 2분/정오 owner 가 갱신하지만 attach 의 게이트가 읽는 `PolicyContext` 를 다시 게시하는 제품 코드가 없다 | 10C 의 지속 publisher(sync 결과·레짐 갱신을 `publish_policy_context` 로 잇는 배선) |
| 21 | **보호 생산자 부재 — P1 설치기 범위에서 해결**(2026-09-23 갱신) | 두 legacy 보호 writer의 attach skip은 유지하고, 설치기가 단일 ProtectionProducer를 첫 MARKET_DATA 핸들러로 연결한다. 원 시세→owner quote→실 엔진/gateway→합성 HTTP·체결 큐를 인수했다 | **개발 설치기 범위 해결, 실제 운영 설치 아님.** 후보11b9f3e UTC/KST 각5377 passed. 제품 설치 호출자0·readiness False이며23/26~31과 나머지 설치 전제가 남는다. 손절/회복 지연까지 legacy 동등이라고 확대하지 않음 |
| 22 | **부분 체결 주문의 장 마감 소멸을 owner 가 배울 수 없다**(2026-09-22, P0-4 조사 B) | 증거 파서는 `FINAL_FILLED` 아니면 `RECONCILING` 만 만들고(`evidence.py`) `reconcile` 은 `FINAL_EXPIRED` 를 최종성으로 읽지 않는다(`lifecycle.py` `final = False`). 부분 체결 SELL 이 장 마감으로 소멸해도 attempt 는 영구 `partial` → 그 종목 잠금(`unresolved_symbol_attempt`) + **일자 전환 영구 BLOCKED**(`day_recovery.py` `unresolved_submit`·`remaining_reservation`). 예약 산술 자체는 정상이다(결정 문서 §4-2 항목 4 의 "held−reserved<0" 서술은 오류 — P0-4 에서 정정) | 만료 종결 계약 — 근거는 저장소에 없다(Q19~Q21). D10 실계좌 관측이 선행. **위임된 보수적 기본값: 영구 BLOCKED 그대로**(만료를 코드나 사람이 판정하게 하는 (a)(b) 는 취소 최종성에서 거부한 종류의 가정) |
| 23 | **보호 접수 소비 배선 후에도 실패 장벽 일부 미해소**(P0-4 등록, 2026-09-23 P1 갱신) | S1의 A/B/C 처분에 S2 단일 소비자·원 intent 보관·quote→prepare 직렬화와 S4 설치기를 추가했다. 각 재개는 최대1행, 관측됐지만 미적용인 체결의 pending은 해제하지 않는다. 실제 운영 설치 호출자0 | **일부 해결/계속 열림.** A/C·repair-only·행 없는 commit/게시 결과 불명·durable day fence와 승인된 복구 절차는 남는다. 실제 새 B commit만 자기 실패를 해소한다. 별도 증거/복구 계약 없이 래치를 열지 않는다. 정본 P1 계획 §6-1 |
...
설치 차단과 별개로 **제품이 쓰는 KIS 주문·취소·조회 TR 5종이 전부 "구TR"이다**(가이드: "사전고지 없이 막힐 수 있다" / 2025-02 공지: "자동 매핑되어 즉각적인 영향은 없으나 장기적으로 삭제 가능성") — main 기준의 별도 과제다. **작업 칩 `task_90d688f8` 은 닫았고 이 세션이 직접 브랜치 `fix/kis-tr-id-migration-switch`(base `origin/main`)로 진행했다: `KIS_TR_SET` 기본 `legacy` 에서 요청이 바이트 단위로 같은 무동작 PR.** 신 TR 은 정규장 주문 접수에만 적용하고, 전환(`KIS_TR_SET=new`) 전에 닫아야 할 미해결(취소·정정이 주문의 접수 세션을 모른 채 `"KRX"` 를 붙인다 · 실계좌로만 확인되는 11항)은 [이행 명세](../integrations/kis-tr-migration-spec-2026-09-21.md) §8-1 과 main 쪽 runbook 에 있다. **2026-09-22 현재:** PR #80 은 09-21 20:38 KST 에 **다른 세션이 사용자 지시로 병합·배포**했다(20:44 재시작, 기동 로그 `KIS TR 세트: legacy` — **전환은 하지 않았다**). 후속 [PR #88](https://github.com/qwq-partners/qwq-ai-trader/pull/88)(연속조회 요청 `tr_cont` 헤더·외부계좌 루프의 종료 판정 통일·취소 재시도 유지의 근거 정정)은 `verify` 통과·독립 재현 APPROVE·Codex 9차 APPROVE 뒤 **사용자 지시로 09-22 07:24 KST 에 병합(`d337494`)·배포했다**(개장 전, verify 1982 passed·자동 롤백 없음, PID 1546587, 기동 로그 `KIS TR 세트: legacy`, 170초 시점 KIS 오류·ERROR 0). 운영 checkout 은 `d337494` 에 detached 로 남아 있다(worktree 격리 세션이라 복귀 명령을 실행하지 못했다 — 트리 동일). 미체결 조회의 50건 잘림은 PR #81 이 같은 함수에서 처리해 #88 에서 뺐다. "취소 POST 에 `retry=False` 누락"은 결함이 아니라고 재판정했다(이행 명세 §8-1). 또 **운영 경로(legacy)의 현행 결함 5건 — 09-21 밤 다른 세션이 PR #81·#83·#84 로 main 에서 고쳐 배포했다(아래는 발견 당시 기록이다). ⚠️ 그 결과 main 의 `engine.py`·`kr_scheduler.py` 가 크게 바뀌었고, engine 브랜치의 S4-0 특성화 26건은 "현행 결함 그대로"를 고정하고 있으므로 engine 에 main 을 들이는 날 그 단언들이 뒤집힌다 — 조용히 고치지 말고 결정으로 기록할 것.**이 S4 특성화로 드러나 있다(동시호가에 취소만 보내고 재주문 없음·90초 폴백 루프에 exit_exempt 확인 없음·`submit_order` 예외 시 접수 여부를 모른 채 `clear_pending`·폴백 상한 뒤 원 지정가 방치·취소 0건 예약 해제). engine 브랜치는 이것을 고치지 않고 시험으로 고정만 했다 — main 기준의 별도 운영 수정 과제다.

### P1 착수 시 추가 등록 (2026-09-23, 운영 설치 차단 유지)

| # | 차단 사유 | 근거/범위 | 처분 |
|---|---|---|---|
| 24 | attach 기본 가격 핸들러의 중복 writer 예외 | S3의 attach 가격 핸들러를 무동작으로 바꾸고 기존 거래/직접 체결 writer 거부는 유지했다. 보호 핸들러 선행과 실제 전략 도달을 검증했다 | **개발 설치기 범위 해결.** legacy 본문 바이트 동등·주석2파일 AST 동일. 실제 운영 설치 아님 |
| 25 | 보호 episode intent의 실제 생산/소비 배선 | S2 생산자+S3 정확 수량/세션 경로+S4 설치기로 원 pending owner와 intent를 연결했다. 실제 익절10(독립5+5) 체결 후 잔량90 예약과 원 단계/R/고점 보존을 인수했다 | **개발 설치기 범위 해결.** native·Opus 범위 승인과 UTC/KST 전체 통과. 미지원 수동 복구·open BUY/late-fill 등 다른 차단의 해소를 뜻하지 않음 |
| 26 | 시간외 보호 SELL 생산 경로 없음 | P1은 regular/사용자가 정한 closing에 한정하며 pre_market·next_market 주문 의미와 종목 자격을 새로 가정하지 않는다 | 별도 계약/인수 전 미지원 유지. P1 완료로 닫지 않음 |
| 27 | intraday_preemptive 보호 결정의 소비자 없음 | quote 결정과 같은 종류의 outbox지만 다른 effect_source이며 P1 quote 생산자가 이를 소비하지 않는다 | P1 S1은 health에서 별도 집계만. 실제 소비자는 별도 단계, 미해결 |
| 28 | 보호 제출의 지연·진행 보장 미완 | N1 `06fc7ed`에서 일반 신호가 공통30초 시계를 반복 갱신하는 기아는 전용 시계로 분리했다. 원 intent/예약·ACK/UNKNOWN·관측미적용 및30초/60초 정책 보존 인수·독립 리뷰·UTC/KST 각각5386 통과 | **일반 기아는 개발 범위 해결, 운영 차단은 유지.** 피드/큐 진행·정상 시계·다른 장벽 해소를 전제한 다음 처리 기회만 보장하며 전체 실시간 지연 상한/수동 복구는 미완. [N1 원장](../reviews/p1-protection-priority-2026-09-23.md) |
| 29 | 감사 결정/실패 장벽의 승인된 수동 복구 절차 미완 | 원 RAM/admission 없는 미제출 결정, 역사 full 거절 순서 모호함, 뒤늦은 체결로 수량이 변한 retained 결정 및 A/C·repair-only·결과 불명은 자동 해소되지 않음 | 원 증거·intent·예약을 보존하는 복구 계약과 runbook/음성 인수. 자동 재수량 계산·TTL 삭제로 해결하지 않음 |
| 30 | 보호 관측의 운영 소비자 연결 미완 | kr_protection_producer record_*는 메모리에 기록되나 일반 heartbeat PERIODS/DAILY/observer 등록 목록에 없어 check/snapshot/loop_status·대시보드/Telegram 정체 감시가 읽지 않음. runtime.health 투영과 실제 경보 전달은 다름 | 설치인지형 등록·closing/휴장/무보유/종료 분류·실제 수신 인수. 미설치 legacy에 전역 경보를 먼저 켜지 않음 |
| 31 | 장기 감사 누적 처리 비용·20초 고점 표본 손실 미검증 | 보호 감사/intents는 append-only, 매 sweep 감사 순회 비용이 누적됨. 짧은8종목 시험은 장기 규모 보장이 아니며20초 무결정 쓰기는 순간 고점을 놓칠 수 있음 | 규모별 지연·보존/압축·보호 결과 손실 인수와 운영 수용 판단. 복구 증거를 임의 삭제하지 않음 |

S1 사전 검토의 source 무효화·실패 래치·교차 창 처분은 [P1 사전 검토](../reviews/p1-handoff-preflight-2026-09-22.md)와 P1 계획 §6에서 이어간다. 부품 구현을 운영 설치나 차단23 전체 해소로 보고하지 않는다.
