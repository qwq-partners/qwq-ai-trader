...
| 28 | 일반 신호와의 cooldown 경합으로 보호 제출이 반복 지연됨 | 기존 종목 단위30초 cooldown을31초마다 일반 신호가 갱신하면 보호가 계속 거절되는 실제 합성 반례. 일반 신호 중단 후 회복해도 최대 지연을 증명하지 못함 | 보호 우선순위와 중복/미해결 SELL 방어를 함께 설계·인수하기 전 운영 차단. 기존 한도를 무단 제거하지 않음 |
