...
| 17 | **30초 legacy writer(`_sync_portfolio`)에 attach 분기가 없다**(2026-09-22 발견) | attach 에서도 포지션·수량·평단·현금을 직접 쓴다 → `legacy_portfolio_writer_conflict` 가 상시 조건이고 한 번 쓰면 다음 owner 게시까지 **전 종목 SELL 이 막힌다**. 반대로 owner 는 사용자의 수동 매도를 배울 경로가 없다(같은 수량 SELL 이 영원히 거절만 반복) | P0-3 — owner 경유 재정합(live 파일 `kr_scheduler.py`) |
| 18 | **attach 에는 체결·종결 증거의 생산자가 0건이다**(2026-09-22 — 3항의 실체) | attach 는 `submit_order` 를 쓰지 않아 `broker._pending_orders` 가 비고 `run_fill_check` 의 `if open_orders:` 가 거짓 → `check_fills()` 가 돌지 않는다. legacy FILL 은 engine 이 거부한다. **ACK 된 주문이 terminal 에 가지 못하고, 매수 체결이 포지션이 되지 않아 손절 신호 자체가 생기지 않는다.** 이 상태에서 게이트를 여는 변경(D4·D6)은 하지 않는다 | **부품 완성(2026-09-22, P0-2, engine `7500d3c`)** — runtime 이 소유하는 주기 task 가 일별체결조회 → 파서 → `reconcile` → 저장 상태 재처리 → `apply_execution_observation` 로 넘기는 경로가 코드로 존재한다(Codex 4라운드 → APPROVE, 전체 5057 passed). **닫힘이 아니다** — `start_reconciler` 의 제품 호출자 0건, 배선·활성화 조건(`engine.run` 기동 순서·환경 거부)은 **P0-3**. `docs/superpowers/plans/2026-09-22-p0-2-fill-evidence-producer.md` §7 |
