...
| 16 | **attach 의 주문 POST 가 킬스위치와 감사 원장을 우회한다**(2026-09-22 발견) | `GuardedKISTransport.send_prepared` 가 `broker._session.post` 로 직접 보낸다 — `KISBroker.submit_order` 의 `kill_switch.check`·`audit_log.record` 를 타지 않는다. `src/execution/safety/` 전체에 두 모듈 참조 0건(coordinator 확인). attach 가 설치되면 **긴급 정지 수단이 없고 시도된 주문이 원장에 남지 않는다** | 결정 문서 §5 **P0-1** — 다음 구현 단계(부품·제품 호출자 0건) |
