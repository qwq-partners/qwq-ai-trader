...
| 21 | **attach 에는 보호 청산의 구동기가 없다**(2026-09-22, P0-3 S-D 재현이 드러냄) | `batch_analyzer.monitor_positions`·`kr_scheduler._check_exit_signal` 은 attach 에서 전면 skip 이다(live `current_price`/`highest_price`/`exit_manager` 를 쓰는 legacy writer 라 Q-3 로 막았다). 그 결과 손절·트레일링·분할익절·갭EOD·보유기간 청산 **어느 것도 attach 에서는 발화하지 않는다** — 차단 사유 1b(보호 SELL 에스컬레이션 부재)보다 앞선 문제로, SELL 신호 자체가 없다 | **P1(보호 SELL 의 main 동등)** — owner 의 quote 경로(`runtime` 의 보호 task)에 시세를 넣는 제품 생산자 + owner 게시본을 읽는 emit. 생산자 유무는 P0-3 §6-2 조사 결과를 본다 |
...
| 23 | **보호 접수 복구 부품 이후에도 실제 소비와 실패 장벽이 미해소**(P0-4 등록, 2026-09-23 P1 갱신) | S1은 원 접수 증거를 검증해 A 폐기/B 재개/C 격리와 source 무효화를 원자화하고, B의 실제 새 commit만 자기 실패를 해소한다. 제품 호출자는 여전히0이며 A/C·repair-only·행 없는 commit/게시 결과 불명·durable day fence는 남는다. C degraded의 자동 repair 증거도 보장하지 않는다 | **열림 — S1 부품 한정 승인/전체 검증 통과·개발 통합.** S2의 단일 소비자·원 intent 보관·quote→prepare 직렬화가 필수다. 각 재개는 최대1행, 관측됐지만 미적용인 체결의 pending은 해제하지 않는다. 잔여 장벽은 별도 증거/복구 계약 없이 열지 않는다. 정본 P1 계획 §6-1 |
...
| # | 차단 사유 | 근거/범위 | 처분 |
|---|---|---|---|
| 24 | attach의 MARKET_DATA 기본 핸들러가 직접 가격 writer를 호출해 예외 | `engine.update_position_price`는 owner 설치 후 직접 변경을 거부한다. 보호 생산자만 등록해도 기본 핸들러 경로가 남는다 | P1 S3에서 attach 분기만 무동작으로 전환하고 실제 핸들러 인수. 미해결 |
| 25 | 보호 episode intent 부품의 실제 생산/소비 배선 미완 | S1′은 명시된 보호 SELL ID를 캐시와 분리했다. 실제 owner에서 익절10 체결 후 별도 손절/EOD90 예약을 검증했지만, 이 ID를 만드는 제품 생산자는 아직 없다 | S1′ 부품 한정 승인/전체 검증 통과·개발 통합. S2 생산자 + S3/S4 배선과 원 pending owner 인계 전까지 열림 |
