...
| 23 | **미해결 보호 접수 행(`protection_quote_admissions`)에 해제 경로가 없다**(2026-09-22, P0-4 설계·심사) | 접수 command_id 는 호출마다 새로 굽고(`runtime.py` `"quote:"+uuid`) 같은 종목의 다음 `quote()` 는 중복 거부, `repair_protection` 은 `unresolved_quote_admission` 거부, **재시작도 행을 지우지 않는다**. 행 하나가 전 종목 SUBMIT(`market_source_pending`)·일자 전환·초기 R·보호 복구를 영구히 막는다. 게다가 **BLOCKED 로 끝난 `prepare_day_rollover` 한 번이 `_day_closed` 를 영구 래치한다**(reducer 전에 True, BLOCKED 여도 `day_transition=PREPARED` commit, 재시작 후 복원 — 심사 ②M1 코드 인용, P0-4 A6 이 런타임 재현). bool 래치 `_protection_failed` 도 해제 경로 0 이나 durable 행보다 약한 문제다 | **P1 의 owner quote 생산자 설계와 함께** — P0-4 심사가 보인 것: 저장 행을 재개(resume)하는 해제는 아무도 받지 않는 보호 결정을 커밋해 `pending_stage` 를 영구히 세우고(손절이 조용히 사라짐), 낡은 진입 증거로 BUY 게이트를 열며, 결정적 reducer 실패는 고치지 못한다 — 재개는 결정의 소비자가 있어야 안전하다. P0-4 는 설정 조건만 좁힌다(`admit` 의 사전 거부는 래치하지 않음) |
