...
- 포트폴리오 동기화는 거래 가능 세션 30초(CLOSED 300초) 주기로 브로커 상태와 로컬 상태를 다시 맞춘다. 잔고·포지션은 같은 8434R 응답(헤더 tr_cont 로 마지막 페이지 판정)을 재사용해 1틱당 1회 호출한다(2026-09-15).
...
| 포트폴리오 동기화 | 거래 가능 세션 30초 / CLOSED 300초 |
