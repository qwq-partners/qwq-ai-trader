...
- **루프 슈퍼바이저 `_supervised()`** (2026-08-04 신설, 08-05 전면 유효화): KR 스케줄러
  루프 16개를 래핑 — 예외 사망 시 60초→최대 30분 배증 백오프로 재기동 + 텔레그램 경보.
  ⚠️ 래핑된 루프의 outer `except Exception`은 반드시 `raise`로 재전파해야 함
  (삼키고 정상 반환하면 "종료 의도"로 간주해 재기동하지 않음 — 8/5 P1의 원인)
- **KR 태스크**: 스크리닝(5분), 체결확인(10초), 포트폴리오(30초), REST피드(20초), 테마(10분), 수급(5분), 배치(08:20/12:30/19:30), evolve(20:30), 리밸런싱(토요일),
  변동성타게팅 캐시(거래일 08:30 — 모멘텀 사이징 배율), DART 공시경보(장중 10분 — 보유 종목 경보 전용),
  밸류코어 shadow(주 1회)·수확 shadow(거래일 08:40)·승격 기준 점검(토요일 — 관측 전용)
