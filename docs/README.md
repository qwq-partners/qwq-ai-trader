...
- [P1 — 보호 SELL 의 main 동등 (Plan, 2026-09-22 — **Do 는 사용자 결정 §4-1 뒤**)](superpowers/plans/2026-09-22-p1-protective-sell-parity.md) — 조사 3(main 기준선 체크리스트·owner 부품·생산자 입력) · 설계 초안(**보호 SELL 을 `regular` 세션에서 시장가로 내서 취소·3분류·폴백을 작성하지 않는다**, MARKET_DATA 핸들러 생산자·에피소드 intent·pending 해제·고아 행 재개) · 심사 2관점 REVISE ×2(must-fix 16) · **§3 처분** · **§4 사용자 결정 7건**(시장가 전환 승인이 P1 전체를 성립) · §5 단계 S1~S5 · 새 차단 사유 후보 24~27
...
- [**KIS 미확정 문항의 판단 결정 (2026-09-22)**](superpowers/plans/2026-09-22-kis-judgement-decisions.md) — KIS 는 답하지 않는다 → 사용자 지시로 31문항을 우리가 판단해 확정(어느 답이 참이어도 안전한 쪽). 문항별 판단표 · 설계 결정 D1~D10(적대적 심사 ① BLOCK·② REVISE 처분 반영) · **심사가 찾은 새 설치 차단 사유 3건과 attach 가 현행보다 약한 지점 10곳** · 전제 우선의 순서(P0-1 킬스위치·감사 원장부터) · 사용자에게 올리는 전략 판단. **설치 차단의 임계 경로는 이 문서부터**
- [KIS 공식 저장소 기준 — 질문지 답변 상태 (2026-09-21)](integrations/kis-repo-grounding-2026-09-21.md) — 사용자 지정 기준 자료(`koreainvestment/open-trading-api@b4e6249`) 전수 조사·적대적 검증(97건). Q1~Q31 각각을 "저장소가 답함 / 예제 동작으로만 / 저장소에 없음"으로 분류하고 **제품 구현 기준으로 채택할 수 있는 것(요청·응답의 모양·프로토콜)과 채택하면 안 되는 것(취소 최종성·누적 범위·cutoff)** 을 가른다. 결론: 설치 차단 사유 1·2·12 불변. 짝 문서 [구 TR → 신 TR 이행 명세(운영 main)](integrations/kis-tr-migration-spec-2026-09-21.md) — TR 표·요청 본문/헤더 차이·고칠 지점 전체·되돌릴 수 있는 전환·오프라인 검증 목록·실계좌로만 확인 가능한 항목, 그리고 이행과 별개인 인접 결함 4건(취소 POST 재전송·미체결 조회의 50건 잘림 등). **§8 이 구현 상태다**(main 쪽 무동작 전환 스위치 브랜치 `fix/kis-tr-id-migration-switch` 와 전환 전 미해결 · engine 쪽 증거 파서/수집기 정합과 chain 판정의 거래소 범위 경고)
- [KIS 증거 좁히기 (2026-09-21)](integrations/kis-execution-evidence-narrowing-2026-09-21.md) — 공개 공식 자료(개발자 포털 문서·공지·FAQ, 공식 GitHub) 재조사를 적대적으로 검증한 결과. 증거 요청 6행 각각의 "더 고정된 근거 / 여전히 미입증 / 필요한 것", **KIS 에 그대로 보낼 질문지 31문항**, 비식별 응답 수집 체크리스트, 답이 오면 바뀌는 제품 지점(증거 파서·`reconcile`·startup 계약 ↔ 설치 차단 사유 1·2·12). 닫힌 계약 행은 0개 — 실 API 호출·실주문 없음. 선행: [실행 증거 계약](integrations/kis-execution-contract-2026-09-17.md)·[증거 요청 목록](integrations/kis-execution-evidence-request-2026-09-18.md)
- [10A3 세부 계획 — 설치 준비](superpowers/plans/2026-09-21-10a3-install-preparation.md) — B2/B3 다음 단계. 적대적 심사(must-fix 13건)가 "설치 factory 본체는 증거 계약·사용자 결정 없이는 명세할 수 없다"를 보여 **10A3a(지금: 로드된 설정에서 owner 정책을 만드는 부품 + owner 게이트 단독 결정 인수, 기존 제품 파일 0줄)** 와 **10A3b(미룸: 설치 factory·관측·시계 주입)** 로 나눴다. 코드로 확정한 사실 9건(UNKNOWN 정지는 보호 SELL·CANCEL 포함 · `restore()` 는 live 를 덮어쓴다 · 일자 전환 없이는 아침 재기동 설치 불가 등), 10A3b 에 앞서 필요한 사용자 결정 6건과 기본값, 10A3b 의 설계 메모
- [S5 세부 계획 — attach 경로의 독립 실큐 인수](superpowers/plans/2026-09-21-s5-independent-acceptance.md) — 구현 worker 의 하네스를 물려받지 않는 새 하네스·새 시험 파일 1개(제품 수정 0). Plan 이 코드로 확정한 사실 8건(claim 이전 실패 5갈래 중 예약 0 은 2갈래뿐 · attach 에서 가격 없는 SELL 은 거부 · UNKNOWN 은 실큐 층 무시험 · POST 본문 단언 0건 · exit_exempt 의 정본은 owner · 걷어낼 수 없는 스텁 2종은 운영 DB 에 닿는다 · 설정 출처 4개 · `__init__` 속성 명시), 하네스 요구(시계 6축+1·단일 sidecar·증가하는 주문번호), 시나리오 wave 1(경로)·wave 2(게이트). **설치 차단 사유의 정본은 [인계 문서](operations/claude-migration-handoff-2026-09-20.md)의 표(현재 15항)**
- [S4 세부 계획 — attach 에서 되살릴 수 있는 것만 owner 경로로](superpowers/plans/2026-09-21-s4-owner-path-restoration.md) — 취소 최종성 증거가 제품에 없다는 사실에서 출발한 **범위 축소**(90초 SELL 에스컬레이션·10분 BUY 취소·owner 취소 배선·SELL 지정가 재확인은 하지 않는다), 결정 9건(적대적 심사 must-fix 5건 처분), 하위 단계 S4-0(legacy 세 경로 특성화)·S4-1/1b(미claim 자식 종료)·S4-2(eviction), 하지 않는 것과 그것을 여는 전제. 각 단계 절의 "통합된 실제 인터페이스/결과"가 구현 뒤의 정본이다(S4 구현 완료·운영 미설치 — attach 설치의 차단 사유가 §0 에 있다)
- [S3 세부 계획 — SIGNAL→gateway→owner prepare/dispatch](superpowers/plans/2026-09-21-s3-signal-gateway.md) — 조사 확정 사실 19·coordinator 결정 17건(적대적 심사 must-fix 8건 수용)·계약 8·하위 단계 S3-1~S3-6b 의 고정 인터페이스/RED/변이·legacy/US 불변 증명·S4/S5/10A3/10C 로 미룬 것. 각 단계 절의 "통합된 실제 인터페이스"가 구현 뒤의 정본이다(S3 구현 완료·운영 미설치)
