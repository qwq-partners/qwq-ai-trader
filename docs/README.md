...
- [S4 세부 계획 — attach 에서 되살릴 수 있는 것만 owner 경로로](superpowers/plans/2026-09-21-s4-owner-path-restoration.md) — 취소 최종성 증거가 제품에 없다는 사실에서 출발한 **범위 축소**(90초 SELL 에스컬레이션·10분 BUY 취소·owner 취소 배선·SELL 지정가 재확인은 하지 않는다), 결정 9건(적대적 심사 must-fix 5건 처분), 하위 단계 S4-0(legacy 세 경로 특성화)·S4-1/1b(미claim 자식 종료)·S4-2(eviction), 하지 않는 것과 그것을 여는 전제. **계획이며 구현 아님**
