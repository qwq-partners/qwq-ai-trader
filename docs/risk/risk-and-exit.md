...
## 2026-09-23 P1 보호 배선 후보 — 운영 미설치

일반 보호 SELL은 regular MARKET, closing(15:20~15:30) LIMIT만 지원하며 취소/시장가 폴백을 추가하지 않는다. 보유기간은 ExitManager 영업일을 따른다. 원 보호 intent·수량·가격을 보존하고 UNKNOWN이나 미적용 체결의 pending을 시간으로 해제하지 않는다. 무결정 quote 쓰기는20초로 제한하되 결정은 즉시 기록한다.

설치기 후보에서 실제 엔진/큐와 합성 HTTP로 익절10→잔량90→손절90을 검증했다. 제품의 `trading_ready=False`와 실제 설치 호출자0은 유지한다. 반복 일반 신호의 cooldown에 의한 보호 기아, 승인된 수동 복구 부재, 관측 API/알람 미연결, 감사 누적 비용·순간 고점 손실은 운영 차단으로 남는다. [P1 결과 원장](../reviews/p1-producer-wiring-2026-09-23.md)과 [후속 작업](../operations/p1-next-steps-2026-09-23.md)을 확인하며, 이 후보가 아래 운영 정책이나 수익성 승격을 바꾸지는 않는다.

## 2026-09-18 실행 상태 이행 브랜치 — 운영 정책과 구분

경제/보호 checkpoint 후보와 실제 core 큐 시험을 추가했지만 운영 진입점에는 설치하지 않았다. 아래 기존 위험 수치·정책/배분은 변경하지 않는다.

- 같은 진입의40+60 체결은 재등록하지 않아 stage/BE/high/pending/확정 R을 유지한다. 별도 추가 주문은 주문 전 보유 대비 누적10% 도달 시 기존 리셋을 한 번만 적용한다.
- pending 익절 목표는 같은 intent의 SELL만 진행한다. 시간 경과만으로 후보의 pending을 풀지 않는다. 보호 실패는 경제 상태를 되돌리지 않고 degraded를 유지하며 자동 repair는 아직 구현하지 않았다.
- 손실 청산은 목적별1회, V재진입 소모는 실제 신규 주문 체결과 같은 checkpoint에 기록한다. 비활성인 기존 KR 승패 집계 메서드를 새로 활성화하지 않는다. 날짜 인계가 미해결이면 임의 리셋하지 않는다.
- 비용은 기존 FeeCalculator 요율/원 단위 반올림의 **누적 추정 비용 차분**이다. 분할 관측에 따른 재반올림 차이를 없애지만 실제 징수액으로 표시하지 않는다. 잔여 취득대금·매수비용을 보존하고 부분매도에 배분하며 마지막 매도는 잔액을 소진한다. 기존 미측정 비용·초과 매도는 대사 대상으로 남긴다.
- initial R은 명시 종결·측정 자료 연결 전 pending, outbox 미전송은 journal_pending이다. 이는 canary 승격/초과수익 증명이 아니다. [완료 범위·잔여](../reviews/engine-execution-safety-2026-09-17.md) 참조.
