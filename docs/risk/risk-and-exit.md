...
- **재시도 구동:** stale 루프는 `on_signal` 안에만 있어 SIGNAL 이 끊기면 멈추고, 스케줄러 장부는 부분체결만 돼도 지워져 받쳐 주지 못한다. 유지는 '다음에 다시 본다'는 약속이라 **그 약속만은** 엔진 하트비트(10초, SIGNAL·FILL 과 같은 직렬 큐 — 루프와 겹치지 않는다)의 `on_heartbeat` 가 구동한다(교차 리뷰 P1) — 재시도 간격이 지난 유지분을 오래 기다린 순으로 **모두**(on_signal 의 stale 루프와 같은 직렬 처리). 하트비트당 한 종목씩 처리하면 간격이 10초×종목 수로 늘어나 8종목이면 해제가 등록 후 820초까지 밀린다(교차 리뷰 4회차 P1 — 시험이 8종목 동시 유지를 SIGNAL·스케줄러 장부 없이 끝까지 구동한다). 다만 엔진은 핸들러가 끝나야 다음 이벤트를 처리하므로, 조회가 타임아웃되는 날(종목당 수십 초) 전부를 끝까지 처리하면 큐의 FILL·ORDER 가 수 분 밀린다(교차 리뷰 5회차 P1) → 하트비트 한 번의 처리에 **5초 예산**을 둔다: 시작 뒤 5초가 지났으면 다음 종목을 **시작하지 않는다**(브로커가 빠르면 한 번에 전부, 느리면 첫 종목만 처리하고 나머지는 다음 하트비트로 — 그 사이 큐의 FILL·ORDER 가 먼저 처리된다). 핸들러 완료의 절대 상한은 아니다: 최악은 '5초 미만의 선행 처리 + 마지막 한 종목의 전체 I/O 시간'이고, 그것은 on_signal 의 stale 루프가 한 종목에 쓰는 시간과 같다. 시도·판정 시각은 종목마다 실제 처리 시각으로 남긴다. **유지는 폴백으로 넘어가는 순간 끝난다**: 소멸 판정 뒤 시장가 제출이 실패(응답 유실 = 접수 불명 포함)해도 유지 장부는 이미 닫혀 있어 하트비트가 SIGNAL 없이 재제출을 구동하지 않는다 — 재시도는 종전처럼 다음 SIGNAL 이 있어야 시작된다(교차 리뷰 5회차 P1). 자동매도 금지(exit_exempt) 종목은 `_fallback_stale_sell` 머리의 면제 분기(PR #83)를 먼저 타므로 하트비트 재시도에서도 남은 지정가만 취소하고 시장가로 재주문하지 않는다. **면제 종목은 이 절의 관문이 아니라 #83 의 면제 처리가 맡는다**(교차 리뷰 6회차 P1 — 병합 상호작용): 면제 분기는 들어오는 순간 유지 장부를 닫고(남기면 `tried_at` 이 갱신되지 않아 하트비트 정렬의 맨 앞을 계속 차지해 뒤 종목을 굶긴다), 엔진의 유지 판단과 스케줄러의 관문은 생존 조회 `await` 뒤에 면제 여부를 다시 확인한다 — 판단 불가 예산의 해제가 살아 있을 수 있는 면제 SELL 의 pending 을 풀면 취소 재시도 대상에서 빠진 채 체결될 수 있기 때문이다. 엔진은 아무것도 풀지 않고 넘기고(다음 주기에 머리의 면제 분기), 스케줄러는 False 를 돌려 호출측의 면제 확인(`_exempt_sell_still_open` — 브로커에 남아 있으면 양쪽 pending 보존·60초 뒤 재취소)이 결정하게 한다. 하트비트는 우선순위 10 이라 큐가 포화되면 밀린다: 10초는 발행 주기이지 처리 보장 시한이 아니고, 시간 예산은 '그 뒤 처음 처리될 때' 적용된다. 유지분이 없으면 아무것도 하지 않는다 — 90초 폴백·10분 BUY 정리의 SIGNAL 의존은 종전 그대로다.
- **스로틀:** 엔진은 pending 등록 시각을 되감지 않고 별도 장부 `_pending_cancel_keep = _SellKeep(횟수, 마지막 시도 시각, 생존 로그 1회 여부, 직전 판정이 생존인가, 마지막 비-판단불가 시각)` 로 간격을 만든다. 간격은 **직전 판정**을 따른다(생존 확인 직후만 60초, 그 밖에는 20초) — '한 번이라도 생존을 봤는가'로 읽으면 생존 뒤의 판단 불가 재시도가 전부 60초 간격이 된다(교차 리뷰 3회차 P1) — 되감으면 헬스 모니터의 5분 교착 경보가 '손절이 막힌 SELL 유지'를 영영 보지 못한다. 폴백 횟수(`_pending_fallback_count`, 상한 2)와 장부를 나눠 유지가 시장가 폴백 예산을 깎지 않으며, `clear_pending`·`on_fill`(완결)·새 pending 등록·**폴백으로 넘어가는 순간(제출 전)**·면제 분기 진입에서 지운다. 스케줄러는 60초 스로틀 표식을 **생존 판정이 끝날 때까지** 유지한다 — 판정 `await` 중에 WS 틱·REST 폴링·독립 정리 루프(`run_pending_cleanup`, 60초)가 같은 종목에 취소·조회를 겹쳐 보내지 않게. 표식 `_stale_cancel_miss = (pending 등록 시각, 연속 판단불가 횟수, 경보 여부)` 는 등록 시각에 묶어 체결 등 다른 경로로 끝난 이전 pending 의 표식이 새 pending 에 새지 않는다.
- **동시호가(15:20~15:30)**: 엔진은 PR #81 대로 취소보다 앞에서 분기하므로 이 구간에는 (자동매도 금지 종목이 아닌 한 — 면제 분기는 동시호가 검사보다 앞에서 취소한다) 취소도 생존 조회도 보내지 않는다(유지 중이던 pending 도 그대로 지정가 유지, 하트비트도 같은 분기를 탄다). 스케줄러의 3분 취소·재발행은 종전 그대로이고 분할 매도의 취소 실패일 때만 위 단계를 탄다. 15:30 이후 엔진 루프·하트비트 재시도는 돌지 않으며 잔여는 장마감 리셋이 지운다.
