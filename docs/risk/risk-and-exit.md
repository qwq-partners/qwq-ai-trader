...
| 그 뒤: 판단 불가(조회 실패·다음 페이지 있음) | 유지. **연속 9회**에서 CRITICAL 로그와 함께 **재주문 없이** pending 해제 — 처음부터 판단 불가면 90초 + 20초×9 = 270초(종전 최장 보유 90초×3), 생존 확인 뒤 판단 불가로 바뀌면 마지막 생존 확인 + 60초 + 20초×8 = 220초 | 유지. **연속 2회**(등록 후 약 3분 첫 회 대기 → 약 4분 판단 불가 1회 → **약 5분** 2회)에서 이 장부와 엔진 pending 을 풀되 **stage 는 롤백하지 않는다** |
