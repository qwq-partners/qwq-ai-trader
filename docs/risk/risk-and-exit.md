...
두 루프는 `on_signal` 본문 안에만 있어 **SIGNAL 이 와야 돈다**(주기 태스크 아님).

| 대상 | 조건 | 동작 |
|------|------|------|
| 미체결 SELL | 90초 경과 + 09:00~15:20 | 거래소 취소 → 원 주문 수량으로 시장가 재주문(최대 2회). **취소 0건이면 재주문하지 않고** 아래 '매도 쪽 취소 0건' 단계 |
...
## 매도 쪽 취소 0건 — 살아 있는 SELL 위 재발행 금지 (2026-09-21 후속, 시험 `tests/test_stale_sell_cancel_failure.py`)

**결함(재현):** 100주 보유, 1차 익절 10주 지정가 SELL 활성, 취소 API 실패(브로커가 0 반환). ① 엔진 90초 폴백은 반환값을 보지 않고 시장가 10주를 추가 제출했다 — 합계 20주는 보유 수량 안이라 KIS 가 거절하지 않는다(전량 매도만 APBK0400 으로 보호). ② 스케줄러 `_cleanup_stale_pending` 은 0건이면 양쪽 pending 을 풀고 `rollback_stage` 했다 → ExitManager 가 같은 10주를 재발행. 방금 체결된 경우엔 롤백이 `ExitManager.on_fill` 의 stage 승격까지 지워(`pending_stage` 가 없으면 승격하지 않는다) 1차 익절이 다시 발동한다.

**권한 분담:** 주문 생존의 근거는 **거래소 실 미체결**(브로커 인메모리 장부는 1차 힌트)이다. 엔진은 '취소 → 시장가 재발행'의 유일한 실행자, 스케줄러는 '청산 의도(stage) 롤백'의 유일한 실행자이고, **둘 다 같은 판정 함수 `RiskManager.stale_order_still_live`(BUY 정리와 공용)로 생존이 부정되기 전에는 자기 행동을 하지 않는다.** 브로커 장부는 완전 체결(`check_fills`)과 취소 성공에서만 빠진다 — 거래소 측 취소·거부는 감지하지 못하므로 MTS 수동 취소 주문은 장부에 계속 남는다.

| 단계 | 엔진 90초 SELL 폴백 | 스케줄러 3분 정리(stale·고아 루프 공통) |
|------|--------------------|----------------------------------------|
| 취소 ≥1건 | 종전대로 즉시 시장가 전환(지연 0) | 종전대로 해제 + stage 롤백 |
| 0건 · 포지션 없음 | 종전대로 해제(재주문이 없어 위험 없음) | — |
| 0건 · 브로커 미추적 | **첫 회는 대기**(장부에서 빠졌는데 pending 이 남았다 = FillEvent 가 오는 중), 다음 주기에도 남으면 종전 시장가 폴백 | 종전대로 즉시 해제 — 엔진이 SELL 신호를 거부해 주문이 없던 고아가 흔한 경우라, 기다리면 그 종목의 청산 판정이 60초 더 막힌다 |
| 0건 · 추적 중(=취소 실패) 첫 회 | 조회 없이 대기, **20초** 뒤 취소 재시도 | 조회 없이 대기, **60초** 스로틀 뒤 재시도 |
| 그 뒤: 거래소에 SELL 미체결 있음 | 재주문 없이 유지, 20초마다 취소 재시도(성공하면 그때 시장가 전환). **상한 없음** — 풀면 발생원이 같은 분할 매도를 그 위에 재발행한다. 확인될 때마다 ERROR 로그, 5분을 넘기면 헬스 모니터 교착 경보(텔레그램) | 유지(해제·롤백 없음), 60초마다 재시도. **상한 없음**. 생존 첫 확인 시 텔레그램 CRITICAL 1회("청산 주문 취소 불가") |
| 그 뒤: 거래소에 없음 | 소멸(수동 취소 등) → 종전 시장가 폴백(원 주문 수량) | 소멸 → 종전대로 해제 + 롤백(재판단 허용) |
| 그 뒤: 판단 불가(조회 실패·다음 페이지 있음) | 유지. **연속 9회(90초 + 20초×9 = 270초 = 종전 최장 보유 90초×3)** 에서 CRITICAL 로그와 함께 **재주문 없이** pending 해제 | 유지. **연속 15회(60초 스로틀 → 종전 상한 15분)** 에서 강제 해제 |

**청산 지연 대 이중 매도의 균형**
- 취소가 성공하면 지연은 0 이다(경로 불변). 지연이 생기는 것은 취소 0건일 때뿐이고, 가장 흔한 원인인 '방금 체결'에서는 재주문이 애초에 필요 없다.
- **전량 매도(손절 등)**: 원 주문이 살아 있는 동안에는 시장가를 보내도 KIS 가 주문가능수량 0 으로 거절한다 — 종전 동작도 청산을 앞당기지 못했고, 거절 2회로 폴백 예산을 태운 뒤 "청산 시그널 소실"로 끝났다. 청산을 앞당기는 유일한 수단은 **취소 재시도**이므로 엔진 간격을 BUY 의 60초가 아니라 20초(체결 확인 5초 주기 몇 회분)로 잡았다. 일시 장애라면 추가 지연은 재시도 1회분이다.
- **분할 매도**: 유지되는 동안 pending 이 그 종목의 다른 신호(손절 포함)도 막는다. 다만 살아 있는 분할 매도가 있는 한 전량 손절은 KIS 가 수량 초과로 거절하므로(주문가능 = 보유 − 미체결 매도) pending 을 풀어도 손절은 나가지 못하고, 풀면 같은 분할 익절만 재발행된다 → **생존 확인 시 상한 없이 유지 + 사람에게 경보**(스케줄러 텔레그램 1회, 헬스 모니터의 5분 pending 교착 경보). MTS 에서 그 미체결을 취소하면 다음 재시도(엔진 20초·스케줄러 60초)에 소멸로 판정돼 정상 흐름으로 돌아온다.
- 상한은 BUY 유지(PR #81)와 같이 **연속 '판단 불가'** 에만 적용한다 — 생존이 확인되면 횟수를 되돌린다. 나이·누적 횟수로 세면 오래 살아 있던 주문이 조회 실패 한 번으로 풀려 그 위에 재발행된다.
- **판단 불가**는 반대다: 조회가 계속 실패하면(미체결 50건 초과로 항상 다음 페이지가 있는 경우 포함) 풀릴 근거가 영영 오지 않으므로 종전 시간 예산(엔진 270초·스케줄러 15분) 안에서 푼다. 이때 엔진은 **시장가를 얹지 않는다** — 재판단은 발생원이 하고, ExitManager 발 매도는 스케줄러 장부와 ExitManager `pending_stage`(거래소 확인 후에만 만료, 하드 30분)가 재발행을 계속 막는다.
- 엔진 스로틀은 pending 등록 시각을 되감지 않고 별도 장부(`_pending_cancel_keep` = (횟수, 마지막 시도 시각))로 만든다 — 되감으면 헬스 모니터의 5분 교착 경보가 '손절이 막힌 SELL 유지'를 영영 보지 못한다. 폴백 횟수(`_pending_fallback_count`, 상한 2)와 장부를 나눠 유지가 시장가 폴백 예산을 깎지 않으며, `clear_pending`·`on_fill`(완결)·새 pending 등록·폴백 제출 성공에서 지운다.
- **동시호가(15:20~15:30)**: 엔진은 PR #81 대로 취소보다 앞에서 분기하므로 이 구간에는 취소도 생존 조회도 보내지 않는다(유지 중이던 pending 도 그대로 지정가 유지). 스케줄러의 3분 취소·재발행은 종전 그대로이고 0건일 때만 위 단계를 탄다. 15:30 이후 엔진 루프는 돌지 않으며 잔여는 장마감 리셋이 지운다.

**브로커 계약을 바꾸는 안과의 비교(채택 안 함):** `cancel_all_for_symbol` 이 실패를 구분해 알리게(예외 또는 `(성공, 실패)` 반환) 바꾸는 안은 ① 호출처 6곳(엔진 2·스케줄러 2·장마감 리셋·운영 스크립트 3)이 전부 try/except 로 감싸고 있어 '예외 = 유지' 분기가 한꺼번에 살아나고 운영 스크립트(`liquidate_all.py` 등)의 동작까지 바뀐다. ② 실패를 정확히 알아도 **'취소 거절'은 원 주문이 체결됐는지 살아 있는지 말해 주지 않는다** — 거래소 조회는 어차피 필요하고, 계약 변경이 대체하는 것은 값싼 1차 힌트뿐이다. ③ 그 힌트는 현 계약에서도 정확히 유도된다: 취소 성공은 주문을 장부에서 빼므로, 0건 반환 뒤 장부에 남은 활성 주문이 곧 취소에 실패한 주문이다. ④ 엔진 실행 안전성 이행 브랜치가 취소 결과·원 주문 최종상태를 분리하는 계약(`OrderEvidence`·lifecycle)을 새로 설계 중이라 legacy 계약을 지금 바꾸면 중복·충돌이다.

**한계(잔존 위험)**
- 엔진 재시도는 **SIGNAL 이 와야 돈다** — 20초·9회는 하한이지 보장 시간이 아니다. ExitManager 발 매도는 WS 틱 구동인 스케줄러(60초 스로틀)가 받쳐 주지만 그 외 발행처(batch·core·교체 축출)의 SELL 은 SIGNAL 이 끊기면 재시도도 폴백도 멈춘다(종전과 같음, 주기 태스크화는 엔진 이행 S4 범위).
- 취소가 **일부만 성공**(같은 종목 활성 주문 2건 중 1건 실패)하면 반환값이 1 이상이라 종전 경로를 탄다. 종목당 pending 1건 불변식 아래에서는 수동 취소된 죽은 주문이 장부에 남은 경우에만 생기며, 그때 실패한 쪽은 이미 죽은 주문이다.
- '거래소에 없음 = 소멸'은 체결 확인(`check_fills`)이 20초(스케줄러 60초) 넘게 밀려 있으면 '체결됐으나 미반영'과 구분되지 않는다 — ExitManager pending 검증자(2026-08-08 P0)와 같은 신뢰 수준이다.
- 스케줄러의 '미추적 즉시 해제'에는 체결 확인이 장부에서 뺀 직후 ~ 스케줄러 장부를 지우기 전(수 ms~수백 ms)에 3분 넘은 pending 의 틱이 끼어드는 창이 남는다(종전과 같음).
- 스케줄러 고아 루프의 관문은 현재 도달하지 않는 심층 방어다 — 같은 호출에서 stale 루프가 같은 종목을 먼저 해제하거나 유지+60초 스로틀하므로(변이 검증에서 유일하게 생존, 제거하지 않음).
