...
**적용 범위 = 분할 매도만, 판정은 등록 시점의 의도.** 과매도는 `원 주문 수량 < 보유 수량` 일 때만 가능하다. 전량 청산(손절·트레일링·EOD, 수량상 전량인 익절)은 원 주문이 살아 있으면 KIS 가 주문가능수량 0 으로 거절한다 → **두 경로 모두 종전 그대로 두어 손절 지연을 만들지 않는다.** 분할 여부는 나중에 추정하지 않고 **발행 의도로 남긴다**: 엔진은 `on_signal` 이 SELL 수량을 정하는 **같은 스냅샷**에서 `metadata.quantity < 보유 수량` 이고 `exit_action` 이 없거나 `sell_partial` 일 때만 분할로 보고(`sell_all`·`replacement_exit` 은 수량과 무관하게 전량, 수량 미지정은 보유 전량) 등록 때 `_pending_signal_cache[종목] = {"sell_partial_intent": True}` 로 남긴다(수명은 그 캐시와 같다 — `clear_pending`·`on_fill` 완결·새 등록에서 지움). 수량을 정한 뒤 매수1호가 조회 `await` 를 건너 등록 시점의 보유량과 다시 비교하면, 그 사이 늦은 잔고 동기화가 보유를 되돌리는 순간 전량 청산이 분할로 굳는다(교차 리뷰 3회차 P1). 스케줄러는 `_check_exit_signal` 이 `sell_partial` 로 발행하며 `_partial_exit_pending[종목] = 등록 시각`(세대에 묶임)을 남긴다. 현재 수량 비교나 ExitManager `pending_stage` 로 추정하면 틀린다(교차 리뷰 2회차 P1): 늦게 도착한 잔고 스냅샷이 포트폴리오를 되돌리면 잔량 90주 손절이 `90 < 100` 으로 분할이 되고, 아래 '롤백 없는 해제' 뒤에는 옛 익절 stage 가 남은 채 새 손절 pending 이 등록된다.
...
| 그 뒤: 판단 불가(조회 실패·다음 페이지 있음) | 유지. 마지막 비-판단불가 판정(첫 회 대기·생존 확인) 뒤 **180초**가 지나도록 판단 불가뿐이면 CRITICAL 로그와 함께 **재주문 없이** pending 해제 — 처음부터 판단 불가면 등록 후 90 + 180 = 270초(종전 최장 보유 90초×3). **횟수가 아니라 시간**으로 센다: 재시도가 SIGNAL·하트비트 구동이라 횟수 상한은 종목이 많거나 큐가 밀리면 그만큼 늦게 찬다 | 유지. **연속 2회**(등록 후 약 3분 첫 회 대기 → 약 4분 판단 불가 1회 → **약 5분** 2회)에서 이 장부와 엔진 pending 을 풀되 **stage 는 롤백하지 않는다** |
...
- **재시도 구동:** stale 루프는 `on_signal` 안에만 있어 SIGNAL 이 끊기면 멈추고, 스케줄러 장부는 부분체결만 돼도 지워져 받쳐 주지 못한다. 유지는 '다음에 다시 본다'는 약속이라 **그 약속만은** 엔진 하트비트(10초, SIGNAL·FILL 과 같은 직렬 큐 — 루프와 겹치지 않는다)의 `on_heartbeat` 가 구동한다(교차 리뷰 P1) — 재시도 간격이 지난 유지분을 오래 기다린 순으로 **모두**(on_signal 의 stale 루프와 같은 직렬 처리). 하트비트당 한 종목씩 처리하면 간격이 10초×종목 수로 늘어나 8종목이면 해제가 등록 후 820초까지 밀린다(교차 리뷰 4회차 P1 — 시험이 8종목 동시 유지를 SIGNAL·스케줄러 장부 없이 끝까지 구동한다). 대가로 다종목이 동시에 고착되면 그 하트비트 처리 동안 뒤의 FILL 이 기다린다. 하트비트는 우선순위 10 이라 큐가 포화되면 밀린다: 10초는 발행 주기이지 처리 보장 시한이 아니고, 시간 예산은 '그 뒤 처음 처리될 때' 적용된다. 유지분이 없으면 아무것도 하지 않는다 — 90초 폴백·10분 BUY 정리의 SIGNAL 의존은 종전 그대로다.
- **스로틀:** 엔진은 pending 등록 시각을 되감지 않고 별도 장부 `_pending_cancel_keep = _SellKeep(횟수, 마지막 시도 시각, 생존 로그 1회 여부, 직전 판정이 생존인가, 마지막 비-판단불가 시각)` 로 간격을 만든다. 간격은 **직전 판정**을 따른다(생존 확인 직후만 60초, 그 밖에는 20초) — '한 번이라도 생존을 봤는가'로 읽으면 생존 뒤의 판단 불가 재시도가 전부 60초 간격이 된다(교차 리뷰 3회차 P1) — 되감으면 헬스 모니터의 5분 교착 경보가 '손절이 막힌 SELL 유지'를 영영 보지 못한다. 폴백 횟수(`_pending_fallback_count`, 상한 2)와 장부를 나눠 유지가 시장가 폴백 예산을 깎지 않으며, `clear_pending`·`on_fill`(완결)·새 pending 등록·폴백 제출 성공에서 지운다. 스케줄러는 60초 스로틀 표식을 **생존 판정이 끝날 때까지** 유지한다 — 판정 `await` 중에 WS 틱·REST 폴링·독립 정리 루프(`run_pending_cleanup`, 60초)가 같은 종목에 취소·조회를 겹쳐 보내지 않게. 표식 `_stale_cancel_miss = (pending 등록 시각, 연속 판단불가 횟수, 경보 여부)` 는 등록 시각에 묶어 체결 등 다른 경로로 끝난 이전 pending 의 표식이 새 pending 에 새지 않는다.
...
**한계(잔존 위험)**
- **두 분할 표식은 서로 다른 시점의 의도다.** 스케줄러 표식은 ExitManager 가 `sell_partial` 로 **발행한** 시점, 엔진 표식은 엔진이 수량을 **채택한** 시점의 판단이고 스케줄러는 엔진의 판정을 읽지 않는다. 그 사이 잔고가 바뀌면(예: 100주 중 10주 익절을 발행했는데 채택 시점 보유가 10주) 엔진은 전량(종전 경로), 스케줄러는 분할(관문)로 어긋난다. 결과는 과매도가 아니라 **스케줄러 쪽 해제 지연**이다 — 판단 불가가 이어지면 종전 약 3분 해제가 약 5분으로(+약 120초), 거래소 생존이 계속 확인되면 상한 없이 유지된다(그 경우 살아 있는 주문이 곧 그 전량 청산이다). '전량 청산은 추가 지연 0'은 **발행 의도가 전량인 청산**(손절·트레일링·EOD)에 대한 말이고, 분할로 발행됐다가 수량상 전량이 된 경우는 이 예외에 든다.
...
- `_pending_fallback_count ≥ 2` 해제는 새 관문보다 앞이다 — 두 번째 시장가가 살아 있어도 pending 이 풀리는 기존 동작은 그대로다. `on_fill` 의 pending 수량 차감이 방향·주문 ID 를 가리지 않는 것(늦은 BUY 체결이 SELL pending 수량을 깎는다)도 기존 결함이다 — 분할 의도 표식은 발행 때 정해져 이후의 수량 변화로 바뀌지 않으므로, 전량으로 발행된 SELL 은 수량 장부가 어긋나도 종전 경로를 탄다.
