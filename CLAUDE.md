...
> 최종 업데이트: 2026-09-15 (MCP 런타임 제거 구현·검증 진행, 아직 미배포. 운영은 PR #61 `82b5039` 소스·문서 포함 checkout `76f7120`. MCP 후속 정본 `docs/reviews/mcp-retirement-2026-09-15.md`)
...
- **MCP 정리(사용자 승인, 구현·검증 진행/미배포):** 미사용 MCP 클라이언트·부팅·보조 조회를 제거한다. KIS 수급·일반 pykrx·네이버 뉴스·DART 직접 경로는 유지하고 수급/공매도/버즈 미획득을 검증 완료로 승격하지 않는다. 패키지 설치·주문·설정·킬스위치 변경 없음. 아래 PR #61의 MCP 경고 잔존은 당시 기록이며 후속 검증·배포 상태는 `docs/reviews/mcp-retirement-2026-09-15.md` 참고.
