...
- **T11(2026-09-15, 리뷰 PR 대기)**: 에이전트 팀 근거 계약(EvidenceItem, positive_basis≠risk_clear, 관측 시각 모르면 None)·판단 v2 shadow(`TeamAssessment`: 매수 매력/위험 허용/자료 충분성/진입 조건 분리, 합의≠확률, 확률 미보정)·EntryPlan(PendingSignal 정본, `check_entry_plan` shadow 기록만)·append-only 심의 원장·실행 상태 5단계·CF `team_buy_unfilled`·A/B/C 오프라인 러너(합성 검증만). 기존 TradeProposal/conviction/주문 경로 기준선 불변(특성화 테스트). 승인된 기존 경로 버그 수정 2건(vol_ratio 키, confidence=0 유효 소스). **투자 성능 미검증·운영 승격 없음·미배포.** 상세 `docs/agents/trading-team.md` T11 절, 계획서 `docs/superpowers/plans/2026-09-15-agent-team-evidence-entryplan.md`
