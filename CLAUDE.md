...
> **2026-09-27 N5 후속 정본:** [N5 진행 원장](docs/reviews/recovery-projection-progress-2026-09-27.md)을 먼저 읽는다. Task2 L1/L2는 구조 한정 승인(집중17·관련757 passed, 성능20셀 제외)이지만 후보 `721fe1c7…`의 새 controlled5k가 freeze13.023871ms로5ms 기준을 실패했다. 첫 실패에서 중단해 나머지6셀 미실행, 별도 trace1회는 미재현·미확정이며 재시도/수정0이다. 후보는 미커밋·전체 미승인, 과거 raw5k57.748ms와policy/100k 실패도 보존한다. [L3 설계 제안](docs/superpowers/specs/2026-09-27-recovery-build-lifecycle-design.md) 독립 한정 승인·사용자 확인 뒤 source/consumer를 읽기 전용 분석하고 [L3-P0 실행계획](docs/superpowers/plans/2026-09-27-recovery-lifecycle-proof.md)을 작성했다. warm SQL source·sequence 시험용 증명 계획이며 사용자 계획 검토 전이다. cold-open/decode/consumer 수명·전체 L3는 미해결, 제품/시험 실행0이다. Task3 FIFO gate는 독립 부품 승인(`9a01fa1`, 관련129 passed)만 받았고 실제 owner 배선·Task4 이후 통합·main·운영 전환은 차단이다. 아래 상태 줄은 각 시점 이력이다.
> **2026-09-23 N4 누적 상태 측정:** 정본은 [N4 원장](docs/reviews/recovery-scale-2026-09-23.md). tests/docs만 변경했다. 직렬16사례 중12완료·4timeout이며100건부터50ms 게이트 실패,5000건 캡처 거부로 **health/경보 배선 보류**다. 초기 오염 수치는 폐기했다. 다음은 원장 보존·revision별 projection/producer index의 critical 설계/인수이며 상한 증대·이력 삭제로 우회하지 않는다. 전체 검증/개발 통합은 원장의 실제 결과를 확인한다. 운영은 PR #90/#91/#92 main `a454277`(실행 제품 `e5ae602`,23:18:02 KST PID2386785)로 별도 배포했으며 주문·전략·위험·Toss grant는 무변경이다. 전체 engine 설치/승격 차단은 유지한다.
> **2026-09-23 N3 읽기 전용 진단 후속:** 신규 capture/diagnostic 모듈의 실제 후보·검증·리뷰·개발 통합 상태는 [N3 원장](docs/reviews/recovery-diagnostics-2026-09-23.md)이 정본이다. `mutation_in_flight`·`counts_complete`·unknown 모드를 함께 해석하고, 표본 안정성을 원자성/설치/복구 허가로 바꾸지 않는다. 운영 CLI/HTTP/경보·실 설치·main 전환은 미완이다. 아래 N2의 exporter 미구현 문구는 당시 이력이다.
> **2026-09-23 N2 정합화 후보·운영 미설치:** main의 TR/페이징·면제/stale 보호와 owner 경계를 함께 보존한 후보의 정본은 [N2 원장](docs/reviews/main-safety-alignment-2026-09-23.md)이다. 구 특성화26개 처분과 실제 heartbeat/일반 면제 SELL의 legacy writer 차단을 중복 구현하지 않는다. [수동 복구 진단](docs/operations/protection-recovery-diagnostics.md)은 읽기 전용 절차이며 exporter/CLI·자동 복구·설치 완료가 아니다. 최종 검증/리뷰/통합 여부는 원장의 실제 기록을 확인한다. main·운영 전환/공식 증거/전체 C/F/G/R 차단은 유지한다. 아래 N1 안내는 당시 이력이다.
> **2026-09-23 후속 N1 개발 검증 완료·운영 미설치:** 운영 반영 요청을 대조해 main의 문서2파일만 `afa6e1e`로 동기화했다(제품 diff0·재시작0). 개발선 통째 배포는 main PR #80/#81/#83/#84/#88 누락 및 설치 차단으로 보류했다. 후보 `06fc7ed`의 보호 전용 cooldown 분리는 독립 native·실제 Opus5 범위 검토와 전체 UTC/KST 각각5386 passed·기존xfail2·격리0을 통과했다. [N1 원장](docs/reviews/p1-protection-priority-2026-09-23.md)이 정본이다. 일반 시계가 보호 대기를 연장하지 못하되 원 intent·예약·UNKNOWN·복구/시작 장벽은 유지한다. 다음은 수동 복구 진단 계약·main 안전 행동 정합화이며 차단28 전체/29~31은 유지한다.
> **P1 후속 현재 정본(2026-09-23):** `docs/reviews/p1-producer-wiring-2026-09-23.md`의 실제 후보·검증·통합 상태를 먼저 읽는다. 사용자 위임으로 regular MARKET/closing LIMIT/영업일/20초 관측·제한된 외부 리뷰 예산을 정했으므로 아래 정책 미답 안내는 이력이다. 새 세션의 다음 개발은 `docs/operations/p1-next-steps-2026-09-23.md`. 개발 배선/합성 인수는 운영 설치·main 전환 승인이 아니며 실 API/주문·설정·배포·재시작 권한을 확대하지 않는다.
> **2026-09-23 P1 S1·S1′ 부품 개발:** runtime의 접수별 복구·체결 없는 pending 해제와 gateway의 보호 episode ID 분리를 개발했다. 독립 재현에서 발견한 관측체결→경제 적용 창의 익절 단계 유실을 수정했고, 두 부품 모두 native 한정 승인이다. **UTC/KST 각 5183 passed·기존 2 xfailed, 개발 통합 `77a3641`·운영 미설치.** 정본은 `docs/reviews/p1-components-2026-09-23.md`와 P1 계획 §6-1 이후다. S2 이후 시장가/closing 정책·외부 Claude 예산은 미답이며, main·배포·재시작·주문·설정 변경은 범위 밖이다. 차단23 전체 해소를 주장하지 않는다. 아래 상태 줄은 이전 시점 기록이다.
